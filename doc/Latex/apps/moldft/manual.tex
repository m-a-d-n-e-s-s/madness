...
Geometry optimization is no longer something {\tt moldft} does to itself.  It is a
task of its own, run through {\tt madqc}:

\begin{verbatim}
    madqc --optimize --wf=scf  deck.in
    madqc --optimize --wf=nemo deck.in
\end{verbatim}

\noindent{\tt --wf} names the reference method whose geometry is to be optimized.
By default the BFGS Hessian update is used, and the convergence test is on the
maximum element of the gradient (with translations and rotations projected out),
the change in the energy between iterations, and the maximum change in Cartesian
coordinates (all in atomic units).

All of its knobs live in the {\tt optimization} parameter group --- {\tt maxiter},
{\tt maxstep}, {\tt algopt}, {\tt etol}, {\tt gtol}, {\tt xtol},
{\tt value\_precision}, {\tt gradient\_precision}, {\tt initial\_hessian} --- see
{\tt madqc --print\_parameters=optimization}.  Anything left unset is derived from
the accuracy actually available: the energy thresholds from the wavefunction
threshold, the gradient thresholds from {\tt dconv}.  Setting them tighter than
the underlying gradient warrants makes the optimizer grind against its own noise
rather than converge, so a deck normally sets only {\tt maxiter}.

The former {\tt dft} keyvals {\tt gopt}, {\tt gtol}, {\tt gtest}, {\tt gval},
{\tt gprec}, {\tt gmaxiter}, {\tt ginitial\_hessian} and {\tt algopt} are retired.
They remain registered so that a deck still setting one fails with a message
naming its replacement, rather than silently computing a single point at the
input geometry.
...
{\tt plotlo lo} / {\tt plothi hi} --- E.g., {\tt plotlo 10} and {\tt plothi 12} --- Plots the molecular orbitals in the given inclusive range (default is none). Orbitals are numbered from zero and the range may extend into the virtuals when {\tt nvalpha} is set; for unrestricted calculations both spins are written ({\tt amo-} and {\tt bmo-} files).

{\tt plotdens} --- Plots the total electronic charge density and, if spin unrestricted, the spin density (default is off).

{\tt plotcoul} --- Plots the total (electronic + nuclear) potential in the electron convention, $v_{\rm nuc} + v_{\rm coul}$ (default is off).

{\tt plotesp} --- Plots the electrostatic potential felt by a positive test charge, $-(v_{\rm nuc} + v_{\rm coul})$ (default is off). $v_{\rm nuc}$ is the smoothed nuclear potential, so values within the smoothing length of a nucleus are not the bare Coulomb potential.

{\tt plotcube} --- Also writes Gaussian {\tt .cube} files (Avogadro, VMD, ParaView) next to the {\tt .dx} files, over the same box and grid (default is off).

{\tt npt\_plot value} --- E.g., {\tt npt\_plot 501} --- Sets the number of points used per dimension in the plotting grid (default 101).

{\tt plot\_cell [xlo,xhi,ylo,yhi,zlo,zhi]} --- E.g., {\tt plot\_cell [-10,10,-15,15,-10,5]} --- Sets the box (in atomic units) used for plotting (default is the entire simulation cell). All plot files use the calculation frame: {\tt moldft} translates (and by default rotates) the molecule before the calculation, so take atom positions from the cube-file header or the printed geometry, not from the input.
