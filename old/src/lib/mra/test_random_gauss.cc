...
public:
  ...
  double alpha_temp;
  std::vector<double> x_random, y_random, z_random, fac_random, alpha_random; 
  
  RandomParam() {
    x_random.resize(nmax);
    y_random.resize(nmax);
    z_random.resize(nmax);
    fac_random.resize(nmax);
    alpha_random.resize(nmax);
    for (int i = 0; i < nmax; i++) {
      x_random[i]     = RandomNumber<double>();
      y_random[i]     = RandomNumber<double>();
      z_random[i]     = RandomNumber<double>();
      fac_random[i]   = RandomNumber<double>();
      alpha_temp = RandomNumber<double>();
      alpha_random[i] = 1000000*sqrt(alpha_temp);
      //alpha_random[i] = RandomNumber<double>();
      cout << x_random[i] << " " << y_random[i] << " " << z_random[i] << " " << fac_random[i] << " " << alpha_random[i] << endl;
    }
  };
...
static RandomParam* globalRP;

double newfred(double x, double y, double z) {
  double function = 0.0, fac, alpha, x_save, y_save, z_save;
  x_save = x; 
  y_save = y; 
  z_save = z; 
  for(int i =0; i < (*globalRP).nmax; i++) {
    x -= (*globalRP).x_random[i];
    y -= (*globalRP).y_random[i];
    z -= (*globalRP).z_random[i];
    fac = (*globalRP).fac_random[i];
    alpha = (*globalRP).alpha_random[i];
    function += fac*exp(-1.0*alpha*(x*x+y*y+z*z)); 
    x = x_save; 
    y = y_save; 
    z = z_save; 
  }
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        comm.Barrier();
        papi::start();
        Function<double> f = FunctionFactory<double>(newfred).thresh(1e-3).nocompress().refine();
	//Function<double> f = FunctionFactory<double>().vf(vfred).thresh(1e-7).nocompress().refine();
        papi::stop("project + refine of f");
        Function<double> df,dfexact;

        print("valuesX",newfred(0.45,0.53,0.48),f(0.45,0.53,0.48));
        comm.Barrier();
        papi::start();
        f.compress();
        ...
        papi::stop("squaring");
        print("valuesSQ",newfred(0.45,0.53,0.48)*newfred(0.45,0.53,0.48),p(0.45,0.53,0.48));
