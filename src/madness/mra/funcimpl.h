...
//                woT::task(world.rank(),&implT::accumulate_timer,time,TaskAttributes::hipri());
        ...
        void do_apply(const opT* op, const keyT& key, const Tensor<R>& c) {
          ...
          if (op->range_restricted() && key.level() >= 1) {
            ...
            using filter_t = std::function<bool(Level, const Vector<std::optional<Translation>,opdim>&, std::optional<opkeyT> &)>;
            ...
            // N.B. for lattice-summed axes displacement is mapped back into the simulation cell
            filter_t filter = max_distsq_reached
                                  ? filter_t([op_domain_is_periodic,op_lattice_summed,range,default_distance_squared,max_distsq_reached,key](const Level level, const Vector<std::optional<Translation>, opdim>& dest, std::optional<Key<opdim>> &displacement) -> bool {
                                      ...
                                      const bool dest_is_in_domain = [&]() {
                                        for(auto d=0; d!=opdim; ++d) {
                                          ...
                                          if (op_domain_is_periodic[d] || op_lattice_summed[d]) continue;
                                      ...
                                      if (dest_is_in_domain) {
                                        if (displacement.has_value()) {
                                          ...
                                          if (among_standard_displacements) {
                                            ...
                                            if (distsq > max_distsq_reached) { // among standard displacements => keep if longer than the longest standard displacement considered
                                              return true;
                                            } {
                                              return false;
                                            }
            ...
            BoxSurfaceDisplacementRange<opdim>
                range_boundary_face_displacements(opkey, box_radius,
                                                  surface_thickness,
                                                  op_lattice_summed,  // along lattice-summed axes treat the box as periodic, make displacements to one side of the box
                                                  filter);
            for_each(
