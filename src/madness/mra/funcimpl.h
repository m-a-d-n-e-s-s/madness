...
template<size_t NDIM>
    ...
    private:
        ...
        // --- neighbor halo (KE-matrix differentiate redesign, Variant B; gated by MAD_KEMAT_HALO in
        //     SCF::kinetic_energy_matrix). Pre-staged REMOTE same-level neighbor coeffs so the derivative
        //     operator's find_neighbor() serves them locally instead of a cross-rank sock_it_to_me.
        //     Value: has_data() => leaf (case a); empty => interior marker (case b, triggers descent);
        //     absent => walk-up (case c) => find_neighbor falls back to the pull. See kemat_redesign.md §9.
        //     mutable: populated/read via const paths (like sock_it_to_me). Built and read across a fence,
        //     so concurrent access is inserts-only (worker tasks) then reads-only (diff) -> lock-free.
        mutable ConcurrentHashMap<keyT, coeffT> neighbor_halo_;
        mutable bool neighbor_halo_enabled_ = false;

        // Disable the default copy constructor
        FunctionImpl(const FunctionImpl<T,NDIM>& p);

    public:
        // --- neighbor halo API (Variant B); all const (modify mutable members), like sock_it_to_me ---
        void halo_enable()  const { neighbor_halo_enabled_ = true; }
        bool halo_enabled() const { return neighbor_halo_enabled_; }
        void halo_clear()   const { neighbor_halo_enabled_ = false; neighbor_halo_.clear(); }
        /// AM handler: insert pushed neighbor nodes into the local halo (runs as a worker-thread task).
        ...
        /// probe the halo; on hit copy the coeff (empty for an interior marker) and return true.
