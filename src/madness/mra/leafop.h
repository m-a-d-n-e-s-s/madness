...
namespace madness {
  ...
  public:
	...
	Level ll=get_half_of_special_level(f->get_special_level());
  ...
  public:
	...
	if(this->box_is_at_boundary(key)) return false;
      ...
      Level ll=this->get_half_of_special_level();
      if(ll<f->get_initial_level()) ll = f->get_initial_level();
      if(key.level()>ll){
  ...
  public:
	...
	// use adaptive scheme: if we are at a low level refine also neighbours
	Level ll=this->get_half_of_special_level(f->get_special_level());
	if(ll<f->get_initial_level()) ll = f->get_initial_level();
	if(key.level()>ll){
