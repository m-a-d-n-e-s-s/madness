...
struct OperatorInfo {
    ...
    OperatorInfo(double mu, double lo, double thresh, OpType type, std::optional<bool> truncate = {}, std::vector<std::optional<unsigned int>> range = std::vector<std::optional<unsigned int>>(3)) : mu(mu), lo(lo), thresh(thresh), type(type), truncate_lowexp_gaussians(truncate), range(range) { }
    double mu=0.0;     ///< some introspection
    double lo=1.e-5;
    double thresh=1.e-4;
    OpType type=OT_UNDEFINED;    ///< introspection
    double hi=-1.0;
    std::vector<std::optional<unsigned int>> range = std::vector<std::optional<unsigned int>>(3);
    bool debug=false;
    ...
    std::array<std::optional<unsigned int>, NDIM> range_as_array() const {
      std::array<std::optional<unsigned int>, NDIM> result;
