...
/// @param[in]     rcondtol rcond less than this will cause the subspace to be
/// shrunk due to linear dependence
...
class NonlinearSolverND {
 ...
 public:
  void set_maxsub(const unsigned int& new_maxsub) { maxsub = new_maxsub; }
  ...
  /// @param[in]          rcondtol rcond less than this will cause the subspace
  /// to be shrunk due to linear dependence
  /// @param[in]          cabsmax  maximum element of c greater than this will
  /// cause the subspace to be shrunk due to linear dependence
  Function<double, NDIM> update(const Function<double, NDIM>& u,
...
/// Generalized version of NonlinearSolver not limited to a single madness
/// function
...
class XNonlinearSolver {
 ...
 public:
  ...
  XNonlinearSolver(const Alloc& alloc = Alloc(), bool print = false)
      : maxsub(10), alloc(alloc), do_print(print) {}

  XNonlinearSolver(const XNonlinearSolver& other)
      : maxsub(other.maxsub), alloc(other.alloc), do_print(other.do_print) {}
