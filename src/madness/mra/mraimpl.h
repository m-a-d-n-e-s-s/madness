...
//        this->on_demand=false;
    ...
    /// the tolerance to the cell's aspect ratio rather than to its resolution. See
    /// FunctionDefaults::set_truncate_scale_by_min_width() for the old behaviour and
    /// what it cost. Cubic cells are unaffected: there the two agree.
    template <typename T, std::size_t NDIM>
    double FunctionImpl<T,NDIM>::truncate_tol(double tol, const keyT& key) const {
        ...
        if (truncate_mode == 0) {
            ...
            double L = FunctionDefaults<NDIM>::get_truncate_cell_width();
...
template <typename T, std::size_t NDIM>
    ...
    template <std::size_t NDIM> bool FunctionDefaults<NDIM>::truncate_scale_by_min_width = false;
