...
namespace madness
{
    ...
    public:
        ...
        tupleT copy_batch(const tupleT &arg) const
        {
            ...
            constexpr std::size_t index = (appearance == 0) ? get_index_of_first_vector_argument<tupleT, 0>() : get_index_of_second_vector_argument<tupleT, 0>();
        ...
        {
            std::stringstream ss;
            if (batch.is_full_size())
                ss << "[  ----  )";
            else
                ss << "[" << std::setw(3) << batch.begin << ", " << std::setw(3) << batch.end << ")";
            os << ss.str();
            return os;
        }
    };
    ...
    public:
        ...
        tupleT copy_input_batch(const tupleT &arg) const
        {
            ...
            if (input.size() > 1)
                arg1 = input[1].template copy_batch<tupleT, 1>(arg1);
    ...
    public:
        ...
        std::size_t result_stride = 1;   ///< stride of the result vector
        ...
        partitionT partition_tasks(const tupleT &argtuple) const
        {
            ...
            if constexpr (I2 < std::tuple_size_v<tupleT>)
            { // found at least 2 vectors of madness functions
                constexpr std::size_t I2 = get_index_of_second_vector_argument<tupleT, 0>();
                vsize2 = std::get<I2>(argtuple).size();
            }
            if constexpr (I1 < std::tuple_size_v<tupleT>)
            { // found at least 1 vector
                constexpr std::size_t I1 = get_index_of_first_vector_argument<tupleT, 0>();
        ...
        partitionT do_1d_partition(const std::size_t vsize, const std::string policy) const
        {
            ...
            else if (policy == "strided")
            {
                ...
                while (end < long(vsize))
                {
                    ...
                    Batch batch(Batch_1D(begin, end, result_stride), Batch_1D(begin, end, result_stride));
                    double priority = MacroTaskPartitioner::compute_priority(batch);
                    result.push_back(std::make_pair(batch, priority));
                    begin = end;
                }
