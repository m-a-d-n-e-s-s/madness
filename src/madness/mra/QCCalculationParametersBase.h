...
struct QCParameter {
 public:
  ...
  QCParameter(const std::string v, const std::string t,
              const std::string comment = "",
              const std::vector<std::string> allowed_values1 = {})
      : default_value(v),
        type(t),
        comment(comment),
        allowed_values(allowed_values1) {
    static int i = 0;
    print_order = i++;
    set_all();
  }
  ...
  std::string print_precedence() const {
    if (precedence == def)
      return "default";
    if (precedence == derived)
      return "derived";
    if (precedence == defined)
      return "defined";
    std::stringstream sprecedence;
    sprecedence << precedence;
    throw std::runtime_error("unknown precedence in QCParameter" +
                             sprecedence.str());
    return "darn";
  }
  ...
  std::string print_line(const std::string& key) const {
    ...
    // key-value block
    std::string keyval = fill_left(20, key) + "  " +
                         fill_right(10, get_value()) + " # " +
    ...
    for (auto word : commentwords) {
      ...
      if (is_first_line and leave_space_for_allowed_values)
        thislinebreak = 80;
    ...
    for (size_t i = 0; i < commentlines.size(); ++i) {
      if (i == 0)
        result = keyval + fill_right(40, commentlines[i]) + allowed_val;
      else
  ...
  void serialize(Archive& ar) {
    ar & value & default_value & derived_value & user_defined_value & type &
        null & comment & allowed_values & print_order & precedence;
  }
 ...
 private:
  void set_all() {
    value = default_value;
    if (derived_value != null)
      value = derived_value;
    if (user_defined_value != null)
      value = user_defined_value;
    if (not check_allowed())
      throw std::invalid_argument(not_allowed_errmsg());
  }
  ...
  int print_order =
      0;  // use this for printing the parameters in the same order as they are defined
};
...
class QCCalculationParametersBase {
 ...
 protected:
  ...
  virtual void read_input_and_commandline_options(
      World& world, const commandlineparser& parser, const std::string tag) {
    try {
      // check that user-defined input files actually exist
      bool file_ok = true;
      if (parser.key_exists("user_defined_input_file"))
        file_ok = file_exists(world, parser.value("input"));
      if (file_ok)
        read_input(world, parser.value("input"), tag);
      else {
        std::string msg =
            "could not find user-defined input file: " + parser.value("input") +
            "\n";
 ...
 private:
  ...
  void read_input(World& world, const std::string filename,
                  const std::string tag);

  void read_commandline_options(World& world, const commandlineparser& parser,
                                const std::string tag);
 ...
 protected:
  ...
  QCCalculationParametersBase(const QCCalculationParametersBase& other)
      : parameters(other.parameters), print_debug(other.print_debug) {}

  /// destructor
  virtual ~QCCalculationParametersBase() {}

  template <typename T>
  void initialize(const std::string& key, const T& value,
                  const std::string comment = "",
    ...
    for (auto av : allowed_values) {
      ...
      std::transform(av_lower.begin(), av_lower.end(), av_lower.begin(),
                     ::tolower);
    ...
    parameters.insert(std::make_pair<std::string, QCParameter>(
        ...
        QCParameter(svalue, type, comment, av_lower_vec)));
  }
 ...
 public:
  ...
  void set_derived_value(const std::string& key, const T& value) {
    ...
    if (not check_type_silent<T>(parameter)) {
      throw std::runtime_error("type error in set_derived_value for key " +
                               key);
    }
 ...
 protected:
  template <typename T>
  bool try_setting_user_defined_value(const std::string& key,
                                      const std::string& val) {

    if (not check_type_silent<T>(get_parameter(key)))
      return false;

    if (print_debug)
      ::madness::print("key:", key, "will set type",
 ...
 public:
  ...
  bool operator==(const QCCalculationParametersBase& other) const {
    ...
    for (auto& p : parameters) {
      ...
      if (param_type == "i") {
        if (get<int>(p.first) != other.get<int>(p.first)) {
          return false;
        };
        // if vector of double
      } else if (param_type == "d") {
        if (get<double>(p.first) != other.get<double>(p.first)) {
          return false;
        };
        // if vector of bool
      } else if (param_type == "b") {
        if (get<bool>(p.first) != other.get<bool>(p.first)) {
          return false;
        };
        // if vector of doubles?
      } else if (param_type == "St6vectorIdSaIdEE") {
        ...
        if (get<size_t>(p.first) != other.get<size_t>(p.first)) {
          return false;
        };
      }
    }
