...
//
#ifndef __MADNESS_MRA_FUSET_MATRIXINNERPRE_OP__INCLUDED__
#define __MADNESS_MRA_FUSET_MATRIXINNERPRE_OP__INCLUDED__
...
namespace madness 
{
	...
	class MatrixInnerPreOp : public PrimitiveOp<T,NDIM> 
    {
    ...
    public:
	MatrixInnerPreOp	(string opName, KTREE* output, const std::vector<KTREE>& f, const std::vector<KTREE>& g, bool sym);
	...
	//!Variables for MatrixInnerPreOp
    ...
    template<typename T, std::size_t NDIM>
	MatrixInnerPreOp<T,NDIM>::MatrixInnerPreOp(string opName, KTREE* output, const std::vector<KTREE>& f, const std::vector<KTREE>& g, bool sym)
    ...
    template <typename T, std::size_t NDIM>
	FuseTContainer<T>
	MatrixInnerPreOp<T,NDIM>::compute(const keyT& key, const FuseTContainer<T> &s)
    {
    ...
    template<typename T, std::size_t NDIM>
	bool 
	MatrixInnerPreOp<T,NDIM>::isDone(const keyT& key) const 
    {
	bool isE1;
	bool isE2;
    ...
    template<typename T, std::size_t NDIM>
	void  
	MatrixInnerPreOp<T,NDIM>::reduce(World& world){
        world.gop.sum(_r->ptr(),_left.size()*_right.size());
    }

}; /*fuset*/

#endif /* __fuset_MatrixInnerPreOp_h__ */
