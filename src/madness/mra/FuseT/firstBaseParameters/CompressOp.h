...
namespace madness 
{
    ...
    template<typename T, std::size_t NDIM>
	Future<GenTensor<T>>
	CompressOp<T,NDIM>::computeB(const keyT& key, const BaseParameters<T, NDIM> &s)
	{	
	...
	CompressOp<T,NDIM>::computeC(const keyT& key, const BaseParameters<T, NDIM> &s) 
	{ 
		if ( key == _i1->world().rank())
		{
			if (_i1->is_compressed() || _i1->get_impl() == NULL) 
		...
		{
			// Leaf node
			Future<BaseParameters<T,NDIM>> result(node.coeff());
			if (!_keepleaves)
				node.clear_coeff();
		...
		Future<BaseParameters<T,NDIM>> temp;	// enough??
		return temp;
	}
	...
	template <typename T, std::size_t NDIM>
	Future<BaseParameters<T,NDIM>>
	CompressOp<T,NDIM>::afterComputeC(const keyT& key, const std::vector<Future<BaseParameters<T,NDIM>>> &v)
	{
		// Copy child scaling coeffs into contiguous block
		tensorT d(this->_cdata.v2k);
		...
		{
			//d(child_patch(kit.key())) += v[i].get().full_tensor_copy();	
			d(child_patch(kit.key())) += v[i].get()._coeff.full_tensor_copy();
		}
		...
		Future<BaseParameters<T,NDIM>> temp(ss);
		return temp;
	}
