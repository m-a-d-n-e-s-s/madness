...
namespace madness 
{
    ...
    public:
		...
		Future<BaseParameters<T>>	afterComputeC	(const keyT& key, const std::vector<Future<BaseParameters<T>>> &v);
...
*/
	...
	InnerOp<T,NDIM>::computeC(const keyT& key, const BaseParameters<T> &s) 
	{
	...
	template <typename T, std::size_t NDIM>
	Future<BaseParameters<T>>
	InnerOp<T,NDIM>::afterComputeC(const keyT& key, const std::vector<Future<BaseParameters<T>>> &v)
	{
