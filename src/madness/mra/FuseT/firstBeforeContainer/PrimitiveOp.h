...
//    struct DependencyInfo<double>;
	...
	virtual BaseParameters<T,NDIM> compute(const keyT& key, const BaseParameters<T,NDIM> &s)				= 0;
	//virtual Future<GenTensor<T>> computeB(const keyT& key, const BaseParameters<T, NDIM> &s)				= 0;
	virtual Future<BaseParameters<T,NDIM>> computeC(const keyT& key, const BaseParameters<T, NDIM> &s)		= 0;
	//virtual Future<GenTensor<T>> afterComputeB(const keyT& key, const std::vector<Future<GenTensor<T>>> &v) = 0;
	virtual Future<BaseParameters<T,NDIM>> afterComputeC(const keyT& key, const std::vector<Future<BaseParameters<T,NDIM>>> &v) = 0;
