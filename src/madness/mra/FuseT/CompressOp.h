...
namespace madness 
{
	...
	CompressOp<T,NDIM>::computeC(const keyT& key, const BaseParameters<T> &s) 
	{ 
		...
		{
			// Leaf node
			Future<BaseParameters<T>> result(node.coeff());
			if (!_keepleaves)
				node.clear_coeff();
		...
		Future<BaseParameters<T>> temp;	// enough??
		return temp;
	}
	...
	template <typename T, std::size_t NDIM>
	Future<BaseParameters<T>>
	CompressOp<T,NDIM>::computeC(const keyT& key, const std::vector<Future<BaseParameters<T>>> &v)
	{
		if (key == _i1->world().rank())
		{
			if (_i1->is_compressed() || _i1->get_impl() == NULL)
			{
				cout << "ERROR: it is already compressed or does not have FunctionImpl" << endl;
				exit(0);
			}
		...
		// targs < Type of tensor to be used in the FunctionNode
		...
		//	Making Future form
		Future<BaseParameters<T>> temp(ss);
		return temp;
	}
