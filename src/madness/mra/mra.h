...
class Function : public archive::ParallelSerializableObject {
 ...
 public:
  ...
  /// An unitialized function can only be assigned to.  Any other operation will
  /// throw.
  Function() : impl() {}
  ...
  /// Destruction of any underlying implementation is deferred to next global
  /// fence.
  ~Function() {}

  /// Evaluates the function at a point in user coordinates.  Possible
  /// non-blocking comm.
  ...
  /// Evaluate function only if point is local returning (true,value); otherwise
  /// return (false,0.0)
  ...
  /// Evaluates the function rank at a point in user coordinates.  Possible
  /// non-blocking comm.
  ...
  /// Evaluates a cube/slice of points (probably for plotting) ... collective
  /// but no fence necessary
  ...
  /// @param[in] cell A Tensor describe the cube where the function to be
  /// evaluated in
  ...
  /// Evaluates the function at a point in user coordinates.  Collective
  /// operation.
  ...
  /// Returns an estimate of the difference ||this-func|| ... global sum
  /// performed

  /// If the function is compressed, it is reconstructed first.  For efficient
  /// use especially with many functions, reconstruct them all first, and use
  /// errsq_local instead so you can perform a global sum on all at the same
  /// time.
  /// @param[in] func Templated interface to the a user specified function
  template <typename funcT>
  ...
  /// Returns the number of coefficients in the function ... collective global
  /// sum
  ...
  /// Truncate the function with optional fence.  Compresses with fence if not
  /// compressed.

  /// If the truncation threshold is less than or equal to zero the default
  /// value specified when the function was created is used. If the function is
  /// not initialized, it just returns.
  ///
  /// Returns this for chaining.
  /// @param[in] tol Tolerance for truncating the coefficients. Default 0.0
  /// means use the implimentation's member value \c thresh instead.
  ...
  /// Replace current FunctionImpl with a new one using the same parameters &
  /// map as f
  ...
  /// Returns the 2-norm of the function ... global sum ... works in either
  /// basis
  ...
  /// Compresses the function, transforming into wavelet basis.  Possible
  /// non-blocking comm.

  /// By default fence=true meaning that this operation completes before
  /// returning, otherwise if fence=false it returns without fencing and the
  /// user must invoke world.gop.fence() to assure global completion before
  /// using the function for other purposes.
  ///
  ...
  /// Compresses the function retaining scaling function coeffs.  Possible
  /// non-blocking comm.
  ...
  /// Converts the function from nonstandard form to standard form.  Possible
  /// non-blocking comm.
  ...
  /// Reconstructs the function, transforming into scaling function basis.
  ...
  /// Sums scaling coeffs down tree restoring state with coeffs only at leaves.
  /// Optional fence.  Possible non-blocking comm.
  ...
  /// Process 0 prints a graphviz-formatted output of all nodes in the tree
  /// (collective)
  ...
  /// Inplace, scale the function by a constant.  No communication except for
  /// optional fence.
  ...
  /// Inplace, general bi-linear operation in wavelet basis.  No communication
  /// except for optional fence.

  /// If the functions are not in the wavelet basis an exception is thrown since
  /// this routine is intended to be fast and unexpected compression is assumed
  ...
  /// to choose an appropriate state with performance, usually compressed for
  /// 3d, reconstructed for 6d)
  template <typename Q>
  ...
  inner_local(const Function<R, NDIM>& g) const {
    PROFILE_MEMBER_FUNC(Function);
    MADNESS_ASSERT(is_compressed());
  ...
  /// With this being an on-demand function, fill the MRA tree according to
  /// different criteria

  /// @param[in]  g   the function after which the MRA structure is modeled (any
  /// basis works)
  template <typename R>
  ...
  template <typename R>
  TENSOR_RESULT_TYPE(T, R)
  inner(const Function<R, NDIM>& g) const {
    PROFILE_MEMBER_FUNC(Function);
  ...
  /// Return the local part of inner product with external function ... no
  /// communication. If you are going to be doing a bunch of inner_ext calls,
  /// set keep_redundant to true and then manually undo_redundant when you are
  /// finished.
  /// @param[in] f Pointer to function of type T that take coordT arguments.
  ...
  /// @return Returns local part of the inner product, i.e. over the domain of
  /// all function nodes on this compute node.
  T inner_ext_local(const std::shared_ptr<FunctionFunctorInterface<T, NDIM> > f,
                    const bool leaf_refine = true,
                    const bool keep_redundant = false) const {
    PROFILE_MEMBER_FUNC(Function);
  ...
  /// Return the inner product with external function ... requires
  /// communication. If you are going to be doing a bunch of inner_ext calls,
  /// set keep_redundant to true and then manually undo_redundant when you are
  /// finished.
  /// @param[in] f Reference to FunctionFunctorInterface. This is the externally
  /// provided function
  /// @param[in] leaf_refine boolean switch to turn on/off refinement past leaf
  /// nodes
  /// @param[in] keep_redundant boolean switch to turn on/off undo_redundant
  /// @return Returns the inner product
  T inner_ext(const std::shared_ptr<FunctionFunctorInterface<T, NDIM> > f,
              const bool leaf_refine = true,
              const bool keep_redundant = false) const {
    PROFILE_MEMBER_FUNC(Function);
    if (not impl->is_redundant()) impl->make_redundant(true);
  ...
  /// Return the local part of gaxpy with external function, this*alpha + f*beta
  /// ... no communication.
  ...
  template <typename R>
  TENSOR_RESULT_TYPE(T, R)
  inner_on_demand(const Function<R, NDIM>& g) const {
  ...
  /// @param[in]  dim over which dimensions to be integrated: 0..LDIM-1 or
  /// LDIM..NDIM-1
  ...
  /// Replaces this function with one loaded from an archive using the default
  /// processor map
  ...
  void load(World& world, Archive& ar) {
    PROFILE_MEMBER_FUNC(Function);
    // Type checking since we are probably circumventing the archive's own type
    // checking
  ...
  /// (*this) is just a dummy Function to be able to call internal methods in
  /// FuncImpl
  ...
  /// Multiplication of function * vector of functions using recursive algorithm
  /// of mulxx
  ...
  /// Same as \c operator* but with optional fence and no automatic
  /// reconstruction
...
template <typename L, typename R, std::size_t NDIM>
Function<TENSOR_RESULT_TYPE(L, R), NDIM> mul(const Function<L, NDIM>& left,
...
/// Multiplies two functions with the new result being of type
/// TensorResultType<L,R>
...
/// addends reconstructed
template <typename T, std::size_t NDIM>
Function<T, NDIM> gaxpy_oop_reconstructed(const double alpha,
...
/// Create a new copy of the function with the same distribution and optional
/// fence
template <typename T, std::size_t NDIM>
...
/// Type conversion implies a deep copy.  No communication except for optional
/// fence.

/// Works in either basis but any loss of precision may result in different
/// errors in applied in a different basis.
///
/// The new function is formed with the options from the default constructor.
///
/// There is no automatic type conversion since this is generally a rather
/// dangerous thing and because there would be no way to make the fence
/// optional.
...
/// Return the complex conjugate of the input function with the same
/// distribution and optional fence
...
template <typename T>
Function<T, 6> swap_particles(const Function<T, 6>& f) {
...
///                 (anti-) symmetric particle permutation ("sy_particle",
...
Function<T, NDIM> multiply(const Function<T, NDIM> f, const Function<T, LDIM> g,
  ...
  //        if (particle==1) result.print_size("finished multiplication
  //        f(1,2)*g(1)"); if (particle==2) result.print_size("finished
  //        multiplication f(1,2)*g(2)");

  return result;
}

template <typename T, std::size_t NDIM>
Function<T, NDIM> project(const Function<T, NDIM>& other,
...
TENSOR_RESULT_TYPE(T, R)
inner(const Function<T, NDIM>& f, const Function<R, NDIM>& g) {
  PROFILE_FUNC;
  return f.inner(g);
}

/// Computes the scalar/inner product between an MRA function and an external
/// functor
...
/// @result     inner(f,g)
template <typename T, typename opT, std::size_t NDIM>
TENSOR_RESULT_TYPE(T, typename opT::value_type)
inner(const Function<T, NDIM>& f, const opT& g) {
  PROFILE_FUNC;
...
template <typename T, typename opT, std::size_t NDIM>
TENSOR_RESULT_TYPE(T, typename opT::value_type)
inner(const opT& g, const Function<T, NDIM>& f) {
  return inner(f, g);
}
...
/// Create a new function that is the square of f - global comm only if not
/// reconstructed
template <typename T, std::size_t NDIM>
