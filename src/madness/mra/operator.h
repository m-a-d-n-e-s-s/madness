...
std::vector<Function<TENSOR_RESULT_TYPE(T, R), NDIM>> apply(const SeparatedConvolution<T, KDIM>& op,
                                                            const std::vector<Function<R, NDIM>> f);

template <typename T, std::size_t NDIM>
std::vector<CCPairFunction<T, NDIM>> apply(const SeparatedConvolution<T, NDIM>& op,
                                           const std::vector<CCPairFunction<T, NDIM>>& argument);

template <typename T, std::size_t NDIM>
std::vector<CCPairFunction<T, NDIM>> apply(const SeparatedConvolution<T, NDIM / 2>& op,
                                           const std::vector<CCPairFunction<T, NDIM>>& argument);

template <typename T, std::size_t NDIM>
CCPairFunction<T, NDIM> apply(const SeparatedConvolution<T, NDIM>& op,
                              const CCPairFunction<T, NDIM>& argument);

template <typename T, std::size_t NDIM>
CCPairFunction<T, NDIM> apply(const SeparatedConvolution<T, NDIM / 2>& op,
                              const CCPairFunction<T, NDIM>& argument);
...
Most of the code is about caching these transition matrices. They are cached (key of the map is the
displacement) in the SimpleCache "data", which is of type SeparatedConvolutionData, which keeps the
matrices for all separated terms and dimensions. These SeparatedConvolutionData are constructed
using ConvolutionND "ops", which is constructed at the construction of the SeparatedConvolution.
                ...
                construction                                            storage
                                                                ...
                                                                SimpleCache<SeparatedConvolutionData>
                                                                    (all terms, all dim) / (all
disp) vector<ConvolutionND> (1 term, all dim) / (all terms) vector<SeparatedConvolutionInternal> (1
term, all dim) / (all terms)
...
Why we need SeparatedConvolutionInternal in the first place I have no idea. ConvolutionND has the
global factor, and SeparatedConvolutionInternal has a norm.
...
private:
  array_of_bools<NDIM>
      lattice_summed_; ///< If lattice_summed_[d] is true, sum over lattice translations along axis
                       ///< d N.B. the resulting kernel can be non-zero at both ends of the
                       ///< simulation cell along that axis
  array_of_bools<NDIM> domain_is_periodic_{
      false}; ///< If domain_is_periodic_[d]==false and lattice_summed_[d]==false,
              ///< ignore periodicity of BC when applying this to function
  std::array<KernelRange, NDIM>
      range; ///< kernel range is along axis d is limited by range[d] if it's nonnull
...
private:
  mutable std::vector<ConvolutionND<Q, NDIM>>
      ops; ///< ConvolutionND keeps data for 1 term, all dimensions, 1 displacement
  const int k;
  ...
  mutable SimpleCache<SeparatedConvolutionData<Q, NDIM>, NDIM>
      data; ///< cache for all terms, dims and displacements
  mutable SimpleCache<SeparatedConvolutionData<Q, NDIM>, 2 * NDIM>
      mod_data; ///< cache for all terms, dims and displacements
...
public:
  ...
  {
    if (p != 1 and p != 2)
      throw std::runtime_error("particle must be 1 or 2");
    particle_ = p;
    return *this;
  }
  ...
  {
    return std::any_of(range.begin(), range.end(), [](const auto& v) { return v.finite(); });
  }
...
private:
                          ...
                          const array_of_bools<NDIM>& lattice_summed)
  {
    ...
    if (info.truncate_lowexp_gaussians.value_or(lattice_summed_any))
    {
      ...
      // the largest spacing along lattice summed axes thus controls the smallest Gaussian exponent
      // that NEEDS to be included
      double max_lattice_spacing = 0;
      for (int d = 0; d != NDIM; ++d)
      {
        if (lattice_summed[d])
          max_lattice_spacing = std::max(max_lattice_spacing, cell_width(d));
      }
  ...
  //        static Tensor<Q> operator_patch(const Translation& source, const Translation& disp,
  //        const Tensor<Q>& upop) {
  //
  ...
  void apply_transformation(long dimk, const Transformation trans[NDIM], const Tensor<T>& f,
                            Tensor<R>& work1, Tensor<R>& work2, const Q mufac,
  ...
  /// accumulate into result
  template <typename T, typename R>
  void apply_transformation3(const Tensor<T> trans2[NDIM], const Tensor<T>& f, const Q mufac,
                             Tensor<R>& result) const
  {
  ...
  template <typename T, typename R>
  void apply_transformation2(Level n, long dimk, double tol, const Tensor<T> trans2[NDIM],
                             const GenTensor<T>& f, GenTensor<R>& work1, GenTensor<R>& work2,
                             const Q mufac, GenTensor<R>& result) const
  {
  ...
  template <typename T>
  void muopxv_fast(ApplyTerms at, const ConvolutionData1D<Q>* const ops_1d[NDIM],
                   const Tensor<T>& f, const Tensor<T>& f0,
                   Tensor<TENSOR_RESULT_TYPE(T, Q)>& result,
                   Tensor<TENSOR_RESULT_TYPE(T, Q)>& result0, const double tol, const Q mufac,
                   ...
                   Tensor<TENSOR_RESULT_TYPE(T, Q)>& work2) const
  {
    ...
    if (at.r_term and (Rnorm > 1.e-20))
    {
      ...
      long break_even;
      if (NDIM == 1)
        break_even = long(0.5 * twok);
      else if (NDIM == 2)
        break_even = long(0.6 * twok);
      else if (NDIM == 3)
        break_even = long(0.65 * twok);
      else
        break_even = long(0.7 * twok);
...
#ifdef USE_GENTENSOR
          r = std::max(2L, r + (r & 1L)); // (needed for 6D == when GENTENSOR is on) NOLONGER NEED
                                          // TO FORCE OPERATOR RANK TO BE EVEN
#endif
          if (r == 0)
          {
            rank_is_zero = true;
    ...
    if (at.t_term and (Tnorm > 0.0))
    {
      ...
      long break_even;
      if (NDIM == 1)
        break_even = long(0.5 * k);
      else if (NDIM == 2)
        break_even = long(0.6 * k);
      else if (NDIM == 3)
        break_even = long(0.65 * k);
      else
        break_even = long(0.7 * k);
      bool rank_is_zero = false;
      for (std::size_t d = 0; d < NDIM; ++d)
      {
        long r;
...
#ifdef USE_GENTENSOR
          r = std::max(2L, r + (r & 1L)); // (needed for 6D == GENTENSOR is USED) NOLONGER NEED TO
                                          // FORCE OPERATOR RANK TO BE EVEN
#endif
          if (r == 0)
          {
            rank_is_zero = true;
  ...
  /// Apply one of the separated terms, accumulating into the result
  template <typename T>
  void muopxv_fast2(Level n, const ConvolutionData1D<Q>* const ops_1d[NDIM], const GenTensor<T>& f,
                    const GenTensor<T>& f0, GenTensor<TENSOR_RESULT_TYPE(T, Q)>& result,
                    GenTensor<TENSOR_RESULT_TYPE(T, Q)>& result0, double tol, const Q mufac,
                    ...
                    GenTensor<TENSOR_RESULT_TYPE(T, Q)>& work2) const
  {
    ...
    if (n > 0 and (Tnorm > 1.e-20))
    {
      ...
      for (std::size_t d = 0; d < NDIM; ++d)
      {
        ...
        //                    }
        //                    else {
        //                        //r += std::max(2L,r&1L); // NOLONGER NEED TO FORCE OPERATOR RANK
        //                        TO BE EVEN trans[d].r = r; trans[d].U = ops_1d[d]->TU.ptr();
        //                        trans[d].VT = ops_1d[d]->TVT.ptr();
        //                    }
        trans2[d] = ops_1d[d]->T;
      }
  ...
  double munorm2_modified(Level n, const ConvolutionData1D<Q>* ops_1d[]) const
  {
    ...
    for (int i = 1; i < static_cast<int>(NDIM) - 1; ++i)
      factorial *= double(i);
    dff *= factorial;
    duu *= factorial;
  ...
  const SeparatedConvolutionData<Q, NDIM>* getop(Level n, const Key<NDIM>& d,
                                                 const Key<NDIM>& source) const
  {
  ...
  /// @return pointer to cached operator
  const SeparatedConvolutionData<Q, NDIM>* getop_modified(Level n, const Key<NDIM>& disp,
                                                          const Key<NDIM>& source) const
  {
    // PROFILE_MEMBER_FUNC(SeparatedConvolution); // Too fine grain for routine profiling

    // in the modified NS form the upsampled part of the operator depends on the modulus of the
    // source
    ...
    SeparatedConvolutionData<Q, NDIM> op(rank);
    for (int mu = 0; mu < rank; ++mu)
      op.muops[mu] = getmuop_modified(mu, n, disp, source);
  ...
  /// @param[in]  key     the key indicating the child -- only some dimensions will be
  /// "reproductive"
  ...
  GenTensor<T> partial_upsample(const Key<FDIM>& key, const GenTensor<T>& coeff,
                                const int particle) const
  {
    ...
    if (particle == 0)
    {
      for (size_t ii = 0; ii < NDIM; ++ii)
        matrices[ii] = h[key.translation()[ii] % 2];
      for (size_t ii = 0; ii < NDIM; ++ii)
        matrices[ii + NDIM] = identity;
    }
    else if (particle == 1)
    {
      for (size_t ii = 0; ii < NDIM; ++ii)
        matrices[ii] = identity;
      for (size_t ii = 0; ii < NDIM; ++ii)
        matrices[ii + NDIM] = h[key.translation()[ii + NDIM] % 2];
    }
    else
    {
  ...
  void init_lattice_summed()
  {
    if (!ops.empty())
    {
      for (int d = 0; d != NDIM; ++d)
      {
        for (const auto& op : ops)
        {
          MADNESS_ASSERT(op.lattice_summed() == ops[0].lattice_summed());
        }
...
public:
  ...
  SeparatedConvolution(World& world, const std::vector<std::shared_ptr<Convolution1D<Q>>>& argops,
                       long k = FunctionDefaults<NDIM>::get_k(), bool doleaves = false)
      : WorldObject<SeparatedConvolution<Q, NDIM>>(world), info(), doleaves(doleaves),
        lattice_summed_(false) // this will be overridden by init_lattice_summed below
        ,
        modified_(false), particle_(1), destructive_(false), k(k),
        cdata(FunctionCommonData<Q, NDIM>::get(k)), rank(argops.size()), vk(NDIM, k),
        v2k(NDIM, 2 * k), s0(std::max<std::size_t>(2, NDIM), Slice(0, k - 1))
  {
  ...
  SeparatedConvolution(World& world, const std::vector<ConvolutionND<Q, NDIM>>& argops,
                       long k = FunctionDefaults<NDIM>::get_k(), bool doleaves = false)
      : WorldObject<SeparatedConvolution<Q, NDIM>>(world), info(), doleaves(doleaves),
        lattice_summed_(false) // this will be overridden by init_lattice_summed below
        ,
        modified_(false), particle_(1), destructive_(false), ops(argops), k(k),
        cdata(FunctionCommonData<Q, NDIM>::get(k)), rank(argops.size()), vk(NDIM, k),
        v2k(NDIM, 2 * k), s0(std::max<std::size_t>(2, NDIM), Slice(0, k - 1))
  {
    init_range();
  ...
  SeparatedConvolution(
      World& world, const OperatorInfo info1,
      const array_of_bools<NDIM>& lattice_summed = FunctionDefaults<NDIM>::get_bc().is_periodic(),
      int k = FunctionDefaults<NDIM>::get_k(), bool doleaves = false)
      : SeparatedConvolution(world, Tensor<double>(0l), Tensor<double>(0l), info1.lo, info1.thresh,
                             lattice_summed, k, doleaves, info1.mu)
  {
  ...
  /// Constructor for Gaussian Convolutions (mostly for backward compatability)
  SeparatedConvolution(
      World& world, const Tensor<Q>& coeff, const Tensor<double>& expnt, double lo, double thresh,
      const array_of_bools<NDIM>& lattice_summed = FunctionDefaults<NDIM>::get_bc().is_periodic(),
      int k = FunctionDefaults<NDIM>::get_k(), bool doleaves = false, double mu = 0.0)
      : WorldObject<SeparatedConvolution<Q, NDIM>>(world), info(mu, lo, thresh, OT_UNDEFINED),
        doleaves(doleaves), lattice_summed_(lattice_summed), ops(coeff.dim(0)), k(k),
        cdata(FunctionCommonData<Q, NDIM>::get(k)), rank(coeff.dim(0)), vk(NDIM, k),
        v2k(NDIM, 2 * k), s0(std::max<std::size_t>(2, NDIM), Slice(0, k - 1))
  {
  ...
  SeparatedConvolution(
      World& world, Vector<double, NDIM> args, const Tensor<Q>& coeff, const Tensor<double>& expnt,
      const array_of_bools<NDIM>& lattice_summed = FunctionDefaults<NDIM>::get_bc().is_periodic(),
      int k = FunctionDefaults<NDIM>::get_k(), bool doleaves = false)
      : WorldObject<SeparatedConvolution<Q, NDIM>>(world), info(0.0, 0.0, 0.0, OT_UNDEFINED),
        doleaves(doleaves), lattice_summed_(lattice_summed), modified_(false), particle_(1),
        destructive_(false), ops(coeff.dim(0)), k(k), cdata(FunctionCommonData<Q, NDIM>::get(k)),
        rank(coeff.dim(0)), vk(NDIM, k), v2k(NDIM, 2 * k),
        s0(std::max<std::size_t>(2, NDIM), Slice(0, k - 1))
  {
    ...
    for (int mu = 0; mu < rank; ++mu)
    {
      double c = std::pow(sqrt(expnt(mu) / madness::constants::pi),
                          static_cast<int>(NDIM)); // Normalization coeff
      ops[mu].setfac(coeff(mu) / c);
      for (std::size_t d = 0; d < NDIM; ++d)
      {
        double c2 = sqrt(expnt[mu] * width[d] * width[d] / madness::constants::pi);
        std::shared_ptr<GaussianConvolution1D<double_complex>> gcptr(
            new GaussianConvolution1D<double_complex>(k, c2, expnt(mu) * width[d] * width[d], 0,
                                                      lattice_summed[d], args[d]));
  ...
  {
    return lattice_summed_;
  }
  /// @return flag for each axis indicating whether the domain is periodic in that direction (false
  /// by default)
  const array_of_bools<NDIM>& domain_is_periodic() const
  {
    return domain_is_periodic_;
  }
  ...
  typename std::enable_if<FDIM != NDIM, Key<NDIM>>::type get_source_key(const Key<FDIM> key) const
  {
    Key<NDIM> source;
    Key<FDIM - NDIM> dummykey;
    if (particle() == 1)
      key.break_apart(source, dummykey);
    if (particle() == 2)
      key.break_apart(dummykey, source);
    return source;
  }
  ...
  template <typename T, size_t FDIM>
  std::vector<Function<TENSOR_RESULT_TYPE(T, Q), FDIM>>
  operator()(const std::vector<Function<T, FDIM>>& f) const
  {
    return madness::apply(*this, f);
  }
  ...
  Function<TENSOR_RESULT_TYPE(T, Q), LDIM + LDIM> operator()(const Function<T, LDIM>& f1,
                                                             const Function<Q, LDIM>& f2) const
  {
  ...
  operator()(const std::vector<Function<T, LDIM>>& f1,
             const std::vector<Function<Q, LDIM>>& f2) const
  {
  ...
  template <typename T>
  Tensor<TENSOR_RESULT_TYPE(T, Q)> apply(const Key<NDIM>& source, const Key<NDIM>& shift,
                                         const Tensor<T>& coeff, double tol) const
  {
    // PROFILE_MEMBER_FUNC(SeparatedConvolution); // Too fine grain for routine profiling
    ...
    for (int mu = 0; mu < rank; ++mu)
    {
      ...
      if (muop.norm > tol)
      {
        ...
        Q fac = ops[mu].getfac();
        muopxv_fast(at, muop.ops, *input, f0, r, r0, tol / std::abs(fac), fac, work1, work2);
      }
    }
  ...
  /// the function
  /// @tparam T   the dimension of the function this operator is applied on. \todo MGR: Make sure
  /// info on T is correct. Was previously labeled FDIM.
  ...
  /// @return     coeff result
  template <typename T>
  GenTensor<TENSOR_RESULT_TYPE(T, Q)> apply2_lowdim(const Key<NDIM>& source, const Key<NDIM>& shift,
                                                    const GenTensor<T>& coeff, double tol,
                                                    double tol2) const
  {
    ...
    for (int r = 0; r < coeff.rank(); ++r)
    {
      ...
      for (int mu = 0; mu < rank; ++mu)
      {
        const SeparatedConvolutionInternal<Q, NDIM>& muop = op->muops[mu];
        Q fac = ops[mu].getfac();
        muopxv_fast(at, muop.ops, chunk, chunk0, result, result0, tol / std::abs(fac), fac, work1,
                    work2);
      }
  ...
  /// @param[in]	tol2	thresh/#neigh
  template <typename T>
  GenTensor<TENSOR_RESULT_TYPE(T, Q)> apply2(const Key<NDIM>& source, const Key<NDIM>& shift,
                                             const GenTensor<T>& coeff, double tol,
                                             double tol2) const
  {
    PROFILE_MEMBER_FUNC(SeparatedConvolution);
    ...
    for (int mu = 0; mu < rank; ++mu)
    {
      ...
      if (muop.norm > tol)
      {
        ...
        //  delta(coeff) * || T || < tol2
        const int r_max =
            SRConf<T>::max_sigma(tol2 / muop.norm, coeff.rank(), coeff.get_svdtensor().weights_);
        //                	print("r_max",coeff.config().weights(r_max));

        // note that max_sigma is inclusive!
        if (r_max >= 0)
        {
          const GenTensor<resultT> chunk =
              SVDTensor<resultT>(input->get_svdtensor().get_configs(0, r_max));
          const GenTensor<resultT> chunk0 =
              SVDTensor<resultT>(f0.get_svdtensor().get_configs(0, r_max));

          double cpu0 = cpu_time();

          Q fac = ops[mu].getfac();
          muopxv_fast2(source.level(), muop.ops, chunk, chunk0, r, r0, tol / std::abs(fac), fac,
                       work1, work2);
          double cpu1 = cpu_time();
          timer_low_transf.accumulate(cpu1 - cpu0);
  ...
  template <typename T>
  double estimate_costs(const Key<NDIM>& source, const Key<NDIM>& shift, const GenTensor<T>& coeff,
                        double tol, double tol2) const
  {

    if (coeff.is_full_tensor())
      return 0.5;
    if (2 * NDIM == coeff.ndim())
      return 1.5;
    ...
    for (int mu = 0; mu < rank; ++mu)
    {
      ...
      if (muop.norm > tol)
      {
        ...
        //                    low_cost+=nterms*low_operator_cost
        //                    + 2.0*nterms*nterms*low_reduction_cost;
        initial_rank += nterms;
  ...
  TensorTrain<double> make_tt_representation(const Key<NDIM>& source, const Key<NDIM>& shift,
                                             double tol, bool do_R, bool do_T) const
  {
    ...
    for (int mu = 0; mu < rank; ++mu)
    {
      double Rnorm = 1.0;
      for (std::size_t d = 0; d < NDIM; ++d)
        Rnorm *= op->muops[mu].ops[d]->Rnorm;
      if (Rnorm > 1.e-20)
        hi = mu;
      if ((Rnorm < 1.e-20) and (mu < hi))
        lo = mu;
    }
    hi++;
    lo++;
  ...
  static bool can_combine(const SeparatedConvolution<Q, NDIM>& left,
  ...
  static OperatorInfo combine_OT(const SeparatedConvolution<Q, NDIM>& left,
                                 const SeparatedConvolution<Q, NDIM>& right)
  {
  ...
  /// combine 2 convolution operators to one
  friend SeparatedConvolution<Q, NDIM>
  combine(const std::shared_ptr<SeparatedConvolution<Q, NDIM>> left,
...
static inline SeparatedConvolution<double_complex, 3> PeriodicHFExchangeOperator(
    World& world, Vector<double, 3> args, double lo, double eps,
    ...
    int k = FunctionDefaults<3>::get_k())
{
  ...
  return SeparatedConvolution<double_complex, 3>(world, args, coeff, expnt, lattice_sum, k, false);
}

/// Factory function generating separated kernel for convolution with 1/r in 3D.
static inline SeparatedConvolution<double, 3>
CoulombOperator(World& world, double lo, double eps,
                const array_of_bools<3>& lattice_sum = FunctionDefaults<3>::get_bc().is_periodic(),
                int k = FunctionDefaults<3>::get_k())
{
...
/// Factory function generating separated kernel for convolution with 1/r in 3D.
static inline SeparatedConvolution<double, 3>* CoulombOperatorPtr(
    World& world, double lo, double eps,
    const array_of_bools<3>& lattice_sum = FunctionDefaults<3>::get_bc().is_periodic(),
    int k = FunctionDefaults<3>::get_k())
{
  return new SeparatedConvolution<double, 3>(world, OperatorInfo(0.0, lo, eps, OT_G12), lattice_sum,
                                             k);
}

/// Factory function generating separated kernel for convolution with BSH kernel in general NDIM
template <std::size_t NDIM>
static inline SeparatedConvolution<double, NDIM> BSHOperator(
    World& world, double mu, double lo, double eps,
    ...
    int k = FunctionDefaults<NDIM>::get_k())
{
  ...
  {
    if (world.rank() == 0)
      print("the accuracy in BSHOperator is too small, tighten the threshold", eps);
    MADNESS_EXCEPTION("0", 1);
  }
  return SeparatedConvolution<double, NDIM>(world, OperatorInfo(mu, lo, eps, OT_BSH), lattice_sum,
                                            k);
}

/// Factory function generating separated kernel for convolution with BSH kernel in general NDIM
template <std::size_t NDIM>
static inline SeparatedConvolution<double, NDIM>* BSHOperatorPtr(
    World& world, double mu, double lo, double eps,
    const array_of_bools<NDIM>& lattice_sum = FunctionDefaults<NDIM>::get_bc().is_periodic(),
    int k = FunctionDefaults<NDIM>::get_k())
{
  ...
  return new SeparatedConvolution<double, NDIM>(world, OperatorInfo(mu, lo, eps, OT_BSH),
                                                lattice_sum, k);
}
...
/// Factory function generating separated kernel for convolution with exp(-mu*r)/(4*pi*r) in 3D
static inline SeparatedConvolution<double_complex, 3> PeriodicBSHOperator3D(
    World& world, Vector<double, 3> args, double mu, double lo, double eps,
    const array_of_bools<3>& lattice_sum = FunctionDefaults<3>::get_bc().is_periodic(),
    int k = FunctionDefaults<3>::get_k())

{
  ...
  return SeparatedConvolution<double_complex, 3>(world, args, coeff, expnt, lattice_sum, k);
}

/// Factory function generating separated kernel for convolution with exp(-mu*r)/(4*pi*r) in 3D
static inline SeparatedConvolution<double_complex, 3>* PeriodicBSHOperatorPtr3D(
    World& world, Vector<double, 3> args, double mu, double lo, double eps,
    const array_of_bools<3>& lattice_sum = FunctionDefaults<3>::get_bc().is_periodic(),
    int k = FunctionDefaults<3>::get_k())

{
  ...
  return new SeparatedConvolution<double_complex, 3>(world, args, coeff, expnt, lattice_sum, k);
}

static inline SeparatedConvolution<double, 3> SlaterF12Operator(
    World& world, double mu, double lo, double eps,
    const array_of_bools<3>& lattice_sum = FunctionDefaults<3>::get_bc().is_periodic(),
    int k = FunctionDefaults<3>::get_k())
{
...
static inline SeparatedConvolution<double, 3> SlaterF12sqOperator(
    World& world, double mu, double lo, double eps,
    const array_of_bools<3>& lattice_sum = FunctionDefaults<3>::get_bc().is_periodic(),
    int k = FunctionDefaults<3>::get_k())
{
...
static inline SeparatedConvolution<double, 3>* SlaterF12sqOperatorPtr(
    World& world, double mu, double lo, double eps,
    const array_of_bools<3>& lattice_sum = FunctionDefaults<3>::get_bc().is_periodic(),
    int k = FunctionDefaults<3>::get_k())
{
  return new SeparatedConvolution<double, 3>(world, OperatorInfo(mu, lo, eps, OT_F212), lattice_sum,
                                             k);
}

/// Factory function generating separated kernel for convolution with exp(-mu*r) in 3D
template <std::size_t NDIM = 3>
static inline SeparatedConvolution<double, NDIM> SlaterOperator(
    World& world, double mu, double lo, double eps,
    const array_of_bools<NDIM>& lattice_sum = FunctionDefaults<NDIM>::get_bc().is_periodic(),
    int k = FunctionDefaults<NDIM>::get_k())
{
  return SeparatedConvolution<double, NDIM>(world, OperatorInfo(mu, lo, eps, OT_SLATER),
                                            lattice_sum, k);
}
...
template <std::size_t NDIM>
static inline SeparatedConvolution<double, NDIM> GaussOperator(
    World& world, double mu, double lo = 0.0, double eps = 0.0,
    const array_of_bools<NDIM>& lattice_sum = FunctionDefaults<NDIM>::get_bc().is_periodic(),
    int k = FunctionDefaults<NDIM>::get_k())
{
  return SeparatedConvolution<double, NDIM>(world, OperatorInfo(mu, lo, eps, OT_GAUSS), lattice_sum,
                                            k);
}
...
/// lo and eps are not used here
template <std::size_t NDIM>
static inline SeparatedConvolution<double, NDIM>* GaussOperatorPtr(
    World& world, double mu, double lo = 0.0, double eps = 0.0,
    const array_of_bools<NDIM>& lattice_sum = FunctionDefaults<NDIM>::get_bc().is_periodic(),
    int k = FunctionDefaults<NDIM>::get_k())
{
  return new SeparatedConvolution<double, NDIM>(world, OperatorInfo(mu, lo, eps, OT_GAUSS),
                                                lattice_sum, k);
}

/// Factory function generating separated kernel for convolution with exp(-mu*r) in 3D
/// Note that the 1/(2mu) factor of SlaterF12Operator is not included, this is just the exponential
/// function
template <std::size_t NDIM>
static inline SeparatedConvolution<double, NDIM>* SlaterOperatorPtr_ND(
    World& world, double mu, double lo, double eps,
    const array_of_bools<NDIM>& lattice_sum = FunctionDefaults<NDIM>::get_bc().is_periodic(),
    int k = FunctionDefaults<NDIM>::get_k())
{
  return new SeparatedConvolution<double, NDIM>(world, OperatorInfo(mu, lo, eps, OT_SLATER),
                                                lattice_sum, k);
}
...
/// Note that the 1/(2mu) factor of SlaterF12Operator is not included, this is just the exponential
/// function
static inline SeparatedConvolution<double, 3>* SlaterOperatorPtr(
    World& world, double mu, double lo, double eps,
    const array_of_bools<3>& lattice_sum = FunctionDefaults<3>::get_bc().is_periodic(),
    int k = FunctionDefaults<3>::get_k())
{
  return new SeparatedConvolution<double, 3>(world, OperatorInfo(mu, lo, eps, OT_SLATER),
                                             lattice_sum, k);
}
...
static inline SeparatedConvolution<double, 3>* SlaterF12OperatorPtr(
    World& world, double mu, double lo, double eps,
    const array_of_bools<3>& lattice_sum = FunctionDefaults<3>::get_bc().is_periodic(),
    int k = FunctionDefaults<3>::get_k())
{
  return new SeparatedConvolution<double, 3>(world, OperatorInfo(mu, lo, eps, OT_F12), lattice_sum,
                                             k);
}

/// Factory function generating separated kernel for convolution with 1/(2 mu)*(1 - exp(-mu*r))/r in
/// 3D
...
FGOperatorPtr(World& world, double mu, double lo, double eps,
              ...
              int k = FunctionDefaults<3>::get_k())
{
  return new SeparatedConvolution<double, 3>(world, OperatorInfo(mu, lo, eps, OT_FG12), lattice_sum,
                                             k);
}

/// Factory function generating separated kernel for convolution with (1/(2 mu)*(1 -
/// exp(-mu*r)))^2/r in 3D
...
F2GOperatorPtr(World& world, double mu, double lo, double eps,
               ...
               int k = FunctionDefaults<3>::get_k())
{
  return new SeparatedConvolution<double, 3>(world, OperatorInfo(mu, lo, eps, OT_F2G12),
                                             lattice_sum, k);
}
...
F2GOperator(World& world, double mu, double lo, double eps,
            ...
            int k = FunctionDefaults<3>::get_k())
{
  return SeparatedConvolution<double, 3>(world, OperatorInfo(mu, lo, eps, OT_F2G12), lattice_sum,
                                         k);
}

/// Factory function generating separated kernel for convolution a normalized
/// Gaussian (aka a widened delta function)
static inline SeparatedConvolution<double, 3> SmoothingOperator3D(
    World& world, double eps,
    const array_of_bools<3>& lattice_sum = FunctionDefaults<3>::get_bc().is_periodic(),
    int k = FunctionDefaults<3>::get_k())
{
...
/// Gaussian (aka a widened delta function)
template <std::size_t NDIM>
static inline SeparatedConvolution<double, NDIM> SmoothingOperator(
    World& world, double eps,
    const array_of_bools<NDIM>& lattice_sum = FunctionDefaults<NDIM>::get_bc().is_periodic(),
    int k = FunctionDefaults<NDIM>::get_k())
{
...
/// Factory function generating separated kernel for convolution with exp(-mu*r)/(4*pi*r) in 3D
static inline SeparatedConvolution<double, 3>* BSHOperatorPtr3D(
    World& world, double mu, double lo, double eps,
    ...
    int k = FunctionDefaults<3>::get_k())
{
  ...
  return new SeparatedConvolution<double, 3>(world, coeff, expnt, lo, eps, lattice_summed, k);
}
...
static inline std::vector<std::shared_ptr<SeparatedConvolution<double, 3>>> GradCoulombOperator(
    World& world, double lo, double eps,
    const array_of_bools<3>& lattice_sum = FunctionDefaults<3>::get_bc().is_periodic(),
    int k = FunctionDefaults<3>::get_k())
{
  ...
  for (int dir = 0; dir < 3; dir++)
  {
    ...
    for (int mu = 0; mu < rank; mu++)
    {
      // We cache the normalized operator so the factor is the value we must multiply by to recover
      // the coeff we want.
      ...
      for (int d = 0; d < 3; d++)
      {
        if (d != dir)
          ops[mu].setop(d, GaussianConvolution1DCache<double>::get(
                               k, expnt(mu) * width[d] * width[d], 0, lattice_sum[d]));
      }
      ops[mu].setop(dir, GaussianConvolution1DCache<double>::get(
                             k, expnt(mu) * width[dir] * width[dir], 1, lattice_sum[dir]));
    }
...
static inline std::vector<std::shared_ptr<SeparatedConvolution<double, 3>>>
GradBSHOperator(World& world, double mu, double lo, double eps,
                const array_of_bools<3>& lattice_sum = FunctionDefaults<3>::get_bc().is_periodic(),
                int k = FunctionDefaults<3>::get_k())
{
  typedef SeparatedConvolution<double, 3> real_convolution_3d;
