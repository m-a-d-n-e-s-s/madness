...
/// SeparatedConvolutionInternal keeps data for 1 term and all dimensions and 1
/// displacement Why is this here?? Why don't you just use ConvolutionND in
...
We try to apply transition matrices to the compressed form of function
coefficients. Most of the code is about caching these transition matrices. They
are cached (key of the map is the displacement) in the SimpleCache "data", which
is of type SeparatedConvolutionData, which keeps the matrices for all separated
terms and dimensions. These SeparatedConvolutionData are constructed using
ConvolutionND "ops", which is constructed at the construction of the
SeparatedConvolution.
                ...
                construction                                            storage
                                                                ...
                                                                SimpleCache<SeparatedConvolutionData>
                                                                    (all terms,
all dim) / (all disp) vector<ConvolutionND> (1 term, all dim) / (all terms)
...
ConvolutionND and SeparatedConvolutionInternal both point to the same data in
ConvolutionData1D. Why we need SeparatedConvolutionInternal in the first place I
have no idea. ConvolutionND has the global factor, and
SeparatedConvolutionInternal has a norm.
...
class SeparatedConvolution
    : public WorldObject<SeparatedConvolution<Q, NDIM> > {
 public:
  typedef Q opT;  ///< The apply function uses this to infer resultT=opT*inputT
  bool doleaves;  ///< If should be applied to leaf coefficients ... false by
                  ///< default
  bool isperiodicsum;  ///< If true the operator 1D kernels have been summed
                       ///< over lattice translations and may be non-zero at
                       ///< both ends of the unit cell
  bool modified_;  ///< use modified NS form
  int particle_;
  bool destructive_;  ///< destroy the argument or restore it (expensive for 6d
                      ///< functions)
 ...
 private:
  mutable std::vector<ConvolutionND<Q, NDIM> >
      ops;  ///< ConvolutionND keeps data for 1 term, all dimensions, 1
            ///< displacement
  ...
  // SeparatedConvolutionData keeps data for all terms and all dimensions and 1
  // displacement
 ...
 private:
  ...
  //        /// return the right block of the upsampled operator (modified NS
  //        only)
  //
  //        /// unlike the operator matrices on the natural level the upsampled
  //        operator
  //        /// matrices are not Toeplitz, so we need more information than just
  //        the displacement
  //        ///.@param[in]  source  the source key
  //        /// @param[in]  disp    the displacement
  //        /// @param[in]  upop    the unfiltered operator matrix from scale
  //        n-1
  //        /// @return     (k,k) patch of the upop(2k,2k) matrix
  //        static Tensor<Q> operator_patch(const Translation& source, const
  //        Translation& disp, const Tensor<Q>& upop) {
  //
  ...
  ///*
  //            if (sx==0 and tx==0) copy_2d_patch(rij.ptr(),             k,
  //            upop.ptr(), 2*k, k, k); if (sx==1 and tx==0)
  //            copy_2d_patch(rij.ptr() + k,         k, upop.ptr(), 2*k, k, k);
  //            if (sx==0 and tx==1) copy_2d_patch(rij.ptr() + 2*k*k,     k,
  //            upop.ptr(), 2*k, k, k); if (sx==1 and tx==1)
  //            copy_2d_patch(rij.ptr() + 2*k*k + k, k, upop.ptr(), 2*k, k, k);
  //*/
  ...
  void apply_transformation(long dimk,
    ...
    // PROFILE_MEMBER_FUNC(SeparatedConvolution); // Too fine grain for routine
    // profiling
    long size = 1;
  ...
  template <typename T, typename R>
  void apply_transformation2(Level n,
                             long dimk,
                             double tol,
    ...
    if (at.r_term and (Rnorm > 1.e-20)) {
      ...
      for (std::size_t d = 0; d < NDIM; ++d) {
        ...
        if (r >= break_even) {
          ...
          r = std::max(
              2L,
              r + (r & 1L));  // (needed for 6D == when GENTENSOR is on)
                              // NOLONGER NEED TO FORCE OPERATOR RANK TO BE EVEN
#endif
          if (r == 0) {
            rank_is_zero = true;
            break;
      ...
      //            apply_transformation2(n, twok, tol, trans2, f, work1, work2,
      //            mufac, result);
    ...
    if (at.t_term and (Tnorm > 0.0)) {
      ...
      for (std::size_t d = 0; d < NDIM; ++d) {
        ...
        if (r >= break_even) {
          ...
          r = std::max(
              2L,
              r + (r & 1L));  // (needed for 6D == GENTENSOR is USED) NOLONGER
                              // NEED TO FORCE OPERATOR RANK TO BE EVEN
#endif
          if (r == 0) {
            rank_is_zero = true;
            break;
      ...
      //                apply_transformation2(n, k, tol, trans2, f0, work1,
      //                work2, -mufac, result0); apply_transformation3(trans2,
      //                f0, -mufac, result0);
    }
  }
  ...
  void muopxv_fast2(Level n,
    ...
    if (Rnorm > 1.e-20) {
      ...
      if (modified()) twok = k;
      //				long break_even;
      //				if (NDIM==1) break_even =
      //long(0.5*twok); 				else if (NDIM==2) break_even = long(0.6*twok); 				else if
      //(NDIM==3) break_even=long(0.65*twok); 				else break_even=long(0.7*twok);
      for (std::size_t d = 0; d < NDIM; ++d) {
        // long r;
        // for (r=0; r<twok; ++r) {
        // 	if (ops_1d[d]->Rs[r] < tol) break;
        ...
        //						trans[d].r = twok;
        //						trans[d].U =
        //ops_1d[d]->R.ptr(); 						trans[d].VT = 0;
        //					}
        //					else {
        //						//r += std::max(2L,r&1L); //
        //NOLONGER NEED TO FORCE OPERATOR RANK TO BE EVEN 						trans[d].r = r;
        //						trans[d].U =
        //ops_1d[d]->RU.ptr(); 						trans[d].VT = ops_1d[d]->RVT.ptr();
        //					}
        trans2[d] = ops_1d[d]->R;
      }
      apply_transformation2(
          n, twok, tol, trans2, f, work1, work2, mufac, result);
    }
    ...
    if (n > 0 and (Tnorm > 1.e-20)) {
      ...
      for (std::size_t d = 0; d < NDIM; ++d) {
        ...
        //                    }
        //                    else {
        //                        //r += std::max(2L,r&1L); // NOLONGER NEED TO
        //                        FORCE OPERATOR RANK TO BE EVEN trans[d].r = r;
        //                        trans[d].U = ops_1d[d]->TU.ptr();
        //                        trans[d].VT = ops_1d[d]->TVT.ptr();
        //                    }
        trans2[d] = ops_1d[d]->T;
      ...
      apply_transformation2(
          n, k, tol, trans2, f0, work1, work2, -mufac, result0);
    }
  }

  /// Computes the Frobenius norm of one of the separated terms ... WITHOUT
  /// FACTOR INCLUDED compute for 1 term, all dim, 1 disp, essentially for
  /// SeparatedConvolutionInternal
  ...
  /// compute for 1 term, all dim, 1 disp, essentially for
  /// SeparatedConvolutionInternal
  ...
  /// Computes the operator norm of one of the separated terms of the modified
  /// NS form
  ///    ... WITHOUT FACTOR INCLUDED
  /// compute for 1 term, all dim, 1 disp, essentially for
  /// SeparatedConvolutionInternal
  double munorm2_modified(Level n, const ConvolutionData1D<Q>* ops_1d[]) const {
    ...
    // follows Eq. (21) ff of Beylkin 2008 (Beylkin Appl. Comput. Harmon. Anal.
    // 24, pp 354)

    // we have all combinations of difference, upsampled, F terms (d, u, f),
    // with the constraint that d is in each term exactly once. In the mixed
    // terms (udf) we just get all possible combinations, in the pure terms
    // (dff, duu) we have to multiply each term (dff, fdf, ffd) with (NDIM-1)!,
    ...
    //            if (NDIM==3) {
    //                Tensor<Q>
    //                R_full=outer(ops_1d[0]->R,outer(ops_1d[1]->R,ops_1d[2]->R));
    //                Tensor<Q>
    //                T_full=outer(ops_1d[0]->T,outer(ops_1d[1]->T,ops_1d[2]->T));
  ...
  /// get the transformation matrices for 1 term and all dimensions and one
  /// displacement

  /// use ConvolutionND, which uses ConvolutionData1D to collect the
  /// transformation matrices
  const SeparatedConvolutionInternal<Q, NDIM>
  getmuop(int mu, Level n, const Key<NDIM>& disp) const {
    // PROFILE_MEMBER_FUNC(SeparatedConvolution); // Too fine grain for routine
    // profiling
    ...
    //             // This rescaling empirically based upon BSH separated
    //             expansion
    //             // ... needs more testing.  OK also for TDSE.
    //             // All is good except for some 000 blocks which are up to
    //             sqrt(k^d) off. for (int d=0; d<NDIM; ++d)  {
    //                 if (disp[d] == 0) newnorm *= 0.5;
    //                 else if (std::abs(disp[d]) == 1) newnorm *= 0.8;
    //             }
    //            double oldnorm = munorm(n, op.ops);
    //             if (oldnorm > 1e-13 && (newnorm < 0.5*oldnorm || newnorm
    //             > 2.0*oldnorm) )
  ...
  /// use ConvolutionND, which uses ConvolutionData1D to collect the
  /// transformation matrices
  const SeparatedConvolutionInternal<Q, NDIM> getmuop_modified(
      int mu,
      Level n,
      const Key<NDIM>& disp,
    ...
    // in the modified NS form we need not only the displacement, but also the
    // source Translation for correctly constructing the operator, b/c the
    // operator is not Toeplitz
    ...
    for (std::size_t d = 0; d < NDIM; ++d) {
      ...
      Translation tx = source.translation()[d] +
                       disp.translation()[d];  // target translation
  ...
  const SeparatedConvolutionData<Q, NDIM>*
  getop(Level n, const Key<NDIM>& d, const Key<NDIM>& source) const {
  ...
  /// uses SeparatedConvolutionInternal (ConvolutionND, ConvolutionData1D) to
  /// construct the transformation matrices.
  /// @param[in]  d   displacement
  /// @return pointer to cached operator
  const SeparatedConvolutionData<Q, NDIM>* getop_ns(Level n,
                                                    const Key<NDIM>& d) const {
    // PROFILE_MEMBER_FUNC(SeparatedConvolution); // Too fine grain for routine
    // profiling
    ...
    for (int mu = 0; mu < rank; ++mu) {
      // op.muops is of type SeparatedConvolutionInternal (1 term, all dim, 1
      // disp) getmuop uses ConvolutionND
  ...
  /// remember that the operator in the modified NS form is not Toeplitz, so we
  /// need information about the displacement and the source key
  /// @param[in]  n       level (=scale) (actually redundant, since included in
  /// source)
  /// @param[in]  disp    displacement key
  /// @param[in]  source  source key
  /// @return pointer to cached operator
  const SeparatedConvolutionData<Q, NDIM>* getop_modified(
      Level n,
      const Key<NDIM>& disp,
      const Key<NDIM>& source) const {
    // PROFILE_MEMBER_FUNC(SeparatedConvolution); // Too fine grain for routine
    ...
    // in the modified NS form the upsampled part of the operator depends on the
  ...
  void check_cubic() {
    ...
    for (std::size_t d = 1; d < NDIM; ++d) {
      MADNESS_CHECK(fabs(cell_width(d) - cell_width(0L)) <
                    1e-14 * cell_width(0L));
    }
  }
  ...
  /// @param[in]  key     the key indicating the child -- only some dimensions
  /// will be "reproductive"
  ...
  GenTensor<T> partial_upsample(const Key<FDIM>& key,
                                const GenTensor<T>& coeff,
                                const int particle) const {
    if (coeff.rank() == 0) return GenTensor<T>();
    MADNESS_ASSERT(coeff.dim(0) == k);
    if (NDIM == coeff.ndim()) {
      MADNESS_ASSERT(particle ==
                     1);  // other particle, leave this particle unchanged
      return coeff;
    }
    ...
    // the twoscale coefficients: for upsampling use h0/h1; see Alpert Eq
    // (3.35a/b) handle the spectator dimensions with the identity matrix
  ...
  /// specialization of the unfilter method, will transform only the sum
  /// coefficients
  ...
  GenTensor<T> upsample(const Key<FDIM>& key, const GenTensor<T>& coeff) const {
    // the twoscale coefficients: for upsampling use h0/h1; see Alpert Eq
    // (3.35a/b) note there are no difference coefficients; if you want that use
    // unfilter
    const Tensor<T> h[2] = {cdata.h0, cdata.h1};
 ...
 public:
  ...
  SeparatedConvolution(
    ...
    for (int mu = 0; mu < rank; ++mu) {
      Q c = std::pow(sqrt(expnt(mu) / pi),
                     static_cast<int>(NDIM));  // Normalization coeff

      // We cache the normalized operator so the factor is the value we must
      // multiply by to recover the coeff we want.
      ops[mu].setfac(coeff(mu) / c);

      for (std::size_t d = 0; d < NDIM; ++d) {
        ops[mu].setop(
            d,
            GaussianConvolution1DCache<Q>::get(
                k, expnt(mu) * width[d] * width[d], 0, isperiodicsum));
      }
    }
  }

  /// WSTHORNTON Constructor for Gaussian Convolutions (mostly for backward
  /// compatability)
  SeparatedConvolution(
      World& world,
    ...
    for (int mu = 0; mu < rank; ++mu) {
      ...
      for (std::size_t d = 0; d < NDIM; ++d) {
        double c2 =
            sqrt(expnt[mu] * width[d] * width[d] / madness::constants::pi);
        std::shared_ptr<GaussianConvolution1D<double_complex> > gcptr(
            new GaussianConvolution1D<double_complex>(
                k,
                c2,
                expnt(mu) * width[d] * width[d],
                0,
                isperiodicsum,
                args[d]));
  ...
  double norm(Level n, const Key<NDIM>& d, const Key<NDIM>& source_key) const {
    // SeparatedConvolutionData keeps data for all terms and all dimensions and
    // 1 displacement
    //            return 1.0;
    return getop(n, d, source_key)->norm;
  }

  /// return that part of a hi-dim key that serves as the base for displacements
  /// of this operator

  /// if the function and the operator have the same dimension return key
  /// if the function has a higher dimension than the operator (e.g. in the
  /// exchange operator) return only that part of key that corresponds to the
  /// particle this operator works on
  /// @param[in]	key	hi-dim key
  /// @return		a lo-dim part of key; typically first or second half
  template <size_t FDIM>
  typename std::enable_if<FDIM != NDIM, Key<NDIM> >::type get_source_key(
      const Key<FDIM> key) const {
    Key<NDIM> source;
  ...
  /// the operator does not need to have the same dimension as the function,
  /// e,g, the Poisson kernel for the exchange operator acts only on 1 electron
  /// of a given (pair) function.
  /// @param[in]  f   a function of same or different dimension as this operator
  /// @return     the result function of the same dimensionality as the input
  /// function f
  template <typename T, size_t FDIM>
  Function<TENSOR_RESULT_TYPE(T, Q), FDIM> operator()(
      const Function<T, FDIM>& f) const {
  ...
  /// @return     the result function of dim NDIM=2*LDIM: g(1,2) = G(1,1',2,2')
  /// f(1',2')
  template <typename T, size_t LDIM>
  Function<TENSOR_RESULT_TYPE(T, Q), LDIM + LDIM> operator()(
  ...
  Tensor<TENSOR_RESULT_TYPE(T, Q)> apply(const Key<NDIM>& source,
    ...
    /// SeparatedConvolutionData keeps data for all terms and all dimensions and
    /// 1 displacement
    const SeparatedConvolutionData<Q, NDIM>* op =
        getop(source.level(), shift, source);
  ...
  /// apply this operator on only 1 particle of the coefficients in low rank
  /// form

  /// note the unfortunate mess with NDIM: here NDIM is the operator dimension,
  /// and FDIM is the function's dimension, whereas in the function we have
  /// OPDIM for the operator and NDIM for the function
  /// @tparam T   the dimension of the function this operator is applied on.
  /// \todo MGR: Make sure info on T is correct. Was previously labeled FDIM.
  /// @param[in]  coeff   source coeffs in SVD (=optimal!) form, in high
  /// dimensionality (FDIM)
  ...
  GenTensor<TENSOR_RESULT_TYPE(T, Q)> apply2_lowdim(const Key<NDIM>& source,
    ...
    for (int r = 0; r < coeff.rank(); ++r) {
      ...
      s[0] = Slice(r, r);
      const Tensor<T> chunk =
          coeff.config().ref_vector(particle() - 1)(s).reshape(
              2 * k, 2 * k, 2 * k);
      const Tensor<T> chunk0 =
          f0.config().ref_vector(particle() - 1)(s).reshape(k, k, k);
      //                const double weight=std::abs(coeff.config().weights(r));

      // accumulate all terms of the operator for a specific term of the
      // function
      ...
      // this loop will return on result and result0 the terms [(P+Q) G
      // (P+Q)]_1, and [P G P]_1, respectively
      for (int mu = 0; mu < rank; ++mu) {
        const SeparatedConvolutionInternal<Q, NDIM>& muop = op->muops[mu];
        Q fac = ops[mu].getfac();
        muopxv_fast(at,
      ...
      // reinsert the transformed terms into result, leaving the other particle
      // unchanged
  ...
  GenTensor<TENSOR_RESULT_TYPE(T, Q)> apply2(const Key<NDIM>& source,
    ...
    for (int mu = 0; mu < rank; ++mu) {
      ...
      if (muop.norm > tol) {
        ...
        //  delta(coeff) * || T || < tol2
        const int r_max = SRConf<T>::max_sigma(
            tol2 / muop.norm, coeff.rank(), coeff.config().weights_);
  ...
  double estimate_costs(const Key<NDIM>& source,
    ...
    for (int mu = 0; mu < rank; ++mu) {
      ...
      if (muop.norm > tol) {
        ...
        // take only the first overlap computation of rank reduction into
        // account
    ...
    //            print("nterms, full, low, full/low", full_cost,
    //            low_cost,shift.distsq(), ratio);
    return ratio;
  }
  ...
  /// @param[in]  do_T    compute the T term of the operator (k^d), including
  /// factor -1 Both do_R and do_T may be used simultaneously, then the final
  ...
  TensorTrain<double> make_tt_representation(const Key<NDIM>& source,
    ...
    std::vector<Tensor<double> > cores(
        NDIM, Tensor<double>(rank_eff, k2k, k2k, rank_eff));
...
static inline SeparatedConvolution<double_complex, 3>
PeriodicHFExchangeOperator(
  ...
  if (bc(0, 0) == BC_PERIODIC)
    hi *= 100;  // Extend range for periodic summation
  ...
  return SeparatedConvolution<double_complex, 3>(
      world, args, coeff, expnt, bc, k, false);
  //        return SeparatedConvolution<double_complex,3>(world, coeff, expnt,
  //        bc, k);
}

/// Factory function generating separated kernel for convolution with 1/r in 3D.
static inline SeparatedConvolution<double, 3> CoulombOperator(
    World& world,
    double lo,
    double eps,
    const BoundaryConditions<3>& bc = FunctionDefaults<3>::get_bc(),
...
/// Factory function generating separated kernel for convolution with BSH kernel
/// in general NDIM
...
static inline SeparatedConvolution<double, NDIM> BSHOperator(
  ...
  if (eps > 1.e-4) {
    if (world.rank() == 0)
      print("the accuracy in BSHOperator is too small, tighten the threshold",
            eps);
...
/// Factory function generating separated kernel for convolution with
/// exp(-mu*r)/(4*pi*r) in 3D
static inline SeparatedConvolution<double, 3> BSHOperator3D(
    World& world,
    double mu,
    double lo,
    double eps,
...

{
  ...
  return new SeparatedConvolution<double_complex, 3>(
      world, args, coeff, expnt, bc, k);
}

/// Factory function generating separated kernel for convolution with (1 -
/// exp(-mu*r))/(2 mu) in 3D
...
/// Factory function generating separated kernel for convolution with exp(-mu*r)
/// in 3D Note that the 1/(2mu) factor of SlaterF12Operator is not included,
/// this is just the exponential function
static inline SeparatedConvolution<double, 3> SlaterOperator(
    World& world,
    double mu,
    double lo,
    double eps,
...
static inline SeparatedConvolution<double, 3>* SlaterF12OperatorPtr(
  ...
  return new SeparatedConvolution<double, 3>(
      world, coeff, expnt, bc, k, false, mu);
}
    ...
    for (int mu = 0; mu < rank; mu++) {
      ...
      for (int d = 0; d < 3; d++) {
        if (d != dir)
          ops[mu].setop(
              d,
              GaussianConvolution1DCache<double>::get(
                  k, expnt(mu) * width[d] * width[d], 0, isperiodicsum));
      }
      ops[mu].setop(
          dir,
          GaussianConvolution1DCache<double>::get(
              k, expnt(mu) * width[dir] * width[dir], 1, isperiodicsum));
    }
...
struct ArchiveLoadImpl<Archive, const SeparatedConvolution<T, NDIM>*> {
  static inline void load(const Archive& ar,
                          const SeparatedConvolution<T, NDIM>*& ptr) {
...
struct ArchiveStoreImpl<Archive, const SeparatedConvolution<T, NDIM>*> {
  static inline void store(const Archive& ar,
                           const SeparatedConvolution<T, NDIM>* const& ptr) {
