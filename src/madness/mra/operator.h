...
namespace madness {
    ...
    */
    ...
    public:
        ...
        std::array<bool, NDIM> lattice_sum;  ///< If lattice_sum[d] is true summation over lattice translations along axis d
    ...
    private:
        ...
        make_coeff_for_operator(World& world, double mu, double lo, double eps, OpType type,
                                const std::array<bool, NDIM>& lattice_sum) {

            OperatorInfo info(mu,lo,eps,type);
            return make_coeff_for_operator(world, info, lattice_sum);
...
//            return std::make_pair(coeff,expnt);
        ...
        make_coeff_for_operator(World& world, OperatorInfo& info,
          ...
          const auto any_lattice_sum = std::accumulate(
              lattice_sum.begin(), lattice_sum.end(), false, std::logical_or{});
          if (any_lattice_sum) {
            hi *= 100;
          }
          ...
          if (info.truncate_lowexp_gaussians.value_or(any_lattice_sum)) {
...
//            op.norm=1.0;
    ...
    public:
        ...
        SeparatedConvolution(World& world,
                             ...
                             const std::array<bool, NDIM>& lattice_sum = FunctionDefaults<NDIM>::get_bc().is_periodic(),
                ...
                for (std::size_t d=0; d<NDIM; ++d) {
                  ...
                  std::shared_ptr<GaussianConvolution1D<double_complex> >
                      gcptr(new GaussianConvolution1D<double_complex>(k, c2, 
                            expnt(mu)*width[d]*width[d], 0, lattice_sum[d], args[d]));
        ...
        const std::vector< Key<NDIM> >& get_disp(Level n) const {
            return Displacements<NDIM>().get_disp(n, false);
        }

        const std::array<bool, NDIM>& includes_lattice_sum() const { return lattice_sum; }
...
//            print("nterms, full, low, full/low", full_cost, low_cost,shift.distsq(), ratio);
        ...
        static SeparatedConvolution<Q,NDIM> combine(const SeparatedConvolution<Q,NDIM>& left,
            ...
            MADNESS_CHECK(left.includes_lattice_sum() == right.includes_lattice_sum());

            auto info=combine_OT(left,right);
            return SeparatedConvolution<Q,NDIM>(left.get_world(),info,left.includes_lattice_sum(),left.k);
        }
    ...
    SeparatedConvolution<double_complex,3> PeriodicHFExchangeOperator(World& world,
                                                   ...
                                                   double lo,
                                                   double eps,
                                                   const std::array<bool, 3>& lattice_sum = FunctionDefaults<3>::get_bc().is_periodic(),
