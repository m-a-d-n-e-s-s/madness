...
public:
  ...
  [[nodiscard]] virtual nlohmann::json read_results(std::string filename) const
  {
    if (has_results(filename))
    {
      ...
      // {
      //   std::string msg =
      //       "Results file " + filename + " does not match the parameters of the calculation";
      //   print(msg);
...
public:
  ...
  {
    return lib_.calc(world_, params_);
  }
...
NextAction valid(World& world, const SCFResultsTuple& results, const SCFParams& params)
{
  ...
  const bool must_redo =
      !at_protocol || (archive_needed && !archive_exists) || !all_properties_computed;
...
private:
  void initialize_(World& world, const Params& params)
  {
    nemo_ = std::make_shared<Nemo>(world, params.get<CalculationParameters>(),
                                   params.get<Nemo::NemoCalculationParameters>(),
                                   params.get<Molecule>());
  }
