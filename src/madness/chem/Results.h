...
public:
  ...
  std::optional<Tensor<double>> normalmodes_atomic; //(normal modes in atomic
                                                  // coordinates)
                                             //
  ...
  nlohmann::json to_json() const override {
    ...
    set_if_exists(j, "normalmodes_atomic", normalmodes_atomic, tensor_out<double>);
  ...
  void from_json(const nlohmann::json &j) override {
    ...
    get_if_exists(j, "normalmodes_atomic", normalmodes_atomic, tensor_in<double>);
  }
};
...
public:
  ...
  struct RamanModeRow {
    int mode;        // 1..M
    double freq_cm1; // vibrational frequency (cm^-1)
    // Provide either all five values, or just alpha2/beta2 and let the
    // printer compute the rest.
    double alpha2; // Alpha**2  (a'²)
    double beta2;  // Beta(a)**2 (γ'²-like invariant in your convention)
    std::optional<double> pol_int;   // Pol.Int.   (45*α'² + 4*β'²), optional
    std::optional<double> depol_int; // Depol.Int. (3*β'²), optional
    std::optional<double>
        dep_ratio; // Dep. Ratio (3β'²) / (45α'² + 4β'²), optional
  };

  void to_json(const RamanModeRow &row, nlohmann::json &j) const {
    j["mode"] = row.mode;
    j["freq_cm1"] = row.freq_cm1;
    j["alpha2"] = row.alpha2;
    j["beta2"] = row.beta2;
    if (row.pol_int)
      j["pol_int"] = *(row.pol_int);
    if (row.depol_int)
      j["depol_int"] = *(row.depol_int);
    if (row.dep_ratio)
      j["dep_ratio"] = *(row.dep_ratio);
  }
  void from_json(const nlohmann::json &j, RamanModeRow &row) const {
    row.mode = j.at("mode").get<int>();
    row.freq_cm1 = j.at("freq_cm1").get<double>();
    row.alpha2 = j.at("alpha2").get<double>();
    row.beta2 = j.at("beta2").get<double>();
    if (j.contains("pol_int"))
      row.pol_int = j.at("pol_int").get<double>();
    if (j.contains("depol_int"))
      row.depol_int = j.at("depol_int").get<double>();
    if (j.contains("dep_ratio"))
      row.dep_ratio = j.at("dep_ratio").get<double>();
  }
  // Raman spectra result at frequency
  // beta2
  std::map<double, std::vector<RamanModeRow>> raman_spectra;

  // map frequency to each modes pol_int, depol_int, depol_ratio, and 'alpha2',
  // 'beta2',mode_frequency
  ...
  void from_json(const nlohmann::json &j) override {
    ...
    if (j.contains("raman_spectra")) {
      raman_spectra.clear();
      for (const auto &item : j["raman_spectra"].items()) {
        double freq = std::stod(item.key());
        std::vector<RamanModeRow> spectrum;
        for (const auto &mode_json : item.value()) {
          RamanModeRow row;
          from_json(mode_json, row);
          spectrum.push_back(row);
        }
        raman_spectra[freq] = spectrum;
      }
    }
  }
  ...
  nlohmann::json to_json() const override {
    ...
    j["raman_spectra"] = nlohmann::json::object();
    for (const auto &item : raman_spectra) {
      nlohmann::json spectrum_json = nlohmann::json::array();
      for (const auto &mode_data : item.second) {
        nlohmann::json mode_json;
        to_json(mode_data, mode_json);
        spectrum_json.push_back(mode_json);
      }
      j["raman_spectra"][std::to_string(item.first)] = spectrum_json;
    }
...
using SCFResultsTuple = std::tuple<SCFResults, PropertyResults,
                                   ConvergenceResults, OptimizationResults>;
