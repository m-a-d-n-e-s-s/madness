...
struct CalculationParameters : public QCCalculationParametersBase {
  static constexpr char const* tag = "dft";
	...
	CalculationParameters() {
		...
		initialize<std::string>("bsh_apply","auto","BSH operator apply backend in compute_residual: auto=pick tile/macrotask from topology+memory (default; BSHApplyStrategy.h). auto is a CONSERVATIVE GUIDE: it routes multinode->macrotask and single-node->tile, with an opt-in single-node OOM gate (bsh_apply_memory_budget) whose memory estimate is calibrated for light atoms at default eprec -- heavy atoms / tight eprec / the HF exchange can exceed it, so for those set a conservative budget or force a backend. tile=rank-aware memory-bounded loop; macrotask=distribute across subworlds (memory-frugal, multinode); plain=single un-tiled apply",{"auto","tile","macrotask","plain"});
