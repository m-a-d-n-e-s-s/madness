...
inline long size_to_long(const std::size_t value) {
  MADNESS_ASSERT(value <=
                 static_cast<std::size_t>(std::numeric_limits<long>::max()));
  return static_cast<long>(value);
}

inline int size_to_int(const std::size_t value) {
  MADNESS_ASSERT(value <=
...
struct dens_inv {
  ...
  void operator()(const Key<3> &key, Tensor<double> &U, const Tensor<double> &t,
    ...
    ITERATOR(U, double d = t(IND); double p = inv(IND); U(IND) = d / p;);
  }
...
public:
  ...
  double operator()(const coord_3d &xyz) const {
    return -molecule.atomic_attraction_potential(size_to_int(iatom), xyz[0], xyz[1], xyz[2]);
  }
...
Tensor<double> NemoBase::compute_gradient(const real_function_3d &rhonemo,
  ...
  Tensor<double> grad(size_to_long(3 * molecule.natom()));
  ...
  for (size_t iatom = 0; iatom < molecule.natom(); ++iatom) {
    ...
    for (int iaxis = 0; iaxis < 3; iaxis++) {
      const long grad_index = size_to_long(3 * iatom) + iaxis;
      grad(grad_index) = -inner(bra[iaxis], aa);
    }
  }
  ...
  for (size_t atom = 0; atom < molecule.natom(); ++atom) {
    for (int axis = 0; axis < 3; ++axis) {
      const long grad_index = size_to_long(atom * 3) + axis;
      grad[grad_index] +=
...
Nemo::Nemo(World &world, const commandlineparser &parser)
  ...
  symmetry_projector = projector_irrep(get_calc_param().pointgroup())
                           .set_ordering("keep")
                           .set_verbosity(0)
                           .set_orthonormalize_irreps(true);
  ;
  if (symmetry_projector.get_verbosity() > 1)
    symmetry_projector.print_character_table();
};
...
vecfuncT Nemo::localize(const vecfuncT &nemo, const double dconv,
  ...
  Tensor<double> UT =
      localizer.compute_localization_matrix(world, mo, randomize);
...
std::shared_ptr<Fock<double, 3>> Nemo::make_fock_operator() const {
  ...
  if (calc->xc.is_dft()) {
    ...
    if (do_ac()) {
      ...
      double charge =
          double(molecule().total_nuclear_charge()) - get_calc_param().charge();
      real_function_3d scaledJ = -1.0 / charge * J.potential() *
                                 (1.0 - calc->xc.hf_exchange_coefficient());
    ...
    if (do_ac())
      xcpot.set_info("xc potential with ac");
    fock->add_operator(
        "Vxc", std::make_shared<LocalPotentialOperator<double, 3>>(xcpot));
  }
  ...
  if (do_pcm()) {
    ...
    fock->add_operator(
        "Vpcm", std::make_shared<LocalPotentialOperator<double, 3>>(pcmpot));
  }
  return fock;
}

/// compute the Fock matrix from scratch
tensorT Nemo::compute_fock_matrix(const vecfuncT &nemo,
                                  const tensorT &occ) const {
...
double Nemo::solve(const SCFProtocol &proto) {
  ...
  std::vector<double> energies(
      1, 0.0); // contains the total energy and all its contributions
  double energy = 0.0;
  ...
  for (int iter = 0; iter < get_calc_param().maxiter(); ++iter) {
    ...
    if (localized)
      nemo = localize(nemo, proto.dconv, iter == 0);
    ...
    if (not localized) {
      ...
      for (int i = 0; i < fock.dim(0); ++i)
        fock_offdiag(i, i) = 0.0;
      double max_fock_offidag = fock_offdiag.absmax();
      if (world.rank() == 0 and get_calc_param().print_level() > 3)
      ...
      tensorT U = calc->get_fock_transformation(
          world, overlap, fock, calc->aeps, calc->aocc,
      ...
      for (int i = 0; i < calc->aeps.size(); ++i)
...
Nemo::compute_energy_regularized(const vecfuncT &nemo, const vecfuncT &Jnemo,
  ...
  return std::vector<double>{energy, ke0, ke + pe,    J,
                             exc,    -K,  pcm_energy, nucrep};
}
...
void Nemo::compute_nemo_potentials(const vecfuncT &nemo, vecfuncT &Jnemo,
...

  {
    ...
    Coulomb<double, 3> J = Coulomb<double, 3>(world, this).set_taskq(taskq);
    {
      ...
      // taskq->set_printlevel(get_calc_param().print_level());
      if (get_calc_param().print_level() > 9)
        taskq->print_taskq();
      taskq->run_all();
    ...
    if (calc->xc.is_dft()) {
      ...
      if (do_ac()) {
        std::cout << "Computing asymtotic correction!\n";
        double charge = double(molecule().total_nuclear_charge()) -
                        get_calc_param().charge();
        real_function_3d scaledJ = -1.0 / charge * J.potential() *
                                   (1.0 - calc->xc.hf_exchange_coefficient());
        xc_pot = ac.apply(xc_pot, scaledJ);
      }
      ...
      // the meta-gga term is a differential operator, not a multiplicative
      // potential, so it cannot ride along in xc_pot. apply_tau_term returns it
      // already divided by R, so it adds straight onto the other nemo-side
      // potentials.
      if (xcoperator.has_tau_term())
...
real_function_3d Nemo::make_density(const tensorT &occ, const vecfuncT &bra,
                                    const vecfuncT &ket,
...
real_function_3d Nemo::make_ddensity(const real_function_3d &rhonemo,
  ...
  real_function_3d RXR =
      real_factory_3d(world).functor(U1_func).truncate_on_project();
...
real_function_3d Nemo::kinetic_energy_potential(const vecfuncT &nemo) const {
  ...
  for (std::size_t i = 0; i < eps.size(); ++i)
    eps[i] = calc->aeps(size_to_long(i));
  ...
  real_function_3d uvnuc =
      0.5 * (calc->potentialmanager->vnuclear() - ncf->U2());
  nu_bar = nu_bar - uvnuc;
...
real_function_3d Nemo::make_sigma(const real_function_3d &rho1,
  ...
  real_function_3d term2 =
      -2.0 * (rho1 * dot(world, uvec, drho2) + rho2 * dot(world, uvec, drho1));
...
VibrationalResults Nemo::hessian(const Tensor<double> &x) {
  ...
  const long hessian_dim = size_to_long(3 * natom);
  Tensor<double> hessian(hessian_dim, hessian_dim);
  ...
  const real_function_3d rhonemo =
      2.0 * make_density(get_calc()->get_aocc(), nemo);
  ...
  for (size_t iatom = 0; iatom < natom; ++iatom) {
    for (int iaxis = 0; iaxis < 3; ++iaxis) {
      const int i = size_to_int(iatom * 3) + iaxis;

      real_function_3d FXF = 4.0 * make_density(calc->get_aocc(), nemo, xi[i]);
      NuclearCorrelationFactor::RX_functor rxr_func(ncf.get(), size_to_int(iatom), iaxis,
                                                    -1);
      const real_function_3d RXR =
          real_factory_3d(world).functor(rxr_func).truncate_on_project();
    ...
    for (int iaxis = 0; iaxis < 3; ++iaxis) {
      ...
      for (size_t jatom = 0; jatom < natom; ++jatom) {
        for (int jaxis = 0; jaxis < 3; ++jaxis) {
          const int j = size_to_int(jatom * 3) + jaxis;
          ...
          MolecularDerivativeFunctor mdf(molecule(), size_to_int(jatom), jaxis);
          hessian(i, j) = inner(dens_pt[i], mdf);

          // integration by parts < 0 | H^{YX} | 0 >
          if (iatom == jatom)
            hessian(i, j) += inner(drho[iaxis], mdf);
        }
      }
    }
  }
  ...
  if (world.rank() == 0)
    print("max asymmetric element in the Hessian matrix: ", max_asymmetric);
  ...
  for (long i = 0; i < hessian_dim; ++i) {
    double sum = 0.0;
    for (long j = 0; j < hessian_dim; j += 3)
      sum += hessian(i, j + (i % 3));
    hessian(i, i) = -sum;
  }
  ...
  Tensor<double> frequencies =
      compute_frequencies(molecule(), hessian, normalmodes, false, hessdebug);
  ...
  frequencies =
      compute_frequencies(molecule(), hessian, normalmodes, true, hessdebug);
...
Tensor<double> Nemo::make_incomplete_hessian() const {
  ...
  for (size_t iatom = 0; iatom < natom; ++iatom) {
    for (int iaxis = 0; iaxis < 3; ++iaxis) {
      const int i = size_to_int(iatom * 3) + iaxis;
      NuclearCorrelationFactor::RX_functor rxr_func(ncf.get(), size_to_int(iatom), iaxis, 2);
      const real_function_3d RXR =
          real_factory_3d(world).functor(rxr_func); //.truncate_on_project();
    ...
    for (int iaxis = 0; iaxis < 3; ++iaxis) {
      ...
      for (size_t jatom = 0; jatom < natom; ++jatom) {
        for (int jaxis = 0; jaxis < 3; ++jaxis) {
          ...
          MolecularSecondDerivativeFunctor m2df(molecule(), size_to_int(jatom), jaxis,
                                                iaxis);
          if (iatom == jatom)
            result += inner(rho, m2df);
...
Tensor<double> Nemo::make_incomplete_hessian_response_part(
  ...
  const long hessian_dim = size_to_long(3 * natom);
  Tensor<double> complementary_hessian(hessian_dim, hessian_dim);
  for (size_t i = 0, iatom = 0; iatom < natom; ++iatom) {
    for (int iaxis = 0; iaxis < 3; ++iaxis, ++i) {
      ...
      Tensor<double> h(size_to_long(3 * molecule().natom()));

      for (size_t jatom = 0, j = 0; jatom < molecule().natom(); ++jatom) {
        for (int jaxis = 0; jaxis < 3; ++jaxis, ++j) {
          if ((iatom == jatom) and (iaxis == jaxis))
            continue;
          MolecularDerivativeFunctor mdf(molecule(), size_to_int(jatom), jaxis);
          h(size_to_long(j)) = inner(dens_pt, mdf);
        }
      }
      complementary_hessian(size_to_long(i), _) = h;
    }
  }
...
vecfuncT Nemo::make_cphf_constant_term(const size_t iatom, const int iaxis,
  ...
  Jconst.potential() =
      Jconst.compute_potential(2.0 * RXR * rhonemo); // factor 2 for cphf
...
vecfuncT Nemo::solve_cphf(const size_t iatom, const int iaxis,
  ...
  // guess for the perturbed MOs
  const vecfuncT nemo = calc->amo;
  const int nmo = size_to_int(nemo.size());
  const Tensor<double> occ = get_calc()->get_aocc();
  const real_function_3d rhonemo =
      2.0 * make_density(occ, nemo); // closed shell
  const real_function_3d arho = 0.5 * R_square * rhonemo;
  NuclearCorrelationFactor::RX_functor rxr_func(ncf.get(), size_to_int(iatom), iaxis, 2);
  const real_function_3d RXR =
      real_factory_3d(world).functor(rxr_func).truncate_on_project();

  int ii = size_to_int(3 * iatom) + iaxis;
  Tensor<double> ihr = incomplete_hessian(ii, _);
  const long molecule_dim = size_to_long(3 * molecule().natom());
  Tensor<double> old_h(molecule_dim);
  ...
  typedef XNonlinearSolver<std::vector<Function<double, 3>>, double, allocT>
      solverT;
  allocT alloc(world, size_to_int(nemo.size()));
  solverT solver(allocT(world, size_to_int(nemo.size())));
  solver.set_maxsub(5);
  ...
  for (int iter = 0; iter < 10; ++iter) {
    ...
    real_function_3d density_pert =
        4.0 * make_density(occ, R2nemo, xi_complete);
    Jp.potential() = Jp.compute_potential(density_pert);
    vecfuncT Kp;
    if (is_dft()) {
      // reconstruct the full perturbed density: do not truncate!
      const real_function_3d full_dens_pt =
          (density_pert + 2.0 * RXR * rhonemo);
    ...
    if (world.rank() == 0) {
      ...
      print("xi_" + stringify(ii), "CPHF BSH residual: rms", rms, "   max",
            maxval, "H, Delta H", h.normf(), h_diff.absmax());
      print("h+ihr");
      print(h + ihr);
    }
    old_h = h;
...
std::vector<vecfuncT> Nemo::compute_all_cphf() {
  ...
  const int natom = size_to_int(molecule().natom());
  ...
  // compute those contributions to the hessian that do not depend on the
  // response
      ...
      for (int iaxis = 0; iaxis < 3; ++iaxis, ++i) {
        if (xi[i].size() > 0) {
          for (real_function_3d &xij : xi[i])
            xij.set_thresh(preiterations.current_prec);
        }
        xi[i] =
            solve_cphf(iatom, iaxis, fock, xi[i], rhsconst[i],
...
vecfuncT Nemo::compute_cphf_parallel_term(const size_t iatom,
  ...
  Tensor<double> FRXRF =
      matrix_inner(world, nemo, RXRnemo); // skipping factor 0.5
...
Tensor<double> Nemo::compute_IR_intensities(const Tensor<double> &normalmodes,
  ...
  Tensor<double> D = MolecularOptimizer::projector_external_dof(
      molecule(), {"tx", "ty", "tz", "rx", "ry", "rz"});
  Tensor<double> DL = inner(D, normalmodes);
  Tensor<double> nm = inner(M, DL);

  // square of the dipole derivative wrt the normal modes Q
  const long dens_pt_size = size_to_long(dens_pt.size());
  Tensor<double> mu_Qxyz2(dens_pt_size);

  // compute the derivative of the dipole moment wrt nuclear displacements X
  for (std::size_t component = 0; component < 3; ++component) {
    const int component_i = size_to_int(component);
    // electronic and nuclear dipole derivative wrt nucl. displacements X
    Tensor<double> mu_X(dens_pt_size), mu_X_nuc(dens_pt_size);

    for (size_t iatom = 0; iatom < molecule().natom(); ++iatom) {
      for (int iaxis = 0; iaxis < 3; ++iaxis) {
        const int i = size_to_int(iatom * 3) + iaxis;
        mu_X(i) = -inner(dens_pt[size_to_int(i)], DipoleFunctor(component_i));
        Tensor<double> dnucdipole =
            molecule().nuclear_dipole_derivative(size_to_int(iatom), iaxis);
        mu_X_nuc(i) = dnucdipole(size_to_long(component));
      }
    }
  ...
  const double dmuq_au_to_SI =
      dmu_au_to_SI *
      sqrt(constants::atomic_mass_in_au / constants::atomic_mass_constant);
  ...
  const double conversion = pi * N * dmuq_au_to_SI * dmuq_au_to_SI /
                            (3.0 * 4.0 * pi * e * c * c) / 1.e3;
