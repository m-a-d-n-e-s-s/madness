...
struct dens_inv {
  ...
  void operator()(const Key<3> &key, Tensor<double> &U, const Tensor<double> &t, const Tensor<double> &inv) const {
    //        real_tensor U,
...
public:
  atomic_attraction(const Molecule &mol, const size_t iatom1) : molecule(mol), iatom(iatom1) {}
...
Tensor<double> NemoBase::compute_gradient(const real_function_3d &rhonemo, const Molecule &molecule) const {
  ...
  for (size_t atom = 0; atom < molecule.natom(); ++atom) {
    for (int axis = 0; axis < 3; ++axis) {
      grad[atom * 3 + axis] += molecule.nuclear_repulsion_derivative(atom, axis);
    }
  }
  return grad;
}
...
Nemo::Nemo(World &world, const commandlineparser &parser)
    : NemoBase(world), calc(std::make_shared<SCF>(world, parser)), nemo_param(world, parser), coords_sum(-1.0),
      ac(world, calc) {
...
Nemo::Nemo(World &world, const CalculationParameters &param, const NemoCalculationParameters &nemo_param,
           const Molecule &molecule)
    : NemoBase(world), calc(std::make_shared<SCF>(world, param, molecule)), nemo_param(nemo_param), coords_sum(-1.0),
      ac(world, calc) {
  ...
  if (skip_solve) {
    if (world.rank() == 0) {
      ...
      if (calc->converged_for_thresh * 0.9999 < p.end_prec)
        print(" -> orbitals are converged to the required threshold of", p.end_prec);
    }
...
double Nemo::solve(const SCFProtocol &proto) {
  ...
  for (int iter = 0; iter < get_calc_param().maxiter(); ++iter) {
    ...
    if (not localized) {
      ...
      tensorT U = calc->get_fock_transformation(world, overlap, fock, calc->aeps, calc->aocc,
    ...
    converged =
        check_convergence(energies, oldenergies, bsh_norm, deltadens, get_calc_param(), proto.econv, proto.dconv);
...
std::vector<double> Nemo::compute_energy_regularized(const vecfuncT &nemo, const vecfuncT &Jnemo, const vecfuncT &Knemo,
...
void Nemo::compute_nemo_potentials(const vecfuncT &nemo, vecfuncT &Jnemo, vecfuncT &Knemo, vecfuncT &xcnemo,
...
real_function_3d Nemo::make_density(const Tensor<double> &occ, const vecfuncT &nemo) const {
  return calc->make_density(world, occ, nemo);
}

real_function_3d Nemo::make_density(const tensorT &occ, const vecfuncT &bra, const vecfuncT &ket,
...
real_function_3d Nemo::make_ddensity(const real_function_3d &rhonemo, const int axis) const {
...
/// \f]
///
real_function_3d Nemo::make_sigma(const real_function_3d &rho1, const real_function_3d &rho2) const {
  ...
  if (world.rank() == 0) {
    ...
    for (size_t i = 0; i < calc->molecule.natom(); ++i) {
      ...
      printf(" %3s %3d %12.6f %12.6f %12.6f %12.6f %12.6f %12.6f\n",
             atomic_number_to_symbol(atom.atomic_number).c_str(), int(i), atom.x, atom.y, atom.z, grad[i * 3 + 0],
             grad[i * 3 + 1], grad[i * 3 + 2]);
    }
  }
  return grad;
}
...
Tensor<double> Nemo::make_incomplete_hessian_response_part(const std::vector<vecfuncT> &xi) const {
...
vecfuncT Nemo::make_cphf_constant_term(const size_t iatom, const int iaxis, const vecfuncT &R2nemo,
...
vecfuncT Nemo::solve_cphf(const size_t iatom, const int iaxis, const Tensor<double> fock, const vecfuncT &guess,
  ...
  for (; not preiterations.finished(); ++preiterations) {
    ...
    for (int i = 0, iatom = 0; iatom < natom; ++iatom) {
      for (int iaxis = 0; iaxis < 3; ++iaxis, ++i) {
        ...
        xi[i] =
            solve_cphf(iatom, iaxis, fock, xi[i], rhsconst[i], incomplete_hessian, parallel[i], preiterations, "LDA");
        save_function(xi[i], "xi_guess" + stringify(i));
      }
    }
    if (world.rank() == 0) {
  ...
  for (; not p.finished(); ++p) {
    ...
    for (int i = 0, iatom = 0; iatom < natom; ++iatom) {
      for (int iaxis = 0; iaxis < 3; ++iaxis, ++i) {
        ...
        xi[i] = solve_cphf(iatom, iaxis, fock, xi[i], rhsconst[i], incomplete_hessian, parallel[i], p,
                           get_calc_param().xc());
        save_function(xi[i], "xi_guess" + stringify(i));
      }
    }
...
Tensor<double> Nemo::compute_IR_intensities(const Tensor<double> &normalmodes, const vecfuncT &dens_pt) const {
  ...
  const double dmuq_au_to_SI = dmu_au_to_SI * sqrt(constants::atomic_mass_in_au / constants::atomic_mass_constant);
