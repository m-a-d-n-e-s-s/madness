...
distmatT SCF::kinetic_energy_matrix(World& world, const vecfuncT& v) const {
    ...
    if (tile_KEmat) {
        ...
        for (int axis = 0; axis < 3; ++axis) {
            START_TIMER(world);
            vecfuncT dv(n);
            // Separate differentiate (apply) from compress within the same phase, timing each.
            ...
            // A/B wall is honest. Cleared after the apply to free the transient surface (~200-900 MB/axis).
            double t_halo = 0.0;
            if (kemat_halo) {
                double h0 = wall_time();
        ...
        // No-tiling reference: all 3 axes applied before a single fence (peak mem ~4n).
        ...
        long user_min, long user_max)
{
    ...
    resolved_max = std::max(resolved_max, resolved_min);
    return {resolved_min, resolved_max};
}

/// Probe round-trip serialization of each MO in \p amo, printing per-orbital
/// and total size and timing statistics. Intended for temporary diagnostics
/// before HF exchange; to be removed once no longer needed.
static void probe_hfex_serialization(World& world, const vecfuncT& amo) {
...
vecfuncT SCF::compute_residual(World& world, tensorT& occ, tensorT& fock,
    ...
    // at a consistent point (inputs freed, new_psi built): the FunctionImpl map is
    // the steady-state working set, while the per-host RSS approximates the in-apply
    // high-water mark (freed heap is not promptly returned to the OS). Adds fences,
    // so enable it only for memory runs, not timing runs.
    if (param.bsh_apply_memcheck()) {
        if (world.rank() == 0) print("=== BSH apply memcheck, mode:", bsh_apply_mode, "===");
