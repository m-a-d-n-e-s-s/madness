...
public:
  ...
  template <typename targetT>
  auto optimize_app(Molecule molecule,
                    targetT &target)
      -> OptimizationResults { ////!!!!!!! pass by value
    const int natom = molecule.natom();
    ...
    for (iter = 0; iter < maxiter; iter++) {
      ...
      bool dxconv = (iter > 0) && (dxmax < xtol);
      bool gconv = results.max_gradient < gtol;
      bool econv = (iter > 0) && (std::abs(de) < etol);
      bool converged = econv && dxconv && gconv;

      if (!converged && results.max_gradient < gradient_precision) {
        if (print_level > 0)
          print("\nInsufficient precision in gradient to proceed further -- "
                "forcing convergence\n");
        converged = true;
      ...
      if (print_level > 0) {
        ...
        printf(" ----------------  ---------  ---------  ---------  --- --- "
               "---\n");
        printf(" %15.6f   %9.2e  %9.2e  %9.2e   %s   %s   %s\n", e, de, dxmax,
               results.max_gradient, tf[econv], tf[dxconv], tf[gconv]);
        // print(e, de, econv, dxmax, dxconv, dxnorm, gmax, gconv, gnorm,
        // converged);
        print(" ");
      }
    ...
    results.final_energy = ep;
    results.nsteps = iter;
    results.final_geometry = molecule;
    // return the optimized molecule
    return results;
  }
};
