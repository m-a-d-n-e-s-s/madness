...
// Generalized Optimization using targetT to provide energy and gradient of a molecule
// targetT must have the following methods:
//  void energy_and_gradient(Molecule& molecule, double& energy, Tensor<double>& gradient)
//  double value(const Tensor<double>& x)  // value of target at x
//  Let's express that in code so we can get compile time errors if we don't have the right methods
//
//
//
...
class MolOpt {
 private:
  ...
  const double
      maxstep;  //< Maximum step in any one coordinate (currently Cartesian in a.u.)
  ...
  Tensor<double> new_search_direction(const Tensor<double>& g,
    ...
    for (int i = 0; i < e.dim(0); i++) {
      if (e[i] > 900.0) {
        ...
        if (print_level > 1)
          print("skipping redundant mode", i);
      } else if (e[i] <
    ...
    if (print_level > 1)
      print("cartesian dx before restriction", gv);
    ...
    for (int i = 0; i < gv.dim(0); i++) {
      if (fabs(gv[i]) > maxstep) {
        ...
        if (print_level > 0) {
          if (!printing)
            printf("  restricting step in Cartesian direction");
          printing = true;
          printf(" %d", i);
        }
      }
  ...
  Tensor<double> make_projector(const Molecule& molecule) {
    ...
    for (int k = 0; k < 3; k++)
      centroid(k) = coords(_, k).sum() / natom;
    if (print_level > 1)
