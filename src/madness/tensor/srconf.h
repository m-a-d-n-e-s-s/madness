...
namespace madness {
    ...
    public:
        ...
        /// check orthonormality at low rank additions at every xth addition
        static const long check_orthonormality_frequency = 100000;
    ...
    public:
        ...
        void add_SVD(const SRConf<T>& rhs, const double& thresh) {
            ...
            if (check_orthonormality_frequency > 0) {
                // get random number
                ...
                // 3. Define the distribution range [inclusive lower bound, inclusive upper bound]
                std::uniform_int_distribution<int> distrib(1, check_orthonormality_frequency);

                // 4. Generate random numbers
                long iran = distrib(gen);
                if (iran==1) {      // any number will do here..
                    print("checking orthonormality in add_SVD");
                    int isuccess=0;
                    if (not check_vector_orthonormality(0)) isuccess++;
                    if (not check_vector_orthonormality(1)) isuccess++;
                    if (not rhs.check_vector_orthonormality(0)) isuccess++;
                    if (not rhs.check_vector_orthonormality(1)) isuccess++;
                    if (isuccess>0) {
                        print("vector in add_svd not orthogonal");
                    }
                }
            }
    ...
    protected:
        ...
        /// @param[in] ivec	which vector (0 or 1) to check
        bool check_vector_orthonormality(const int ivec) const {
            // fast return if possible
            if (rank() == 0) return true;

            const tensorT t1 = ref_vector(ivec)(c0(ivec)).reshape(rank(), kVec(ivec));
