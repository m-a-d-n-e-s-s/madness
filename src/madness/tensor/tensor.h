...
/*!
  ...
  See tensor_macros.h for documentation on the easiest mechanisms for iteration
  over elements of tensors and tips for optimization.  See \c TensorIterator for
...
struct conditional_conj_struct {
  static Q op(const Q& coeff) { return coeff; }
};

/// For real types return value, for complex return conjugate
template <typename Q>
struct conditional_conj_struct<Q, true> {
  static Q op(const Q& coeff) { return conj(coeff); }
};
...
static inline std::ostream& operator<<(std::ostream& s, const TensorType& tt) {
...
#define TENSOR_ALIGNMENT 64
        ...
        // Ideally use if constexpr here but want headers C++14 for cuda
        // compatibility
...
#pragma GCC diagnostic pop
 ...
 public:
  ...
  /// Default constructor does not allocate any data and sets ndim=-1, size=0,
  /// _p=0, and id.
  Tensor() : _p(0) { _id = TensorTypeData<T>::id; }
...
#pragma GCC diagnostic pop
  ...
  template <class Q>
  operator Tensor<Q>() const {  // type conv => deep copy
  ...
  /// Create and optionally zero new n-d tensor. This is the most general
  /// constructor.

  /// @param[in] d Vector containing size of each dimension, number of
  /// dimensions inferred from vcector size.
  ...
  /// @param[in] t Conforming tensor to be subtracted out-of-place from this
  /// tensor
  /// @return New tensor
  template <typename Q>
  Tensor<TENSOR_RESULT_TYPE(T, Q)> operator-(const Tensor<Q>& t) const {
  ...
  /// Multiplication of tensor by a scalar of a supported type to produce a new
  /// tensor
  ...
  /// Add a scalar of the same type to all elements of a tensor producing a new
  /// tensor
  ...
  /// Subtract a scalar of the same type from all elements producing a new
  /// tensor
  ...
  /// @return %Reference to this tensor
  template <typename Q>
  typename IsSupported<TensorTypeData<Q>, Tensor<T>&>::type scale(Q x) {
    return (*this) *= x;
  }
  ...
  /// Inplace fill with random values ( \c [0,1] for floats, \c [0,MAXSIZE] for
  /// integers)
  ...
  /// Each element is assigned it's logical index according to this loop
  /// structure \code Tensor<float> t(5,6,7,...) long index=0; for (long i=0;
  /// i<_dim[0]; ++i)
  ...
  Tensor<T>& fillindex() {
    ...
    UNARY_UNOPTIMIZED_ITERATOR(T, (*this),
                               *_p0 = count++);  // Fusedim would be OK
    return *this;
  }

  /// Inplace set elements of \c *this less than \c x in absolute magnitude to
  /// zero.
  ...
  /// Return a 1d constant Tensor that views the specified range of the 2d
  /// Tensor
  ...
  /// Return a 2d constant Tensor that views the specified range of the 2d
  /// Tensor
  ...
  /// Return a 3d constant Tensor that views the specified range of the 3d
  /// Tensor
  ...
  /// Return a 2d constant Tensor that views the specified range of the 3d
  /// Tensor
  ...
  /// Return a 1d constant Tensor that views the specified range of the 3d
  /// Tensor
  ...
  /// Return a 1-4d constant Tensor that views the specified range of the 4d
  /// Tensor
  ...
  /// Return a 1-5d constant Tensor that views the specified range of the 5d
  /// Tensor
  ...
  /// Return a 1-6d constant Tensor that views the specified range of the 6d
  /// Tensor
  ...
  /// Returns new view/tensor rehaping to conforming 2-d tensor with given
  /// dimensions
  ...
  /// Returns new view/tensor rehaping to conforming 3-d tensor with given
  /// dimensions
  ...
  /// Returns new view/tensor rehaping to conforming 4-d tensor with given
  /// dimensions
  ...
  /// Returns new view/tensor rehaping to conforming 5-d tensor with given
  /// dimensions
  ...
  /// Returns new view/tensor rehaping to conforming 6-d tensor with given
  /// dimensions
  ...
  /// Returns new view/tensor splitting dimension \c i as \c dimi0*dimi1 to
  /// produce conforming d+1 dimension tensor
  ...
  /// Returns new view/tensor cycling the sub-dimensions `(start,...,end)` with
  /// `shift` steps
  ...
  /// Return the absolute minimum value (and if ind is non-null, its index) in
  /// the Tensor
  ...
  /// Return the absolute maximum value (and if ind is non-null, its index) in
  /// the Tensor
  ...
  /// Return the trace of two tensors with complex conjugate of the leftmost
  /// (i.e., this)
  template <class Q>
  ...
  BaseTensor* base() { return static_cast<BaseTensor*>(this); }
...
/// Transforms one dimension of the tensor t by the matrix c, returns new
/// contiguous tensor
...
/// \endcode
/// @param[in] t Tensor to transform (size of dimension to be transformed must
/// match size of first dimension of \c c )
...
/// Returns a new deep copy of the complex conjugate transpose of the input
/// tensor
...
template <class T>
class SliceTensor : public Tensor<T> {
 private:
  SliceTensor<T>();
...
template <>
float_complex Tensor<float_complex>::min(long* ind) const;
template <>
double_complex Tensor<double_complex>::min(long* ind) const;
template <>
float_complex Tensor<float_complex>::max(long* ind) const;
template <>
double_complex Tensor<double_complex>::max(long* ind) const;
...
Tensor<T> outer(const Tensor<T>& left, const Tensor<T>& right) {
  ...
  TENSOR_ASSERT(nd <= TENSOR_MAXDIM, "too many dimensions in result", nd, 0);
...
/// Inner product ... result(i,j,...,p,q,...) = sum(z)
/// left(i,j,...,z)*right(z,p,q,...)
...
/// Accumulate inner product into user provided, contiguous, correctly sized
/// result tensor
...
/// \code
/// result(i,j,k...) <-- sum(i',j', k',...) t(i',j',k',...) c(i',i) c(j',j)
/// c(k',k) ... \endcode The input dimensions of \c t must all be the same and
/// agree with the first dimension of \c c .  The dimensions of \c c may differ
/// in size.  If the dimensions of \c c are the same, and the operation is being
/// performed repeatedly, then you might consider calling \c fast_transform
/// instead which enables additional optimizations and can eliminate all
...
/// distinct matrix.
/// \code
/// result(i,j,k...) <-- sum(i',j', k',...) t(i',j',k',...) c[0](i',i)
...
///
/// \code
///     result(i,j,k,...) <-- sum(i',j', k',...) t(i',j',k',...)  c(i',i)
///     c(j',j) c(k',k) ...
/// \endcode
///
...
/// Return a new tensor holding the argument of each element of t (complex types
/// only)
...
/// Return a new tensor holding the real part of each element of t (complex
/// types only)
...
/// Return a new tensor holding the imaginary part of each element of t (complex
/// types only)
...
/// Returns a new deep copy of the complex conjugate of the input tensor
/// (complex types only)
