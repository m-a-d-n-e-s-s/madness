...
#define OLDXXX
    ...
    public:
        ...
        mutable volatile state_t* volatile fifo[MAX_NTHREAD]; // Circular buffer of flags
    ...
    public:
      ConditionVariable() : back(0), front(0), fifo() { }
        ...
        void wait() const {
            ...
            //while (!state.ready && state.yield) std::this_thread::yield();
            while (!state.ready) cpu_relax();
            lock();
            //fifo[b] = nullptr;
        }
        ...
        void signal() const {
          ...
          //if (fifo[next]) fifo[next]->yield = false;
          this->front = next;
        }
