...
/// approach is to define load/store methods for a pointer to the
...
///
/// Other things that you know are contiguously stored in memory
...
/// files by wrapping an instance of the \c fstream class, an STL \c
/// vector<unsigned char> and a general buffer for storing an archive
/// in memory.  The buffer and \c vector archives are bitwise identical to the
/// binary file archive.  There is also an efficient mapping to
/// MPI for passing data between processes.
///
