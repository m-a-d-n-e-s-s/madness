...
namespace madness {
  namespace archive {
	...
	for (long i=0; i<n; i++) os << t[i] << std::endl;
      ...
      void store(const char* t, long n) {
	...
	os << std::endl;
      ...
      void store(const unsigned char* t, long n) {
	...
	if (os.is_open()) os.flush();
      ...
      ~TextFstreamOutputArchive() {close();};
    };
	...
	if (filename) open(filename, mode);
      ...
      template <class T>
      void check_start_tag(bool end=false) {
	...
	eat_eol();
      ...
      void load(unsigned char* t, long n) {
	for (long i=0; i<n; i++) {
	  unsigned int x;
	  is >> x;
	  t[i] = (unsigned char) x;
	...
	is.close();
      ...
      struct ArchivePrePostImpl<TextFstreamOutputArchive,T> {
          static void preamble_store(TextFstreamOutputArchive& ar) {
              ar.store_start_tag<T>();
          };
          static inline void postamble_store(TextFstreamOutputArchive& ar) {
              ar.store_end_tag<T>();
          };
      };
      ...
      struct ArchivePrePostImpl<TextFstreamInputArchive,T> {
          static inline void preamble_load(TextFstreamInputArchive& ar) {
              ar.check_start_tag<T>();
          };
          static inline void postamble_load(TextFstreamInputArchive& ar) {
              ar.check_end_tag<T>();
          };
      };
  }
