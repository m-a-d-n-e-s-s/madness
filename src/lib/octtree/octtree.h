...
namespace madness {
    ...
    inline Translation two_to_power(long n) {
        return Translation(1)<<n;
    }
...
//            std::cout << "OctTree copy constructor: the beginning" << std::endl;
            ...
            _comm = t._comm;
            _cost = t._cost;
            _localSubtreeCost = t._localSubtreeCost;
            _visited = t._visited;
            _data = t._data;
            _sendto = t._sendto;
            //FORIJK(_c[i][j][k] = 0;);
...
//	    std::cout << "OctTree destructor" << std::endl;
        ...
        /// Set cost of node
        inline void setCost(Cost cost) {
            _cost = cost;
        };

        /// Get cost for local subtree
        inline Cost getLocalSubtreeCost() {
            return _localSubtreeCost;
        };

        /// Set cost for local subtree
        inline void setLocalSubtreeCost(Cost cost) {
            _localSubtreeCost = cost;
        };

        /// Level of node in tree (0 is highest)
        inline Level n() const {
            return _n;
        };

        /// Set x index of node (0,...,2**n-1)
        inline void setx(Translation x) {
            _x = x;
        };

        /// x index of node (0,...,2**n-1)
        inline Translation x() const {
            return _x;
        };

        /// Set y index of node (0,...,2**n-1)
        inline void sety(Translation y) {
            _y = y;
        };

        /// y index of node (0,...,2**n-1)
        inline Translation y() const {
            return _y;
        };

        /// Set z index of node (0,...,2**n-1)
        inline void setz(Translation z) {
            _z = z;
        };

        /// z index of node (0,...,2**n-1)
        inline Translation z() const {
            return _z;
        };

        /// Set remote (true if node is remote, false otherwise)
        inline void setRemote(bool remote) {
            _remote = remote;
        };
        ...
        inline void setRank(ProcessID rank) {
            _rank = rank;
        };
        ...
        inline OctTreeT* parent() const {
            return _p;
        };

        /// set the child's parent pointer
        inline void setParent(OctTreeT* p) {
            _p = p;
        };
...
//			<< std::endl;
        ...
        inline void setSendto(ProcessID p) {
            _sendto = p;
        }
        inline ProcessID getSendto() {
            return _sendto;
        }
