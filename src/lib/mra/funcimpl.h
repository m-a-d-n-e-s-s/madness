...
namespace madness
{
    ...
    template<typename T, int NDIM>
        ...
        public:
            ...
            {   _norm_tree = norm_tree; return None;}
        ...
        public:
            ...
            , apply_time(SharedPtr<ApplyTime<NDIM> > ())
            {
                ...
                if (empty)
                { // Do not set any coefficients at all
                }
                else if (functor)
                { // Project function and optionally refine
            ...
            {   bc=copy(value); MADNESS_ASSERT(bc.dim[0]==NDIM && bc.dim[1]==2 && bc.ndim==2);}
...
#define FINER_GRAIN
                ...
                { // Assumes monotonic decay beyond nearest neighbor
                    break;
                }
            }
            ...
            if (key.level() > 0) {
                ...
                if (parent.level() > 0) {
                    for (IndexIterator it(NDIM,3); it; ++it) {
                        Translation l[NDIM];
                        for (int d=0; d<NDIM; d++) l[d] = it[d]-2;
