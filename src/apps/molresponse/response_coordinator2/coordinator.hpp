...
struct runSchema {
    ...
    explicit runSchema(World &world, const Molecule &molecule, const CalculationParameters &moldft_params) {


        std::string xc = moldft_params.xc();
        ...
        auto moldft_hash = std::hash<json>{}(moldft_json);
        auto moldft_dir = "moldft_" + std::to_string(moldft_hash);
...
struct moldftSchema {
    ...
    moldftSchema(World &world, const runSchema &schema) {
        moldft_path = schema.moldft_path;
...
frequencySchema::frequencySchema(World &world, const runSchema &run_schema, const moldftSchema &m_schema, std::string r_operator, bool static_calc)
    : op(std::move(r_operator)), moldft_path(m_schema.moldft_path) {
    if (!static_calc) {
        if (world.rank() == 0) {
            print_schema();
            freq = {0.0};
...
void runMOLDFT(World &world, const moldftSchema &moldftSchema, bool try_run, bool restart, const std::string &precision) {
    ...
    if (world.rank() == 0) {
        ...
        // param1.set_user_defined_value<std::string>("xc");
