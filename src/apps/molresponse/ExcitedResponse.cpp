...
X_space ExcitedResponse::create_trial_functions(World &world, size_t k) const {
  ...
  // Truncate
  madness::truncate(world, trials_X, madness::FunctionDefaults<3>::get_thresh(),
                    true);
...
void ExcitedResponse::iterate_trial(World &world, X_space &guesses) {
  ...
  while (iteration < r_params.guess_max_iter()) {
    ...
    if (iteration + 1 < r_params.guess_max_iter()) {
      ...
      auto bsh_x_operators = create_bsh_operators(
          world, x_shifts, ground_energies, omega, r_params.lo(),
...
void ExcitedResponse::deflateGuesses(World &world, X_space &Chi,
                                     X_space &Lambda_X, Tensor<double> &S,
                                     Tensor<double> &omega, size_t &iteration,
                                     size_t &m) const {
  // XX =Omega XAX
  ...
  omega.clear();
  diagonalizeFockMatrix(world, Chi, Lambda_X, omega, XAX, S,
                        FunctionDefaults<3>::get_thresh());
}

void ExcitedResponse::deflateTDA(World &world, X_space &Chi, X_space &old_Chi,
                                 X_space &Lambda_X, X_space &old_Lambda_X,
                                 Tensor<double> &S, Tensor<double> old_S,
                                 Tensor<double> old_A, Tensor<double> &omega,
                                 size_t &iteration, size_t &m) {
  S = response_space_inner(Chi.X, Chi.X);
  Tensor<double> XAX = response_space_inner(Chi.X, Lambda_X.X);
  ...
  if (iteration < r_params.larger_subspace()) {
    ...
    unaugment(world, Chi, old_Chi, Lambda_X, old_Lambda_X, omega, S, XAX, old_S,
              old_A, r_params.num_states(), iteration, r_params.print_level());
  }
}

void ExcitedResponse::deflateFull(World &world, X_space &Chi, X_space &old_Chi,
                                  X_space &Lambda_X, X_space &old_Lambda_X,
                                  Tensor<double> &S, Tensor<double> old_S,
                                  Tensor<double> old_A, Tensor<double> &omega,
                                  size_t &iteration, size_t &m) {
  ...
  if (iteration < r_params.larger_subspace() and iteration > 0) {
    ...
    augment_full(world, Chi, old_Chi, Lambda_X, old_Lambda_X, S, A, old_S,
                 old_A, r_params.print_level());
  ...
  Tensor<double> U = diagonalizeFullResponseMatrix(
      world, Chi, Lambda_X, omega, S, A, FunctionDefaults<3>::get_thresh(),
      r_params.print_level());

  if (iteration < r_params.larger_subspace() and iteration > 0) {
    print("Entering Unaugment Full");
    unaugment_full(world, Chi, old_Chi, Lambda_X, old_Lambda_X, omega, S, A,
                   old_S, old_A, r_params.num_states(), iteration,
                   r_params.print_level());
  } else {
    old_Chi = Chi.copy();
...
void ExcitedResponse::augment(World &world, X_space &Chi, X_space &old_Chi,
                              X_space &Lambda_X, X_space &last_Lambda_X,
                              Tensor<double> &S, Tensor<double> &A,
...
// the right spot
void ExcitedResponse::augment_full(World &world, X_space &Chi, X_space &old_Chi,
                                   X_space &Lambda_X, X_space &last_Lambda_X,
                                   Tensor<double> &S, Tensor<double> &A,
                                   Tensor<double> &old_S, Tensor<double> &old_A,
                                   size_t print_level) {
...
void ExcitedResponse::unaugment(World &world, X_space &Chi, X_space &old_Chi,
                                X_space &Lambda_X, X_space &last_Lambda_X,
                                Tensor<double> &omega, Tensor<double> &S_x,
                                Tensor<double> &A_x, Tensor<double> &old_S,
                                Tensor<double> &old_A, size_t num_states,
                                size_t iter, size_t print_level) {
  // Basic output
  if (print_level >= 1)
    molresponse::start_timer(world);
...
void ExcitedResponse::unaugment_full(World &world, X_space &Chi,
                                     X_space &old_Chi, X_space &Lambda_X,
...
Tensor<double> ExcitedResponse::diagonalizeFullResponseMatrix(
    World &world, X_space &Chi, X_space &Lambda_X, Tensor<double> &omega,
    Tensor<double> &S, Tensor<double> &A, const double thresh,
    size_t print_level) {
...
Tensor<double> ExcitedResponse::GetFullResponseTransformation(
...
Tensor<double> ExcitedResponse::diagonalizeFockMatrix(
    World &world, X_space &Chi, X_space &Lambda_X, Tensor<double> &evals,
    Tensor<double> &A, Tensor<double> &S, const double thresh) const {
  // compute the unitary transformation matrix U that diagonalizes
  // the fock matrix
...
Tensor<double> ExcitedResponse::get_fock_transformation(
    World &world, Tensor<double> &overlap, Tensor<double> &fock,
    Tensor<double> &evals, const double thresh_degenerate) const {
  // Run an SVD on the overlap matrix and ignore values
  // less than thresh_degenerate
...
Tensor<int> ExcitedResponse::sort_eigenvalues(World &world,
                                              Tensor<double> &vals,
...
Tensor<double> ExcitedResponse::create_shift(World &world,
                                             const Tensor<double> &ground,
...
response_space ExcitedResponse::apply_shift(World &world,
                                            const Tensor<double> &shifts,
                                            const response_space &V,
...
void ExcitedResponse::excited_to_json(json &j_mol_in, size_t iter,
...
void ExcitedResponse::iterate(World &world) {
  ...
  for (iter = 0; iter < r_params.maxiter(); ++iter) {
    ...
    update_x_space_excited(world, old_Chi, Chi, old_Lambda_X, residuals, xc,
                           projector, omega, kain_x_space, Xvector, Xresidual,
                           energy_residuals, old_energy, bsh_residualsX,
                           bsh_residualsY, S, old_S, A, old_A, iter, maxrotn);

    excited_to_json(j_molresponse, iter, bsh_residualsX, bsh_residualsY,
                    density_residuals, omega);
...
void ExcitedResponse::update_x_space_excited(
    World &world, X_space &old_Chi, X_space &Chi, X_space &old_Lambda_X,
    X_space &res, XCOperator<double, 3> &xc, QProjector<double, 3> &projector,
    Tensor<double> &omega, NonLinearXsolver &kain_x_space,
  ...
  print(omega);
  old_energy = omega;
  compute_new_omegas_transform(world, old_Chi, Chi, old_Lambda_X, Lambda_X,
                               omega, old_energy, S, old_S, A, old_A,
                               energy_residuals, iter);
  ...
  if (iter == r_params.maxiter() - 1) {
    ...
    if (r_params.kain() && (iter > 0) && true) {
      temp = kain_x_space_update(world, Chi, res, kain_x_space, Xvector,
                                 Xresidual);
    }
    if (iter > 0) {
...
void ExcitedResponse::compute_new_omegas_transform(
    World &world, X_space &old_Chi, X_space &Chi, X_space &old_Lambda_X,
    X_space &Lambda_X, Tensor<double> &omega, Tensor<double> &old_energy,
    Tensor<double> &S, Tensor<double> &old_S, Tensor<double> &A,
    Tensor<double> &old_A, Tensor<double> &energy_residuals, size_t iter) {
  size_t m = Chi.X.size();
  bool compute_y = not r_params.tda();
  // Basic output
  if (r_params.print_level() >= 1 and world.rank() == 0) {
  ...
  if (!compute_y) {
    deflateTDA(world, Chi, old_Chi, Lambda_X, old_Lambda_X, S, old_S, old_A,
               omega, iter, m);
    // Constructing S
    // Full TDHF
  } else {
    deflateFull(world, Chi, old_Chi, Lambda_X, old_Lambda_X, S, old_S, old_A,
                omega, iter, m);
  }
...
X_space ExcitedResponse::bsh_update_excited(World &world,
                                            const Tensor<double> &omega,
                                            X_space &theta_X,
                                            QProjector<double, 3> &projector) {
  ...
  std::vector<std::vector<std::shared_ptr<real_convolution_3d>>> bsh_x_ops =
      create_bsh_operators(world, x_shifts, ground_energies, omega_plus,
                           r_params.lo(), FunctionDefaults<3>::get_thresh());
  std::vector<std::vector<std::shared_ptr<real_convolution_3d>>> bsh_y_ops;
  if (compute_y) {
    Tensor<double> omega_minus = -omega;
    bsh_y_ops =
        create_bsh_operators(world, y_shifts, ground_energies, omega_minus,
                             r_params.lo(), FunctionDefaults<3>::get_thresh());
  }
...
void ExcitedResponse::analysis(World &world, const X_space &chi) {
  ...
  if (world.rank() == 0) {
    for (long i = 0; i < m; i++) {
      printf("   Response Function %d\t\t%7.8f a.u.", static_cast<int>(i),
             omega(i));
      print("\n   --------------------------------------------");
