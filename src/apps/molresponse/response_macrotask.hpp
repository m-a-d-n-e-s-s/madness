...
namespace madness
{
  ...
  public:
        ...
        const vector_real_function_3d &dipole_perturbations) const
    {
      ...
      {
        // left right pairs ka and kb and apply apply
        // TODO: There maybe an optimization that can tell whether we can skip this orbital or not
  ...
  public:
    ...
    // beta_abc = -2 * <bx|cy * va> + <cx|by * va> + <zeta_bc|phi0 * va> + <zeta_cb|phi0 * va> + <ax|vbcx> + <ay|vbcy>
    // beta[i], a, b, c, A, B, C, zeta_BC, zeta_CB, phi0, dipole_perturbations, vbc
    typedef std::tuple<
...

    {
      ...
      auto compute_beta_i = [&](
                                const vector_real_function_3d &ax,
                                ...
                                const vector_real_function_3d &vbcy)
      {
        ...
        auto beta = one_trace + two_trace + three_trace + four_trace + five_trace +
                    six_trace;
        *result[0] = -2.0 * beta;
        return result;
      };
      ...
      return compute_beta_i(ax, ay, bx, by, cx, cy, zeta_bc, zeta_cb, phi0, va, vbcx, vbcy);
    }
  };
  ...
  public:
    ...
    typedef std::tuple<
        const std::vector<int> &, const std::vector<int> &,
        const vector_real_function_3d &, const vector_real_function_3d &, const vector_real_function_3d &>
        argtupleT;
    ...
    operator()(
        const std::vector<int> &b, const std::vector<int> &c,
        const vector_real_function_3d &B, const vector_real_function_3d &C, const vector_real_function_3d &phi0) const
    {
