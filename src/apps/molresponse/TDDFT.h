...
public:
    ...
    void orbital_load_balance(World& world, vecfuncT& psi0, vecfuncT& psi0_copy, X_space& Chi,
                              X_space& Chi_copy);
    ...
    X_space create_trial_functions(World& world, size_t k, std::vector<real_function_3d>& orbitals,
                                   size_t print_level);
    X_space create_trial_functions2(World& world, std::vector<real_function_3d>& orbitals,
                                    size_t print_level);
    ...
    X_space compute_gamma_full(World& world, X_space& Chi, const XCOperator<double, 3>& xc);
    ...
    void PrintRFExpectation(World& world, response_space f, response_space g, std::string fname,
                            std::string gname);
    ...
    X_space Compute_Lambda_X(World& world, X_space& Chi, XCOperator<double, 3> xc,
                             std::string calc_type);
    // Returns the hamiltonian matrix, equation 45 from the paper
    // -2.0 * (ground_state_energy + excited_state_energy) is positive
    Tensor<double> create_shift(World& world, Tensor<double>& ground, Tensor<double>& omega,
    ...
    Tensor<double> create_shift_target(World& world, Tensor<double>& ground, Tensor<double>& omega,
                                       double target, size_t print_level, std::string xy);

    // Returns the given shift applied to the given potentials
    response_space apply_shift(World& world, Tensor<double>& shifts, response_space& V,
                               response_space& f);
    // single shift value
    ...
    std::vector<std::shared_ptr<real_convolution_3d>> CreateBSHOperatorPropertyVector(
            World& world, double& shift, Tensor<double>& ground, double& omega, double lo,
            double thresh);

    void update_x_space_response(World& world, X_space& Chi, X_space& res,
    ...
    X_space bsh_update_response(World& world, X_space& theta_X, std::vector<poperatorT>& bsh_x_ops,
                                std::vector<poperatorT>& bsh_y_ops,
                                QProjector<double, 3>& projector, double& x_shifts);
    ...
    void print_residual_norms(World& world, X_space& old_Chi, bool compute_y, size_t iteration);
    ...
    void update_x_space_excited(World& world, X_space& old_Chi, X_space& Chi, X_space& old_Lambda_X,
                                X_space& residuals, XCOperator<double, 3>& xc,
    ...
    void augment(World& world, X_space& Chi, X_space& old_Chi, X_space& Lambda_X,
                 X_space& last_Lambda_X, Tensor<double>& S_x, Tensor<double>& A_x,
    ...
    Tensor<double> GetFullResponseTransformation(World& world, Tensor<double>& S, Tensor<double>& A,
                                                 Tensor<double>& evals, const double thresh);
    ...
    // parameters
    XCOperator<double, 3> create_XCOperator(World& world, std::vector<real_function_3d> orbitals,
                                            std::string xc);

    // Uses an XCOperator<double,3>  to construct v_xc for the ground state
    // density Returns d^2/d rho^2 E_xc[rho]
    std::vector<real_function_3d> create_fxc(World& world, std::vector<real_function_3d>& orbitals,
    ...
    response_space diagonalize_CIS_guess(World& world, std::vector<real_function_3d>& virtuals,
                                         Tensor<double>& omega,
    ...
    Tensor<double> CreateGroundHamiltonian(World& world, std::vector<real_function_3d> f,
                                           size_t print_level);
    ...
    X_space create_random_guess(World& world, size_t m, size_t n, vector_real_function_3d& grounds,
                                Molecule& molecule);

    // Creates random guess functions semi-intelligently(?)
    std::vector<real_function_3d> create_random_guess(World& world, size_t m,
    ...
    void compute_and_print_polarizability(World& world, X_space& Chi, X_space& PQ,
                                          std::string message);
    void plot_excited_states(World& world, size_t iteration, response_space& x_response,
    ...
    static void excited_to_json(json& json, size_t iter, const Tensor<double>& tensor,
                                const Tensor<double>& tensor1, const Tensor<double>& tensor2,
                                const Tensor<double>& tensor3);
    static void frequency_to_json(json& json, size_t iter, const Tensor<double>& tensor,
                                  const Tensor<double>& tensor1, const Tensor<double>& tensor2,
                                  const Tensor<double>& tensor3);
...
public:
    void load_calc(World & world,TDDFT* p,double thresh){
        p->set_protocol<3>(world,thresh);
        p->load(world,p->r_params.restart_file());
        p->check_k(world, thresh, FunctionDefaults<3>::get_k());

    }
    X_space compute_gamma_full(World & world,TDDFT* p,double thresh){
        auto xc = p->create_XCOperator(world, p->ground_orbitals, p->r_params.xc());
        X_space gamma=p->compute_gamma_full(world,p->Chi,xc);
        return gamma;
    }

    X_space compute_lambda_X(World & world,TDDFT* p,double thresh){
        auto xc = p->create_XCOperator(world, p->ground_orbitals, p->r_params.xc());
        X_space gamma=p->Compute_Lambda_X(world,p->Chi,xc,p->r_params.calc_type());
        return gamma;
    }
    std::pair<X_space,X_space> compute_VFOX(World & world,TDDFT* p,bool compute_y){
        auto xc = p->create_XCOperator(world, p->ground_orbitals, p->r_params.xc());
