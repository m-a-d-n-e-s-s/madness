...
std::map<std::vector<int>, real_function_3d> TDDFT::solid_harmonics(World &world, int n) {
  ...
  real_function_3d c =
      real_factory_3d(world).functor(real_functor_3d(new BS_MomentFunctor(std::vector<int>{0, 0, 0})));
...
X_space TDDFT::create_trial_functions(World &world,
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    for (const auto &key : solids) {
      // Temp zero functions
      std::vector<real_function_3d> temp = zero_functions_compressed<double, 3>(world, static_cast<int>(n));
...
void TDDFT::make_nuclear_potential(World &world) {
  molresponse::start_timer(world);
  potential_manager = std::shared_ptr<PotentialManager>(new PotentialManager(molecule, r_params.core_type()));
...
void TDDFT::deflateFull(World &world,
  ...
  if (iteration < r_params.larger_subspace() and iteration > 0) {
    ...
    augment_full(world, Chi, old_Chi, Lambda_X, old_Lambda_X, S, A, old_S, old_A, r_params.print_level());
