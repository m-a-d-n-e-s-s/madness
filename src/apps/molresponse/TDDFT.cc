...
void TDDFT::load_balance(World &world,
                         vecfuncT rho_omega,
                         X_space Chi,
                         X_space Chi_old) {
    molresponse::start_timer(world);
...
void TDDFT::vector_stats_new(const Tensor<double> v,
                             double &rms,
                             double &maxabsval) const {
    rms = 0.0;
    for (size_t i = 0; i < v.size(); ++i) {
...
void TDDFT::x_space_step_restriction(World &world,
    ...
    for (size_t b = 0; b < m; b++) {
        if (true) {
            ...
            do_step_restriction(
                    world, old_Chi.X[b], temp.X[b], "x_response", maxrotn[b]);

            do_step_restriction(
                    world, old_Chi.Y[b], temp.Y[b], "y_response", maxrotn[b]);
...
response_space TDDFT::diagonalize_CIS_guess(
    ...
    if (true_virtuals.empty()) MADNESS_EXCEPTION(
...
void TDDFT::compute_and_print_polarizability(World &world,
                                             X_space &Chi,
                                             X_space &PQ,
                                             std::string message) {
