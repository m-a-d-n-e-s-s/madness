...
void ResponseBase::check_k(World &world, double thresh, int k) {
    ...
    if (FunctionDefaults<3>::get_k() != ground_orbitals[0].k()) {
        ...
        for (auto &orbital: ground_orbitals) {
            orbital = project(orbital, FunctionDefaults<3>::get_k(), thresh,
                              false);
        }
        world.gop.fence();
    ...
    if (r_params.store_potential()) {
        if (FunctionDefaults<3>::get_k() != stored_potential[0][0].k()) {
            ...
            for (auto &potential_vector: stored_potential) {
                ...
                for (auto &vi: potential_vector) {
                    vi = project(vi, FunctionDefaults<3>::get_k(), thresh,
                                 false);
                }
                world.gop.fence();
            }
        ...
        if (FunctionDefaults<3>::get_k() != stored_v_coul.k())
            stored_v_coul = project(stored_v_coul, FunctionDefaults<3>::get_k(),
                                    thresh, false);
        if (FunctionDefaults<3>::get_k() != stored_v_nuc.k())
            stored_v_nuc = project(stored_v_nuc, FunctionDefaults<3>::get_k(),
                                   thresh, false);
    }
    // project the mask
...
auto ResponseBase::ComputeHamiltonianPair(World &world) const
    ...
    for (const auto &phi_i: phi) {
        ...
        /// and v[i] --- q[i] = a * v[i]
        auto psif = mul_sparse(world, phi_i, phi,
        ...
        psif = mul_sparse(world, phi_i, psif,
                          FunctionDefaults<3>::get_thresh());
    ...
    if (r_params.xc() == "hf") {
        // Construct phiVphi
        phiVphi = matrix_inner(world, phi, v_phi0) -
                  matrix_inner(world, phi, Kphi);
    } else {// DFT
...
void ResponseBase::load_balance_chi(World &world) {
    ...
    FunctionDefaults<3>::redistribute(
            world,
            lb.load_balance(
                    r_params.loadbalparts()));// 6.0 needs retuning after
    world.gop.fence();
...
auto ResponseBase::make_bsh_operators_response(World &world, double &shift,
    ...
    std::for_each(ops.begin(), ops.end(), [&](auto &operator_p) {
        ...
        operator_p =
                poperatorT(BSHOperatorPtr3D(world, mu, r_params.lo(), tol));
    });
    if (r_params.print_level() >= 1) {
...
auto ResponseBase::compute_gamma(World &world, const gamma_orbitals &density,
    ...
    inner_to_json(world, "k1", response_context.inner(chi_alpha, K),
                  iter_function_data);
    if (r_params.print_level() >= 1) {
    ...
    if (c_xc != 1.0) {
        ...
        inner_to_json(world, "v1_xc", response_context.inner(chi_alpha, W),
                      iter_function_data);
...
auto ResponseBase::compute_theta_X(World &world, const X_space &chi,
    ...
    if (r_params.localize() != "canon") {
        ...
        inner_to_json(world, "E0", response_context.inner(chi, E0X),
                      iter_function_data);
        if (r_params.print_level() >= 20) {
    ...
    if (r_params.print_level() >= 1) {
        molresponse::end_timer(world, "gamma_compute", "gamma_compute",
                               iter_timing);
    }

    inner_to_json(world, "gamma_x", response_context.inner(chi, gamma),
                  iter_function_data);

    if (r_params.print_level() >= 1) { molresponse::start_timer(world); }
    Theta_X = (V0X - E0X) + gamma;

    inner_to_json(world, "theta_x", response_context.inner(chi, Theta_X),
                  iter_function_data);
    world.gop.fence();
    //    Theta_X.truncate();
    if (r_params.print_level() >= 1) {
        molresponse::end_timer(world, "compute_ThetaX_add",
                               "compute_ThetaX_add", iter_timing);
    }
    if (r_params.print_level() >= 20) {
...
auto ResponseBase::compute_gamma_static(World &world,
    ...
    inner_to_json(world, "j1", response_context.inner(xy, J),
                  iter_function_data);
    if (xcf.hf_exchange_coefficient() != 1.0) {
    ...
    inner_to_json(world, "gamma_x_beforeQ", response_context.inner(xy, gamma),
                  iter_function_data);
    if (r_params.print_level() >= 1) {
...
auto ResponseBase::compute_V0X(World &world, const X_space &X,
    ...
    if (xcf.hf_exchange_coefficient() != 1.0) {
        ...
        v_xc = Function<double, 3>(
                FunctionFactory<double, 3>(world).fence(false).initial_level(
                        1));
    }
    if (r_params.print_level() >= 1) { molresponse::start_timer(world); }
    ...
    if (r_params.print_level() >= 15) {
        ...
        inner_to_json(world, "v0_nuc", response_context.inner(X, v_nuc * X),
                      iter_function_data);
        inner_to_json(world, "j0", response_context.inner(X, v_j0 * X),
                      iter_function_data);
        inner_to_json(world, "k0", response_context.inner(X, K0),
                      iter_function_data);
        inner_to_json(world, "v0_xc", response_context.inner(X, v_xc * X),
                      iter_function_data);
    }
    if (r_params.print_level() >= 1) {
    ...
    if (compute_Y) {
        ...
        V0 += -c_xc * K0;
        V0.truncate();
        inner_to_json(world, "v0", response_context.inner(X, V0),
                      iter_function_data);
    } else {
        V0 = X.copy();
        V0.x = v0 * V0.x;
...
auto ResponseBase::update_residual(World &world, const X_space &chi,
    ...
    if (compute_y) {
        ...
        for (const auto &b: chi.active) {
            residual_norms(b) =
                    norm2(world, res.x[b]) / norm2(world, g_chi.x[b]);
        }
    }
    if (r_params.print_level() >= 1) {
...
auto ResponseBase::kain_x_space_update(World &world, const X_space &chi,
    ...
    if (compute_y) {
        ...
        for (const auto &i: Chi.active) {
            kain_update.x[i] =
                    kain_x_space[i].update(chi.x[i], residual_chi.x[i]);
        }
    }
    if (r_params.print_level() >= 1) {
...
void ResponseBase::x_space_step_restriction(World &world,
    ...
    if (compute_y) {
        ...
        for (size_t b = 0; b < m; b++) {
            ...
            if (world.rank() == 0) {
                ...
                if (world.rank() == 0) {
                    print("max_step = max_rotation*norm_X: ", max_step);
                }
            }
            if (step_size > max_step && step_size < 25) {
                ...
                if (world.rank() == 0) {
                    if (r_params.print_level() > 1)
                        print("  restricting step for response-state: ", b,
                              " step size", s);
                }
        ...
        for (size_t b = 0; b < m; b++) {
            ...
            if (step_size > max_step &&
                step_size < 10000 * FunctionDefaults<3>::get_thresh()) {
                // and if the step size is less thant 10% the vector norm
                double s = .80 * max_step / step_size;
                if (world.rank() == 0) {
                    if (r_params.print_level() > 1)
    ...
    if (r_params.print_level() >= 1) {
        molresponse::end_timer(world, "x_space_restriction",
                               "x_space_restriction", iter_timing);
    }
}
...
void ResponseBase::solve(World &world) {
    ...
    for (const auto &iter_thresh: protocol) {
        ...
        if (first_protocol) {
            ...
            if (world.rank() == 0) {
                print("Successfully check K first initialization ");
            }
            first_protocol = false;
        } else {
            check_k(world, iter_thresh, FunctionDefaults<3>::get_k());
    ...
    if (0 != Chi.x.size()) {
        if (FunctionDefaults<3>::get_k() != Chi.x[0].at(0).k()) {
            ...
            for (auto &xi: Chi.x) {
                ...
                for (auto &xij: xi) {
                    xij = project(xij, FunctionDefaults<3>::get_k(), thresh,
                                  false);
                }
                world.gop.fence();
            }
            for (auto &yi: Chi.y) {
                ...
                for (auto &yij: yi) {
                    yij = project(yij, FunctionDefaults<3>::get_k(), thresh,
                                  false);
                }
                world.gop.fence();
            }
...
void response_timing::add_data(
    ...
    std::for_each(cpu_time_data.begin(), cpu_time_data.end(), [&values](auto &v) {
        ...
        v.second.push_back(
                values[v.first]
                        .second);// .first to get first value of pair wall_time
    });
}
...
void response_data::add_data(std::map<std::string, Tensor<double>> values) {
    ...
    std::for_each(function_data.begin(), function_data.end(), [&values](auto &v) {
        v.second.push_back(
                values[v.first]);// .first to get first value of pair wall_time
    });
}
