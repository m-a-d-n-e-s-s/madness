...
// void ResponseBase::plotResponseOrbitals(World &world, size_t iteration, const response_space &x_response, const response_space &y_response, ResponseParameters const &responseParameters,
...
//             do_vtk_plots(world, int(r_params.plot_pts()), r_params.plot_l(), 0, int(r_params.num_orbitals()), molecule, square(world, ground_orbitals), "ground");
//         else
//             do_vtk_plots(world, int(r_params.plot_pts()), r_params.L() / 2.0, 0, int(r_params.num_orbitals()), molecule, square(world, ground_orbitals), "ground");
//     }
// }
...
ResponseBase::ResponseBase(World &world, const CalcParams &params) :
    r_params(params.response_parameters), molecule(params.molecule), ground_calc(params.ground_calculation), ground_orbitals(ground_calc.orbitals()), ground_energies(ground_calc.get_energies()),
...
auto ResponseBase::compute_theta_X(World &world, const X_space &chi, const vector_real_function_3d &rho1, const XCOperator<double, 3> &xc, const std::string &calc_type) const -> X_space
{
...
auto ResponseBase::compute_gamma_static(World &world, const gamma_orbitals &gammaOrbitals, const XCOperator<double, 3> &xc) const -> X_space
{
    ...
    if (r_params.print_level() >= 1)
    {
        molresponse::end_timer(world, "gamma_clear_functions", "gamma_clear_functions", iter_timing);
    }
    // Done
    return gamma;
    // Get sizes
...
auto ResponseBase::compute_response_potentials(World &world, const X_space &chi, XCOperator<double, 3> &xc, const std::string &calc_type) const -> std::tuple<X_space, X_space, X_space>
{
    // compute
    bool compute_Y = calc_type == "full";
...
auto ResponseBase::update_residual(World &world, const X_space &chi, const X_space &g_chi, const std::string &calc_type, const Tensor<double> &old_residuals, const X_space &xres_old) -> residuals
{
    if (r_params.print_level() >= 1)
    {
        molresponse::start_timer(world);
...
void ResponseBase::function_data_to_json(json &j_mol_in, size_t iter, const Tensor<double> &x_norms, const Tensor<double> &x_abs_norms, const Tensor<double> &rho_norms,
...
auto solid_harmonics(World &world, int n) -> std::map<std::vector<int>, real_function_3d>
{
    ...
    for (int l = 0; l < n; l++)
    {
        ...
        result[std::vector<int>{l + 1, l + 1}] =
            sqrt(pow(2, kronecker(l, 0) * (2 * l) / (2 * l + 1))) * (x * result[std::vector<int>{l, l}] - (1 - kronecker(l, 0) * y * result[std::vector<int>{l, -l}]));
        result[std::vector<int>{l + 1, -l - 1}] =
            sqrt(pow(2, kronecker(l, 0) * (2 * l) / (2 * l + 1))) * (y * result[std::vector<int>{l, l}] + (1 - kronecker(l, 0) * x * result[std::vector<int>{l, -l}]));
        ...
        for (int m = -l; m < l + 1; m++)
        {
            ...
            result[std::vector<int>{l + 1, m}] = 1.0 / std::sqrt((l + m + 1) * (l - m + 1)) *
...
void response_timing::add_data(std::map<std::string, std::pair<double, double>> values)
{
    //   print("ADDING DATA");
    iter++;
    std::for_each(wall_time_data.begin(), wall_time_data.end(),
                  [&values](auto &v)
                  {
    ...
    std::for_each(cpu_time_data.begin(), cpu_time_data.end(),
                  [&values](auto &v)
                  {
                      // print(v.first, " : ", values[v.first]);
...
void response_data::add_data(std::map<std::string, Tensor<double>> values)
{
    //   print("ADDING DATA");
    iter++;
    std::for_each(function_data.begin(), function_data.end(),
                  [&values](auto &v)
                  {
