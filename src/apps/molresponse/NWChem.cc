...
void NWChem_Interface::read(Properties::Properties props) {
  ...
  //    specified in props (post step 1), but not already read (in
  //    my_properties).
  //    ... bitwise and props with (not my_properties) -- has to be requested
  //    and not read.

  // basis requires atoms
  if ((props & Properties::Basis).any()) props |= Properties::Atoms;

  // if we're doing the MO expansion coefficients, we might as well do the
  // energies
  if ((props & Properties::MOs).any()) props |= Properties::Energies;

  // skip things we've already read
  props &= ~my_properties;

  // do we need to open and read fname.out?
  if ((props & Properties::Atoms).any() || (props & Properties::Basis).any()) {
    // open the NWChem log file: fname.out
    std::ifstream in(fname + ".out");
    if (!in)
      throw std::invalid_argument("Cannot open " + fname + ".out for reading.");

    if ((props & Properties::Atoms).any()) read_atoms(in);
    if ((props & Properties::Basis).any()) read_basis_set(in);

    in.close();
  }

  // do we need to open and read fname.movecs?
  if ((props & Properties::Energies).any() || (props & Properties::MOs).any() ||
...
void NWChem_Interface::read_atoms(std::istream &in) {
  ...
  bool reading =
      false;  // are we in the block of text where we can read basis functions?
  double unitcf =
      1.;  // Conversion factor to go from units in the NWChem file to a.u.

  // regex search strings
  std::regex startline{R"(Geometry)"},
      unitline{
          R"(Output coordinates in (a.u.|angstroms|nanometer|picometer|.+) \(scale)"},
      atomline{
          R"([[:digit:]]+[[:space:]]+([[:alpha:]]+)[[:space:]]+[^[:space:]]+[[:space:]]+([^[:space:]]+)[[:space:]]+([^[:space:]]+)[[:space:]]+([^[:space:]]+))"},
      doneline{R"(Atomic Mass)"};
  std::smatch matches;

  // note that NWChem outputs the orbitals by atom type, listing ang. momentum,
  ...
  while (std::getline(in, line)) {
    ...
    // in what units are the atomic positions?
      ...
      if (matches[1] == "angstroms")
        ...
        err.get() << "Unknown units: " << matches[1] << ". Assuming angstroms."
                  << std::endl;
        unitcf = 1.0 / 0.529177;
        doprint = false;
      }
    ...
    // does this line have an atomic position?
      ...
      addme.position = {{std::stod(matches[2]) * unitcf,
                         std::stod(matches[3]) * unitcf,
                         std::stod(matches[4]) * unitcf}};
      my_atoms.emplace_back(std::move(addme));
      err.get() << matches[1] << " at (" << addme.position[0] << ", "
                << addme.position[1] << ", " << addme.position[2] << ")."
                << std::endl;
    }
...
static std::ostream &operator<<(std::ostream &out, const BasisShell &bs) {
  ...
  for (auto ex : bs.exps) out << ex << ' ';
  out << "], coeffs = [ ";
  for (auto coeff : bs.coeffs) out << coeff << ' ';
  out << "]";
  return out;
}
...
void NWChem_Interface::read_basis_set(std::istream &in) {
  ...
  bool spherical =
      true;  // are we using spherical orbitals (true) or Cartesian (false)?
  std::string curatom;  // Symbol of the current atom being read.
  unsigned curnum =
      1;  // The current basis function number (for the symbol) being read.
  BasisShell curfunc;  // The current basis function being read.
  std::map<std::string, std::list<BasisShell>>
      basisperatom;  // List of basis functions for the given symbol (map key).
  std::list<BasisShell>
      basiscuratom;  // List of basis functions for the current atom.

  // regex search strings
  std::regex startline{R"(Basis.*(cartesian|spherical))"},
      newatomline{R"(([[:alpha:]]+)[[:space:]]+\(([[:alpha:]]+)\))"},
      orbitalline{
          R"(([[:digit:]]+)[[:space:]]+([[:alpha:]]+)[[:space:]]+([^[:space:]]+)[[:space:]]+([^[:space:]]+))"},
      lineardepline{R"( !! The overlap matrix has *([\d]+))"},
      almostdoneline{R"(Summary)"}, doneline{R"(Superposition)"};
  ...
  while (std::getline(in, line)) {
    ...
    if (!reading && std::regex_search(line, matches, startline)) {
      ...
      err.get() << "Using " << (spherical ? "spherical" : "Cartesian")
                << " orbitals." << std::endl;
    }
    ...
    // Is this the start of a new atom?
      ...
      // if this isn't the first symbol, store the previous symbol's basis and
      // reset
      if (curatom.length() > 0) {
        err.get() << "   Function " << curnum << ": " << curfunc << std::endl;
      ...
      curatom = matches[1];
      curnum = 1;
      err.get() << "Reading basis for " << curatom << " (" << matches[2] << ")."
                << std::endl;
    }

    // Is this the line containing linear dependencies?
    else if (reading_lindeps &&
             std::regex_search(line, matches, lineardepline)) {
      err.get() << "Found " << matches[1]
                << " linear dependencies in the basis." << std::endl;
      line = matches[1].str();
      ...
      if (func == curnum) {
        ...
        if (curfunc.exps.size() == 0)
          ...
          throw std::runtime_error(
              "Inconsistent orbital types while reading NWChem basis set.");

        curfunc.exps.push_back(std::stod(matches[3]));
        curfunc.coeffs.push_back(std::stod(matches[4]));
      } else
        throw std::runtime_error(
            "Inconsistent basis function counting while reading NWChem basis "
            "set.");
    }
  }
  ...
  for (const Atom &atom : atoms) {
    ...
    for (const BasisShell &bs : basisperatom[atom.symbol]) {
      if (bs.type == 'S') {
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::s, atom.position, bs.exps, bs.coeffs));
        ++nfunc;
      } else if (bs.type == 'P') {
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::px, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::py, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::pz, atom.position, bs.exps, bs.coeffs));
        nfunc += 3;
      } else if (bs.type == 'D' && !spherical) {
        throw std::runtime_error(
            "Cartesian D shell requested: These are not tested or verified.");

        // THESE ROUTINES HAVE NOT BEEN TESTED
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::dxx, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::dxy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::dxz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::dyy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::dyz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::dzz, atom.position, bs.exps, bs.coeffs));
        nfunc += 6;
      } else if (bs.type == 'D' && spherical) {
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::dxy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::dyz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::dzzmrr, atom.position, bs.exps, bs.coeffs));

        std::unique_ptr<GaussianFunction> dxz(new GaussianFunction(
            GaussianType::dxz, atom.position, bs.exps, bs.coeffs));
        dxz->operator*=(-1.);
        gaussians.emplace_back(std::move(dxz));

        gaussians.emplace_back(new GaussianFunction(
            GaussianType::dxxmyy, atom.position, bs.exps, bs.coeffs));
        nfunc += 5;
      } else if (bs.type == 'F' && !spherical) {
        throw std::runtime_error(
            "Cartesian F shell requested: These are not tested or verified.");

        // THESE ROUTINES HAVE NOT BEEN TESTED
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fxxx, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fxxy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fxxz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fxyy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fxyz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fxzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fyyy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fyyz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fyzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fzzz, atom.position, bs.exps, bs.coeffs));
        nfunc += 10;
      } else if (bs.type == 'F' && spherical) {
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fxxymyyy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fxyz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fyzzmrry, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fzzzmrrz, atom.position, bs.exps, bs.coeffs));

        std::unique_ptr<GaussianFunction> fxzzmrrx(new GaussianFunction(
            GaussianType::fxzzmrrx, atom.position, bs.exps, bs.coeffs));
        fxzzmrrx->operator*=(-1.);
        gaussians.emplace_back(std::move(fxzzmrrx));

        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fxxzmyyz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::fxyymxxx, atom.position, bs.exps, bs.coeffs));
        nfunc += 7;
      } else if (bs.type == 'G' && !spherical) {
        throw std::runtime_error(
            "Cartesian G shell requested: These are not tested or verified.");

        // THESE FUNCTIONS HAVE NOT BEEN TESTED OR VERIFIED!!!
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gxxxx, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gxxxy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gxxxz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gxxyy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gxxyz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gxxzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gxyyy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gxyyz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gxyzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gxzzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gyyyy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gyyyz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gyyzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gyzzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gzzzz, atom.position, bs.exps, bs.coeffs));
        nfunc += 15;
      } else if (bs.type == 'G' && spherical) {
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gxydx2my2, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gyzdx2my2, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gxydz2mr2, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gyzdz2mr2, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gzero, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gxzdz2mr2, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gx2my2dz2mr2, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gxzdx2my2, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::gx4mx2y2py4, atom.position, bs.exps, bs.coeffs));
        nfunc += 9;
      } else if (bs.type == 'H' && !spherical) {
        throw std::runtime_error(
            "Cartesian H shell requested: These are not tested or verified.");

        // THESE FUNCTIONS HAVE NOT BEEN TESTED OR VERIFIED!!!
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxxxxx, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxxxxy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxxxxz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxxxyy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxxxyz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxxxzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxxyyy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxxyyz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxxyzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxxzzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxyyyy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxyyyz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxyyzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxyzzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hxzzzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hyyyyy, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hyyyyz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hyyyzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hyyzzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hyzzzz, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hzzzzz, atom.position, bs.exps, bs.coeffs));
        nfunc += 21;
      } else if (bs.type == 'H' && spherical) {
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hm5, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hm4, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hm3, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hm2, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hm1, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hzero, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hp1, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hp2, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hp3, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hp4, atom.position, bs.exps, bs.coeffs));
        gaussians.emplace_back(new GaussianFunction(
            GaussianType::hp5, atom.position, bs.exps, bs.coeffs));
        nfunc += 11;
      } else
        throw std::runtime_error(
            std::string("Unknown or unimplemented shell type: ") + bs.type);
    }
...
static T read_endian(std::istream &in, const bool swap) {
  ...
  if (swap) {
    ...
    for (unsigned j = 0; j < bytes; ++j) in.read(ptr + (bytes - j - 1), 1);
  } else
    // use the data as-is
    in.read(ptr, bytes);
...
void NWChem_Interface::read_movecs(const Properties::Properties props,
  ...
  if (num[0] != 142 && num[0] != 110) {
    ...
    err.get() << "Unable to read header. Trying opposite endian-ness."
              << std::endl;
    ...
    if (num[0] != 142 && num[0] != 110) {
      ...
      err.get() << "Using opposite endian-ness for the remainder of this read."
                << std::endl;
  }
  ...
  if (num[0] == 4) {
    ...
    err.get() << "Unknown or unimplemented integer type for the number of sets "
                 "in header."
              << std::endl;
    throw errmess;
  }
  ...
  if (nsets > 2) {
    err.get()
        << "The read_MOs basis has only been tested for movecs files that have "
           "a one or two sets.\n"
        << "The results from this function need to be verified before use ("
        << nsets
        << " sets).\n"
           "Only the last two sets will be kept."
        << std::endl;
  }
  ...
  if (num[0] == 4) {
    ...
    err.get()
        << "Unknown or unimplemented integer type for basis set size in header."
        << std::endl;
    throw errmess;
  }
  num[1] = read_endian<int32_t>(in, swap_endian);
  ...
  if ((properties & Properties::Basis) != Properties::None &&
      nbasis != basis_set.size()) {
    err.get() << "The number of basis functions in read_MOs does not match the "
                 "number of "
              << "basis functions read\nfrom the NWChem log file. Use the "
                 "following results with caution."
              << std::endl;
  }
  ...
  //  }
  //  else {
  //    err.get() << "Unknown or unimplemented integer type for nmo[" << j << "]
  //    in header." << std::endl; throw errmess;
  //  }
  //  num[1] = read_endian<int32_t>(in, swap_endian);
  //  if(num[0] != num[1]) {
  ...
  for (unsigned set = 0; set < nsets; ++set) {
    ...
    if (num[0] != static_cast<int>(8 * nmo[set])) {
      err.get() << "Unexpected number of occupancies for set " << set << '.'
                << std::endl
                << "Read " << num[0] << " but expected "
                << static_cast<int>(8 * nmo[set]) << std::endl;
      throw errmess;
    }
    if (do_occupancies)
    ...
    num[1] = read_endian<int32_t>(in, swap_endian);
    if (num[0] != num[1]) {
      err.get() << "Error reading occupancies for set " << set << '.'
                << std::endl
    ...
    for (unsigned mo = 0; mo < nmo[set] - my_lineardeps; ++mo) {
      ...
      if (num[0] != static_cast<int>(8 * nbasis)) {
        err.get() << "Unexpected number of coefficients in MO " << mo
                  << " of set " << set << '.' << std::endl
                  << "Read " << nmo[0] << " but expected "
  ...
  // at the very end of the movecs file is the total energy and nuclear
