...
X_space TDDFT::compute_gamma_full(World& world, X_space& Chi, const XCOperator<double, 3>& xc) {
...
X_space TDDFT::compute_gamma_static(World& world, X_space& Chi, XCOperator<double, 3> xc) {
    ...
    // Create Coulomb potential on ground_orbitals
    if (xcf.hf_exchange_coefficient() != 1.0) {
        auto compute_wx = [& ](auto rho_alpha) {
            auto xc_rho = xc.apply_xc_kernel(rho_alpha);
            return mul(world, xc_rho, phi0_copy);
        };
        molresponse::start_timer(world);
        // for every transition density apply the exchange kernel and multiply the
        // vector of orbitals
        std::transform(rho_omega.begin(), rho_omega.end(), W.X.begin(), compute_wx);
        W.Y = W.X.copy();
...
X_space TDDFT::compute_V0X(World& world, X_space& Chi, XCOperator<double, 3> xc, bool compute_Y) {
    ...
    vecfuncT phi0_copy = copy(world, ground_orbitals);
    Chi_copy.truncate();
    truncate(world, phi0_copy);
    // v_nuc first
