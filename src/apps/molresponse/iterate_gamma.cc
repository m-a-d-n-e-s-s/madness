...
GammaResponseFunctions TDDFT::ComputeGammaFunctions(World& world,
                                                    response_space& x,
                                                    response_space& y,
  ...
  for (size_t k = 0; k < n; k++) {
    ...
    phi_phi[k] = apply(world, op, mul(world, ground_orbitals[k], ground_orbitals));
  }
  phi_phi.truncate_rf();
  ...
  if (compute_Y) {
    rho_omega = transition_density(world, ground_orbitals, x, x);
  }
  ...
  for (size_t b = 0; b < m; b++) {
    for (size_t k = 0; k < n; k++) {
      ...
      y_phi[b][k] = apply(world, op, mul(world, ground_orbitals[k], y[b]));
      if (compute_Y) {
        x_phi[b][k] = apply(world, op, mul(world, ground_orbitals[k], x[b]));
      }
    }
  }
  for (size_t b = 0; b < m; b++) {
  ...
  for (size_t b = 0; b < m; b++) {
    ...
    for (size_t k = 0; k < n; k++) {
      ...
      Ky[b][k] = dot(world, y_phi[b][k], ground_orbitals);

      if (compute_Y) {
        Ky_conjugate[b][k] = dot(world, y[b], phi_phi[k]);
        Kx_conjugate[b][k] = dot(world, x_phi[b][k], ground_orbitals);
      }
    }
  }
  ...
  if (r_params.print_level() >= 2) {
    ...
    if (compute_Y) {
      ...
      PrintResponseVectorNorms(world, Kx_conjugate + Ky_conjugate, "Kx_conjugate+Ky_conjugate");
    }
  }
