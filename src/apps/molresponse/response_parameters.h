...
struct ResponseParameters : public QCCalculationParametersBase {
  ...
  ResponseParameters() {
    initialize<std::string>(
        "archive", "restartdata", "file to read ground parameters from");
    initialize<bool>(
        "nwchem", false, "Using nwchem files for intelligent starting guess");
    initialize<std::string>(
        "nwchem_dir",
        "",
    ...
    initialize<bool>(
        "tda", false, "turn on Tam-Danchof approximation (excitations energy");
    initialize<bool>(
        "plot",
        false,
        "turn on plotting of final orbitals. Output format is .vts");
    initialize<bool>(
        "plot_range", false, "controls which orbitals will be plotted");
    initialize<std::vector<int>>(
        "plot_data", std::vector<int>{0}, "Orbitals to plot");
    ...
    initialize<bool>("plot_all_orbitals",
                     false,
    ...
    initialize<double>(
        "dconv", 1.e-4, "recommended values: 1.e-4 < dconv < 1.e-8");
    initialize<bool>(
        "conv_only_dens",
        false,
        "if true remove bsh_residual from convergence criteria (deprecated)");
    initialize<bool>(
        "dconv_set", false, "Convergence flage for the orbtial density");
    ...
    initialize<std::string>(
        "restart_file", "", "file to read ground parameters from");
    // kain
    initialize<bool>(
    ...
    initialize<std::string>(
        "save_file", "", "File name to save orbitals for restart");
    initialize<bool>(
        "save_density", false, "Flag to save density at each iteration");
    initialize<int>("vnucextra", 2, "load balance parameter for nuclear pot");
    initialize<int>("loadbalparts", 2, "??");
    initialize<std::string>(
        "save_density_file", "", "File name to save density for restart");
    initialize<bool>("load_density", false, "Flag to load density for restart");
    initialize<std::string>(
        "load_density_file", "", "File name to load density for restart");
    initialize<size_t>(
        "guess_max_iter", 5, "maximum number of guess iterations");
    // properties
    initialize<std::string>(
        "response_type", "excited_state", "dipole,nuclear,order2,order3");
    initialize<std::string>("calc_type", "full", "full,static,tda");
    initialize<bool>(
        "excited_state", false, "Flag to turn on excited state calc");
    initialize<bool>(
        "first_order", false, "Flag to turn on first order response calc");
    initialize<bool>(
        "second_order", false, "Flag to turn on second order response calc ");
    initialize<bool>(
        "third_order", false, "Flag to turn on second order response calc ");
    initialize<bool>(
        "dipole", false, "Flag to turn on frequency dependent property calc");
    initialize<bool>(
        "nuclear", false, "Flag to turn on frequency dependent property calc");
    initialize<bool>(
        "order2", false, "Flag to turn on frequency dependent property calc");
    initialize<bool>(
        "order3", false, "Flag to turn on frequency dependent property calc");
 ...
 public:
  ...
  std::vector<int> plot_data() const {
    return get<std::vector<int>>("plot_data");
  }
  std::vector<double> plot_cell() const {
    return get<std::vector<double>>("plot_cell");
  }
  ...
  std::string save_density_file() const {
    return get<std::string>("save_density_file");
  }
  bool load_density() const { return get<bool>("load_density"); }
  std::string load_density_file() const {
    return get<std::string>("load_density_file");
  }
  ...
  std::string response_type() const {
    return get<std::string>("response_type");
  }
