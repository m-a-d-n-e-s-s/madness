...
void FrequencyResponse::iterate(World &world) {
    ...
    const double dconv = std::max(FunctionDefaults<3>::get_thresh() * 100, r_params.dconv());//.01 .0001 .1e-5
    const double relative_max_target = 15 * FunctionDefaults<3>::get_thresh();
    ...
    for (iter = 0; iter < r_params.maxiter(); ++iter) {
        ...
        if (iter > 0) {
            ...
            if ((d_residual < dconv * std::max(size_t(10), molecule.natom())) and
