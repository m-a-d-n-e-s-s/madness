                  ...
                  std::string name)
{
    ...
    // Write an .xyz file with current geometry (to deal with molecular reorientations that might occur)
    FILE *f = 0;
    b = "geometry.xyz";
    f = fopen(b.c_str(), "w");
    ...
    {
                std::fprintf(f, "%5s   %16.12f %16.12f %16.12f\n", atomic_number_to_symbol(molecule.get_atom_number(i)).c_str(), coords[i][0], coords[i][1], coords[i][2]);
    }
