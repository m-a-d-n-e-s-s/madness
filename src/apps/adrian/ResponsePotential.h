...
class ResponsePotential {
   ...
   private:
      ...
      std::vector<real_function_3d> perturbed_density(ResponseFunction& x, ResponseFunction& y) {
         // Get sizes
         int m = x.size();
         int n = x[0].size();
      ...
      void coulomb_terms(ResponseFunction& x,
          ...
          gammaK = ResponseFunction(world, x.r_states, x.g_states);
          if(is_dft) {
      ...
      void exchange_terms(ResponseFunction& x,
          ...
          gammaJ = ResponseFunction(world, x.size(), x[0].size());
          groundK = ResponseFunction(world, x.size(), x[0].size());
          x.compress_rf();
          gammaJ.compress_rf();
          groundK.compress_rf();
