                                    ...
                                    std::string xy)
{
   ...
   ResponseFunction deriv_J = create_coulomb_derivative(world, f, g, orbitals, small, thresh);
   ...
   for (int k = 0; k < m; k++)
   {
      ...
      for (int p = 0; p < n; p++)
      {
         if (ground(p) + omega(k) > 0)
         {
            ...
            {
               if (world.rank() == 0)
                  printf("   Shift needed for transition from ground orbital %d to response %s state %d\n", p, xy.c_str(), k);
               if (world.rank() == 0)
...
void TDHF::select_active_subspace(World &world)
{
   ...
   if (Rparams.print_level >= 0)
   {
      ...
      if (world.rank() == 0)
         print("   Selecting ground state subspace to excite from for components.");
      if (world.rank() == 0)
         print("   This is all orbitals between", Rparams.range_low, "and", Rparams.range_high, "\n");
   ...
   for (int i = 0; i < s_vals.dim(0); i++)
   {
      if (s_vals(i) < 10 * thresh_degenerate)
      {
         if (world.rank() == 0 and num_sv == 0)
            print("");
         if (world.rank() == 0)
            printf("   Detected singular value (%.8f) below threshold (%.8f). Reducing subspace size.\n",
                                   ...
                                   std::string xc)
{
   ...
   std::vector<real_function_3d> vsq = square(world, Gparams.orbitals); // we square each orbital
   ...
   XCOperator xcop(world, xc, false, rho, rho); //world,which xc, spin_polarized? ,spinup, spindown
...
void TDHF::iterate(World &world)
{
   ...
   // Here we create the kain solver for response functions...TDHF_allocator returns a Response function of correct size
   // With correct number of occ and virtual orbitals all set to zero.

   // Setting max sub size for KAIN solver
   if (Rparams.kain)
      kain.set_maxsub(Rparams.maxsub);
   // Not sure what max subspace size means....I'm guessing how many previous points I would like to use and keep track of

   // Set y things if not doing TDA
   if (not Rparams.tda)
      old_y_response = ResponseFunction(world, m, n); //If not doing TDA create a zero y function
   ...
   while (iteration < Rparams.max_iter && !all_converged)
   {
      ...
      if (not Rparams.tda)
         for (int i = 0; i < m; i++)
            y_response[i] = projector(y_response[i]);
      ...
      {
         if (world.rank() == 0)
            print("   Norms of off-diagonal hamiltonian correction for x components:");
      ...
      if (not Rparams.tda)
      {
         ...
         {
            if (world.rank() == 0)
               print("   Norms of off-diagonal hamiltonian correction for y components:");
            print_norms(world, temp);
         }
      }
      ...
      if (not relax)
      {
         ...
         if (num_conv == m)
         {
            ...
            relax = true;
            if (world.rank() == 0)
               print("   All components converged. Unfreezing all states for final relaxation.");
...
void TDHF::solve(World &world)
{
   ...
   if (Rparams.plot_initial)
   {
      ...
      if (Rparams.plot_L > 0.0)
         do_vtk_plots(world, Rparams.plot_pts, Rparams.plot_L, 0, Gparams.num_orbitals, Gparams.molecule, square(world, Gparams.orbitals), "ground");
      else
         do_vtk_plots(world, Rparams.plot_pts, Gparams.L / 2.0, 0, Gparams.num_orbitals, Gparams.molecule, square(world, Gparams.orbitals), "ground");
   }
   ...
   if (Rparams.plot)
   {
      ...
      if (Rparams.plot_L > 0.0)
         do_vtk_plots(world, Rparams.plot_pts, Rparams.plot_L, 0, Rparams.plot_data.size(), Gparams.molecule, plot_densities, "response-state");
      else
         do_vtk_plots(world, Rparams.plot_pts, Gparams.L, 0, Rparams.plot_data.size(), Gparams.molecule, plot_densities, "response-state");
   }
                                  ...
                                  ResponseFunction &dipoles)
{
   ...
   while (iteration < Rparams.max_iter and !converged)
   {
      ...
      // Get norms
      for (int i = 0; i < m; i++)
         x_norms[i] = sqrt(inner(x_response[i], x_response[i]) - inner(y_response[i], y_response[i]));
      ...
      if (Rparams.omega != 0.0)
         B_x = create_B(world, x_response, y_response, Gparams.orbitals, Rparams.small, FunctionDefaults<3>::get_thresh(), Rparams.print_level);
