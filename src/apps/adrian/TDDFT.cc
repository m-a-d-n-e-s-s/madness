...
ResponseFunction TDHF::createAf(World &world, ResponseFunction &Vf,
                                ResponseFunction &F0_f,
                                ResponseFunction &Epsilonf,
...
ResponseFunction TDHF::ComputeHf(World &world, const ResponseFunction &f,
...
ResponseFunction TDHF::ComputeGf(World &world, const ResponseFunction &f,
                                 const std::vector<real_function_3d> &orbitals,
                                 double small, double thresh, int print_level,
                                 std::string xy) {
  // Start a timer
...
ResponseFunction TDHF::CreatePotential(World &world, const ResponseFunction &f,
...
Tensor<double> TDHF::expectation(World &world, const ResponseFunction &a,
...
Zfunctions TDHF::ComputeZFunctions(World &world, const ResponseFunction &x,
                                   const ResponseFunction &y, XCOperator xc,
                                   Tensor<double> x_shifts,
  ...
  ResponseFunction v0_x =
      CreatePotential(world, x, xc, Rparams.print_level, "x");
  v0_x = apply_shift(world, x_shifts, v0_x, x);
  ResponseFunction x_f_no_diag = scale_2d(world, x, ham_no_diagonal);
  ResponseFunction H_x =
      ComputeHf(world, x, Gparams.orbitals, Rparams.small,
                FunctionDefaults<3>::get_thresh(), Rparams.print_level, "x");
  ResponseFunction G_y =
      ComputeGf(world, y, Gparams.orbitals, Rparams.small,
                FunctionDefaults<3>::get_thresh(), Rparams.print_level, "y");
  Z.Z_x = v0_x + x_f_no_diag - H_x - G_y;
  // y functions
  if (Rparams.omega != 0.0) {
    ResponseFunction v0_y =
        CreatePotential(world, y, xc, Rparams.print_level, "y");
    v0_y = apply_shift(world, y_shifts, v0_y, y);
    ResponseFunction y_f_no_diag = scale_2d(world, y, ham_no_diagonal);
    ResponseFunction G_x =
        ComputeGf(world, x, Gparams.orbitals, Rparams.small,
                  FunctionDefaults<3>::get_thresh(), Rparams.print_level, "x");
    ResponseFunction H_y =
        ComputeHf(world, y, Gparams.orbitals, Rparams.small,
                  FunctionDefaults<3>::get_thresh(), Rparams.print_level, "y");

    Z.Z_y = v0_y + y_f_no_diag - H_y - G_x;

    if (Rparams.print_level >= 2) {
      Tensor<double> t = expectation(world, x, v0_x);
      if (world.rank() == 0) {
  ...
  Tensor<double> TDHF::createFullResponseMatrix(
    ...
    ResponseFunction A_x = createAf(world, I.Vx, I.F0_x, I.EpsilonX, I.Hx, x,
                                    ground_orbitals, print_level, "x");
    ResponseFunction A_y = createAf(world, I.Vy, I.F0_y, I.EpsilonY, I.Hy, y,
                                    ground_orbitals, print_level, "y");

    // Create the B pieces
    ResponseFunction B_x = createBf(world, I.Gx, ground_orbitals, print_level);
    ResponseFunction B_y = createBf(world, I.Gy, ground_orbitals, print_level);
  ...
  Tensor<double> TDHF::create_shift_target(
      World & world, Tensor<double> & ground, Tensor<double> & omega,
      double target, int print_level, std::string xy) {
    // Start a timer
    if (print_level >= 1) start_timer(world);
  ...
  // Not currently used.
  Tensor<double> TDHF::calculate_energy_update(
      World & world, ResponseFunction & rhs, ResponseFunction & f_residuals,
      ResponseFunction & new_f, int print_level, std::string xy) {
    /*
     *  The correction is:
  ...
  ResponseFunction TDHF::select_functions(World & world, ResponseFunction & f,
    ...
    if (print_level >= 1) {
      if (world.rank() == 0)
        print("\n   Selecting the", k,
              "lowest excitation energy components.\n");
    }
  ...
  Tensor<double> TDHF::get_fock_transformation(
      World & world, Tensor<double> & overlap, Tensor<double> & fock,
      Tensor<double> & evals, const double thresh_degenerate) {
  ...
  Tensor<double> TDHF::diagonalizeFockMatrix(
      World & world, Tensor<double> & fock, ResponseFunction & psi,
      ElectronResponseFunctions & I, Tensor<double> & evals,
  ...
  void TDHF::augment(World & world, Tensor<double> & S_x, Tensor<double> & A_x,
    ...
    ResponseFunction TempCurrentAf =
        Current.Hx + Current.F0_x - Current.EpsilonX;
    ResponseFunction TempLastAf = Last.Hx + Last.F0_x - Last.EpsilonX;
  ...
  void TDHF::augment_full(
      World & world, Tensor<double> & S, Tensor<double> & A,
      ResponseFunction & B_x, ResponseFunction & x_gamma,
  ...
  void TDHF::unaugment(World & world, int m, int iter, Tensor<double> &omega,
                       ...
                       ElectronResponseFunctions &Current,
                       ElectronResponseFunctions &Last,
                       ResponseFunction &x_response, Tensor<double> &old_S,
  ...
  Tensor<double> TDHF::diagonalizeFullResponseMatrix(
      World & world, Tensor<double> & S, Tensor<double> & A,
      ResponseFunction & x, ResponseFunction & y, ElectronResponseFunctions & I,
  ...
  void TDHF::deflateTDA(World & world, Tensor<double> & S, Tensor<double> old_S,
    ...
    if (iteration < Rparams.larger_subspace and iteration > 0) {
      ...
      augment(world, S, XAX, ElectronResponses, OldElectronResponses,
              x_response, old_S, old_A, old_x_response, Rparams.print_level);
    }
  ...
  void TDHF::deflateFull(
      World & world, Tensor<double> & S, Tensor<double> old_S,
      Tensor<double> old_A, ResponseFunction & x_response, ResponseFunction & y,
      ResponseFunction & old_x_response, ResponseFunction & old_y_response,
      ElectronResponseFunctions & ElectronResponses,
      ElectronResponseFunctions & OldElectronResponses, Tensor<double> & omega,
      int &iteration, int &m) {
    S = expectation(world, x_response, x_response) -
        expectation(world, y_response, y_response);
  ...
  // parameters
  XCOperator TDHF::create_xcoperator(
      World & world, std::vector<real_function_3d> orbitals, std::string xc) {
    ...
    while (iteration < Rparams.max_iter && !all_converged) {
      ...
      if (Rparams.tda) {
        deflateTDA(world, S, old_S, old_A, x_response, old_x_response,
                   ElectronResponses, OldElectronResponses, omega, iteration,
                   m);
        // Constructing S
        // Full TDHF
      } else {
  ...
  void TDHF::IterateGuess(World & world, ResponseFunction & guesses) {
    ...
    while (iteration < Rparams.guess_max_iter) {
      ...
      if (iteration + 1 < Rparams.guess_max_iter) {
        //  Calculates shifts needed for potential / energies
        //  If none needed, the zero tensor is returned
        shifts = create_shift(world, Gparams.energies, omega,
                              Rparams.print_level, "x");
  ...
  ResponseFunction TDHF::diagonalize_CIS_guess(
    ...
    for (int i = 0; i < n; i++) {
      ...
      const std::vector<real_function_3d> igv =
          apply(world, op, virtuals * brai);
      const std::vector<real_function_3d> igm =
          apply(world, op, orbitals * brai);
      ...
      for (int a = 0; a < m; a++) {
        ...
        for (int j = 0; j < n; j++) {
          ...
          for (int b = 0; b < m; b++) {
            ...
            MCIS(I, J) = diag_element +
                         2.0 * inner(braj * virtuals[b], igv[a]) -
  ...
  Tensor<double> TDHF::CreateGroundHamiltonian(
      World & world, std::vector<real_function_3d> f, int print_level) {
    // Basic output
    if (print_level >= 1) start_timer(world);
    // Get sizes
    int m = f.size();
    ...
    real_convolution_3d op = CoulombOperator(world, Rparams.small,
                                             FunctionDefaults<3>::get_thresh());
  ...
  ResponseFunction TDHF::create_nwchem_guess(World & world, int m) {
    ...
    if (n < m) {
      ...
      ResponseFunction rand =
          create_random_guess(world, m - n, Gparams.num_orbitals,
  ...
  void TDHF::create_all_potentials(
      World & world, ResponseFunction & x, ResponseFunction & x_gamma,
      ResponseFunction & x_V0, ResponsePotential & potentials,
      int print_level) {
    // Intermediaries
    ...
    for (unsigned int proto = 0; proto < Rparams.protocol_data.size();
      ...
      if (proto == 0) {
        if (Rparams.restart) {
          ...
          if (Rparams.random) {
            // Random guess
            x_response = create_random_guess(
                world, 2 * Rparams.states, Gparams.num_orbitals,
                Gparams.orbitals, Gparams.molecule);
            ...
            x_response =
                create_trial_functions(world, 2 * Rparams.states,
                                       Gparams.orbitals, Rparams.print_level);
          }
          ...
          if (world.size() > 1) {
            ...
            for (int j = 0; j < Rparams.states; j++) {
              for (unsigned int k = 0; k < Gparams.num_orbitals; k++) {
                lb.add_tree(x_response[j][k], lbcost<double, 3>(1.0, 8.0),
                            true);
              }
            }
            for (unsigned int j = 0; j < Gparams.num_orbitals; j++) {
              lb.add_tree(Gparams.orbitals[j], lbcost<double, 3>(1.0, 8.0),
                          true);
            }
            FunctionDefaults<3>::redistribute(world, lb.load_balance(2));
          ...
          x_response = select_functions(world, x_response, omega,
                                        Rparams.states, Rparams.print_level);
  ...
  void TDHF::IteratePolarizability(World & world, ResponseFunction & dipoles) {
    ...
    ResponseFunction
        B_x;  // Holds the off diagonal perturbed piece of y equation
    ResponseFunction
        B_y;  // Holds the off diagonal perturbed piece of x equation
  ...
  void TDHF::IterateFrequencyResponse(World & world, ResponseFunction & rhs_x,
    ...
    ResponseFunction z_x;  // Holds the perturbed two electron piece
    ResponseFunction z_y;  // Holds the perturbed two electron piece
    ...
    while (iteration < Rparams.max_iter and !converged) {
      ...
      x_response.scale(rec_norms);
      y_response.scale(rec_norms);
      // here we compute Z_x and Z_y

      ComputeZFunctions(world, x_response, y_response, xc, Gparams, Rparams,
                        ham_no_diag);
      // Load balance
      // Only balancing on x-components. Smart?
      // Only balance on first two iterations or every 5th iteration
  ...
  void PrintPolarizabilityAnalysis(
      World & world, const Tensor<double> polar_tensor, const double omega) {
