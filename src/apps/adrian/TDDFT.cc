...
void TDHF::normalize(World &world, ResponseFunction &f) {
  // Run over rows
...
// (Each state's norm should be 1, not the
// individual functions norms)
void TDHF::normalize(World &world, ResponseFunction &f, ResponseFunction &g) {
  // Run over rows
  for (unsigned int i = 0; i < f.size(); i++) {
    // Get the normalization constant
    // (Sum included inside inner)
...
ResponseFunction TDHF::CreateXCDerivativeRF(World &world, ResponseFunction &f,
                                            std::vector<real_function_3d> &phi,
                                            double small, double thresh) {
  // Get sizes
  int m = f.size();
...
ResponseFunction TDHF::CreateAf(World &world, ResponseFunction &Hf,
...
ResponseFunction TDHF::CreateGf(World &world, ResponseFunction &f,
  ...
  ResponseFunction G(world, m, n);

  // Initialize function
  ResponseFunction Jdagger(world, m, n);
  ResponseFunction Kdagger(world, m, n);
...
ResponseFunction TDHF::exchange(World &world, ResponseFunction &f) {
  // Get sizes
  int m = f.size();
  int n = f[0].size();
...
Tensor<double> TDHF::expectation(World &world, ResponseFunction &a,
                                 ResponseFunction &b) {
  // Get sizes
  int dim_1 = a.size();
  int dim_2 = a[0].size();
...
Tensor<double>
TDHF::create_response_matrix(World &world, ResponseFunction &Hf,
  ...
  ResponseFunction temp =
      CreateAf(world, Hf, Vf, f, fe, ground_orbitals, print_level, xy);
...
Tensor<double> TDHF::CreateFullResponseMatrix(
    World &world,
    ResponseFunction &Hx,   // Hx perturbed two electron piece
    ResponseFunction &Hy,   // Hy perturbed two electron piece
    ResponseFunction &Gx,   // Gx perturbed two electron piece
    ResponseFunction &Gy,   // Gy perturbed two electron piece
  ...
  ResponseFunction A_x =
      CreateAf(world, Hx, Vx, x, x_fe, ground_orbitals, print_level, "x");
  ResponseFunction A_y =
      CreateAf(world, Hy, Vy, y, y_fe, ground_orbitals, print_level, "y");

  // Create the B pieces
  ResponseFunction B_x = CreateBf(world, Gx, ground_orbitals, print_level);
  ResponseFunction B_y = CreateBf(world, Gx, ground_orbitals, print_level);
...
// functions
ResponseFunction TDHF::gram_schmidt(World &world, ResponseFunction &f) {
  // Sizes inferred
  int m = f.size();
...
// Specialized for response calculations that returns orthonormalized
// functions
void TDHF::gram_schmidt(World &world, ResponseFunction &f,
                        ResponseFunction &g) {
  // Sizes inferred
  int m = f.size();
...
double TDHF::calculate_max_residual(World &world, ResponseFunction &f) {
  // Container for max
  double max = 0.0;
...
void TDHF::select_active_subspace(World &world) {
  ...
  if (Rparams.print_level >= 0) {
    ...
    if (world.rank() == 0)
      print(
          "   Selecting ground state subspace to excite from for components.");
    if (world.rank() == 0)
...
// energy
ResponseFunction TDHF::select_functions(World &world, ResponseFunction &f,
...
Tensor<double>
TDHF::diag_fock_matrix(World &world, Tensor<double> &fock,
  ...
  Hpsi = transform(world, Hpsi, U);
  fe = transform(world, fe, U);
  fe2 = transform(world, fe2, U);
...
ResponseFunction TDHF::transform(World &world, ResponseFunction &f,
                                 Tensor<double> &U) {
  // Return container
...
// If using a larger subspace to diagonalize in, this will put everything in the
// right spot
void TDHF::augment(
    World &world, Tensor<double> &S_x, Tensor<double> &A_x,
    ...
    Tensor<double> &old_S, Tensor<double> &old_A, ResponseFunction &old_x_gamma,
    ResponseFunction &old_x_response, ResponseFunction &old_V_x_response,
    ResponseFunction &old_x_fe, int print_level) {
  // Basic output
  if (print_level >= 1)
    start_timer(world);
...
void TDHF::augment_full(
    ...
    ResponseFunction &old_B_y, ResponseFunction &old_y_gamma,
...
void TDHF::unaugment(
    World &world, int m, int iter, Tensor<double> &omega, Tensor<double> &S_x,
    Tensor<double> &A_x, ResponseFunction &x_gamma,
...
Tensor<double> TDHF::DiagonalizeFullResponseMatrix(
  ...
  Hx = transform(world, Hx, vecs);
  Hy = transform(world, Hy, vecs);
  Gx = transform(world, Gx, vecs);
  Gy = transform(world, Gy, vecs);
  ...
  x_fe2 = transform(world, x_fe2, vecs);
  y_fe2 = transform(world, y_fe2, vecs);
...
// response functions
void TDHF::sort(World &world, Tensor<double> &vals, ResponseFunction &f) {
  // Get relevant sizes
  int k = vals.size();
...
void TDHF::Iterate(World &world) {
  ...
  ResponseFunction x_fe(world, m, n);
  ResponseFunction x_fe2(world, m, n);
  // response components
  ResponseFunction y_fe(world, m, n);
  ResponseFunction y_fe2(world, m, n);
  ;
  // response components
  ResponseFunction V_x_response(world, m,
                                n); // Holds V^0 applied to response functions
  ResponseFunction V_y_response(world, m,
                                n); // Holds V^0 applied to response functions
  ResponseFunction Hx(world, m, n);
  ResponseFunction Hy(world, m, n);
  ResponseFunction Gx(world, m, n);
  ResponseFunction Gy(world, m, n);
  ResponseFunction B_x(
      world, m, n); // Holds the off diagonal perturbed piece of y equation
  ResponseFunction B_y(
      world, m, n); // Holds the off diagonal perturbed piece of x equation
  ResponseFunction shifted_V_x_response(world, m,
                                        n); // Holds the shifted V^0 applied to
  // response functions
  ResponseFunction shifted_V_y_response(world, m,
                                        n); // Holds the shifted V^0 applied to
  // response functions
  ...
  ResponseFunction old_V_x_response(world, m, n);
  ResponseFunction old_x_fe(world, m, n);
  ResponseFunction old_B_x(world, m, n);
  ResponseFunction old_y_gamma(world, m, n);
  ResponseFunction old_V_y_response(world, m, n);
  ResponseFunction old_y_fe(world, m, n);
  ResponseFunction old_B_y(world, m, n);
  Tensor<double> old_A;
  Tensor<double> old_S;
  ...
  // Here we create the kain solver for response functions...TDHF_allocator
  // returns a Response function of correct size With correct number of occ
  // and virtual orbitals all set to zero.

  // Setting max sub size for KAIN solver
  if (Rparams.kain)
    kain.set_maxsub(Rparams.maxsub);
  // Not sure what max subspace size means....I'm guessing how many previous
  // points I would like to use and keep track of

  // Set y things if not doing TDA
  if (not Rparams.tda)
    old_y_response = ResponseFunction(
        world, m, n); // If not doing TDA create a zero y function
  ...
  while (iteration < Rparams.max_iter && !all_converged) {
    ...
    if (Rparams.tda)
      normalize(world, x_response); // Gotta checkout out what normalize does
    else
      normalize(world, x_response, y_response);

    // This is where we will diverge...I should create a function that
    // computes all the electronic pieces of the calculation.  Keeping in a
    // struct or a seperate class would be pretty neat Constructor takes in
    // response functions x and y and computes
    //
    // Hx Gy
    // Gx Hy
    //
    // Call it the Electronic Interaction Class
    ...
    ResponseFunction x_fe = scale_2d(
        world, x_response, hamiltonian); // epsilon with diag for FullR matrix
    ResponseFunction x_fe2 =
        scale_2d(world, x_response, ham_no_diag); // for rhs
    ...
    if (not Rparams.tda) { // not sure why this is the condition
      ResponseFunction y_fe = scale_2d(world, x_response, hamiltonian);
      ResponseFunction y_fe2 = scale_2d(world, y_response, ham_no_diag);
      Hy =
    ...
    if (Rparams.tda) {
      ...
      Tensor<double> A_x = create_response_matrix(
          world, Hx, V_x_response, x_fe, x_response, Gparams.orbitals,
          hamiltonian, Rparams.print_level, "x");
      ...
      if (iteration < Rparams.larger_subspace and iteration > 0) {
        augment(world, S, A_x, x_gamma, x_response, V_x_response, x_fe, old_S,
                old_A, old_x_gamma, old_x_response, old_V_x_response, old_x_fe,
                Rparams.print_level);
      }
      ...
      omega.clear();
      diag_fock_matrix(world, A_x, x_response, V_x_response, Hx, x_fe, x_fe2,
                       omega, S, FunctionDefaults<3>::get_thresh());

      // If larger subspace, need to "un-augment" everything
      if (iteration < Rparams.larger_subspace) {
        unaugment(world, m, iteration, omega, S, A_x, x_gamma, x_response,
                  V_x_response, x_fe, old_S, old_A, old_x_gamma, old_x_response,
                  old_V_x_response, old_x_fe, Rparams.print_level);
      }
    }
    // Full TDHF
    else {
      ...
      Tensor<double> A = CreateFullResponseMatrix(
          world, Hx, Hy, Gx, Gy, V_x_response, V_y_response, x_response,
          y_response, x_fe, y_fe, Gparams.orbitals, hamiltonian, Rparams.small,
      ...
      Tensor<double> U = DiagonalizeFullResponseMatrix(
          world, S, A, Hx, Hy, Gx, Gy, V_x_response, V_y_response, x_response,
          y_response, x_fe, y_fe, x_fe2, y_fe2, omega,
          FunctionDefaults<3>::get_thresh(), Rparams.print_level);
    ...
    shifted_V_x_response =
        apply_shift(world, x_shifts, V_x_response, x_response);
    if (not Rparams.tda) {
      y_shifts = -y_shifts;
      shifted_V_y_response =
          apply_shift(world, y_shifts, V_y_response, y_response);
    }
    ...
    rhs_x = shifted_V_x_response + rhs_x - x_fe2;
    if (not Rparams.tda) {
      rhs_y = shifted_V_x_response + rhs_x - x_fe2;
    }
    ...
    if (not relax) {
      ...
      if (num_conv == m) {
        ...
        relax = true;
        if (world.rank() == 0)
          print("   All components converged. Unfreezing all states for final "
                "relaxation.");
    ...
    //   FunctionDefaults<3>::get_k(), i, iteration-1); plot_line(plotname,
    //   5001, lo, hi, x_response[i][0]);
    //}
    //
...
void TDHF::analysis(World &world) {
  ...
  // Per response function, want to print the contributions from each ground
  // state So print the norm of each function?
...
void TDHF::IterateGuess(World &world, ResponseFunction &guesses) {
  ...
  while (iteration < Rparams.guess_max_iter) {
    ...
    // Create gamma
    gamma = CreateGamma(world, guesses, zeros, Gparams.orbitals, Rparams.small,
                        FunctionDefaults<3>::get_thresh(), Rparams.print_level,
                        "x");

    /*    gamma =
            CreateHf(world, guesses, Gparams.orbitals, Rparams.small,
                     FunctionDefaults<3>::get_thresh(), Rparams.print_level,
       "x");
                     */
    // Create \hat{V}^0 applied to response functions
    ...
    ResponseFunction g_fe = scale_2d(world, guesses, hamiltonian);
    ResponseFunction g_fe2 = scale_2d(world, guesses, ham_no_diag);

    // Constructing response matrix
    Tensor<double> A =
        create_response_matrix(world, gamma, V, g_fe, guesses, Gparams.orbitals,
                               hamiltonian, Rparams.print_level, "x");

    diag_fock_matrix(world, A, guesses, V, gamma, g_fe, g_fe2, omega, S,
                     FunctionDefaults<3>::get_thresh());
    ...
    if (omega.dim(0) != m) {
      if (world.rank() == 0)
        print("\n   Adding", m - omega.dim(0),
              "eigenvalue(s) (counters subspace size reduction in "
              "diagonalizatoin).");
...
ResponseFunction TDHF::diagonalize_CIS_guess(
  ...
  int I = -1; // combined index from i and a, start is -1 so that initial value
  // is 0
  int J = -1; // combined index from j and b, start is -1 so that initial value
  // is 0
...
TDHF::transition_density(World &world, std::vector<real_function_3d> &orbitals,
  ...
  for (int i = 0; i < m; i++) {
    ...
    for (int j = 0; j < n; j++) {
      // y functions are zero if TDA is active
      densities[i] =
          densities[i] + orbitals[j] * x[i][j] + orbitals[j] * y[i][j];
    }
  }
...
void TDHF::set_protocol(World &world, double thresh) {
  ...
  // GaussianConvolution1DCache<double>::map.clear();//(TODO:Adrian-What is
  // this? Do i need it?)
...
ResponseFunction TDHF::create_random_guess(
  ...
  // Create and apply a centered gaussian on each atom so that the randomness
  // is localized around the atoms
...
ResponseFunction TDHF::create_nwchem_guess(World &world, int m) {
  // Basic output
  if (world.rank() == 0)
  ...
  // Create as many vectors of functions as we can from these nwchem virtual
  // orbitals putting 1 virtual orbital from nwchem per vector
  ...
  if (n < m) {
    ...
    if (world.rank() == 0)
      print("\n   Only", n,
            "guess functions were provided by augmenting NWChem functions.\n   "
            "Augmenting with random functions.");

    // Create the random guess
    ResponseFunction rand = create_random_guess(
...
void TDHF::create_all_potentials(World &world, ResponseFunction &x,
  ...
  for (unsigned int proto = 0; proto < Rparams.protocol_data.size(); proto++) {
    ...
    if (proto == 0) {
      if (Rparams.restart) {
        ...
        // (Always creating (at least) twice the amount requested for initial
        // diagonalization)
        if (world.rank() == 0)
        ...
        y_response =
            ResponseFunction(world, Rparams.states, Gparams.num_orbitals);
      }
    }
