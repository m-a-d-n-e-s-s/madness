...
struct ResponseParameters {
  ...
  // Default constructor
  ResponseParameters()
      : archive(""), nwchem(""), states(1), print_level(1), tda(false),
        plot(false), plot_range(false), plot_data(std::vector<int>{0}),
        plot_L(-1.0), plot_pts(201), max_iter(20), dconv(0), dconv_set(false),
        small(1e-6), protocol_data(madness::vector_factory(1e-6)),
        larger_subspace(0), k(0), random(false), store_potential(true),
        e_window(false), range_low(0.0), range_high(1.0), plot_initial(false),
        restart(false), restart_file(""), kain(false), maxsub(5), xc("hf"),
        save(false), guess_max_iter(5), property(false), polarizability(false),
        omega(0.0), old(true) {}
