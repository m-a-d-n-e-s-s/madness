  ...
  else
  {
    ...
    try
    {
      ...
      else if (user_workflow == "oep")
      {
        ...
        if (std::find(convergence_crit.begin(), convergence_crit.end(), "each_energy") ==
            convergence_crit.end())
        {
      ...
      {
        std::string msg = "The optimize workflow is currently disabled. Please use the dft + "
