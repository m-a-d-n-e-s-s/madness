    ...
    if (parser.key_exists("help")) {
        ...
        try {
            ...
            if (user_workflow == "scf") {
                ...
                wf.addDriver(std::make_unique<qcapp::SinglePointDriver>(reference));
                wf.addDriver(
                    std::make_unique<qcapp::SinglePointDriver>(std::make_unique<CC2Application>(world, pm, ref_calc)));
                ...
                auto ref_calc = reference->calc();
                wf.addDriver(
                    std::make_unique<qcapp::SinglePointDriver>(std::make_unique<OEPApplication>(world, pm, ref_calc)));
            } else if (user_workflow == "optimize") {
                std::string msg =
                        "The optimize workflow is currently disabled. Please use the dft + gopt() application instead.\n";
