...
double CC2::solve_MP2_alternative(Pairs<CC_Pair> &pairs)const{
...
double CC2::solve_uncoupled_mp2(Pairs<CC_Pair> &pairs)const{
	...
	for(auto& utmp:pairs.allpairs){
		...
		real_convolution_6d G = BSHOperator<6>(world, sqrt(-2.0 * CCOPS.get_epsilon(pair.i,pair.j)),
...
double CC2::solve_mp2(Pairs<CC_Pair> &doubles, CC_vecfunction &singles){
...
//	std::cout << "\n\n\n\nDifference between CC2 residues is " << diff << "\n\nTimings were (Wall), (CPU):\n" << time1 << " and " << time2  << std::endl;
	...
	if(thresh<FunctionDefaults<6>::get_thresh()){
		...
		FunctionDefaults<6>::set_thresh(parameters.thresh_Ue);
	}
	const real_function_6d coulomb_part = CCOPS.make_cc2_coulomb_parts(singles(pair.i),singles(pair.j),singles);
...
bool CC2::iterate_cc2_doubles(Pairs<CC_Pair> &doubles, const CC_vecfunction &singles)const{
	...
	for(size_t i=parameters.freeze;i<mo.size();i++){
		for(size_t j=i;j<mo.size();j++){
			...
			std::cout << "Iteration of pair |u" << i << j << "> completed, current error is:" << error << ", current correlation energy is " << std::setprecision(parameters.output_prec) << omega << std::endl;
...
static double gauss_2s(const coord_3d &r){

}
void CC2::decompose_constant_part(){
	...
	// make guess basis
	vecfuncT basis1;vecfuncT basis2;
	...
	for(size_t iter=0;iter<100;iter++){
		...
		for(const auto &b:basis1) basis1_const_part.push_back(const_part.project_out((b*nemo.nuclear_correlation->square()),0));
		vecfuncT basis2_const_part;
		for(const auto &b:basis2) basis2_const_part.push_back(const_part.project_out((b*nemo.nuclear_correlation->square()),0));
