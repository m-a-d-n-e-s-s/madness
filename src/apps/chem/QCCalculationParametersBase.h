...
template <typename T>
static typename std::enable_if<std::is_floating_point<T>::value, void>::type check_for_inf(const std::string& str,
                                                                                           T& arg) {
  std::string sinf;
  std::stringstream ss;
...
struct QCParameter {
 public:
  QCParameter(){};
  // ctor value, type, "comment", allowed values
  QCParameter(const std::string v,
              const std::string t,
  ...
  void serialize(Archive& ar) {
    ar& value& default_value& derived_value& user_defined_value& type& null& comment& allowed_values& print_order&
        precedence;
  }
 ...
 private:
  void set_all() {
    // sets the value to (derived, defined,or default)
    value = default_value;
...
class QCCalculationParametersBase {
 ...
 protected:
  ...
  void initialize(const std::string& key,
    ...
    //
    // action: inserts QCParameter pair
    //
    // <key,QCParameter(value,type,comment,allowed_value))
    //
    // value and and allowd values are strings
    //
    ...
    // for each allowed value in allowed_values
    ...
    parameters.insert(std::make_pair<std::string, QCParameter>(std::string(key_lower),
 ...
 public:
  ...
  template <typename T>
  static typename std::enable_if<std::is_floating_point<T>::value, void>::type overwrite_if_inf(std::string& str,
                                                                                                const T& arg) {
    if (std::isinf(arg)) {
      std::stringstream ss;
      ss << std::numeric_limits<T>::infinity();
  ...
  static typename std::enable_if<!std::is_same<T, bool>::value, T>::type fromstring(const std::string& arg) {
    ...
    if (type_conversion_failed) {
      std::string errmsg =
          "error in type conversion for argument >> " + arg + " << to type " + std::type_index(typeid(T)).name();
  ...
  template <typename T>
  static typename std::enable_if<!std::is_same<T, std::string>::value, T>::type read_quotes(
