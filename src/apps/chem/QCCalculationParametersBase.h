...
namespace madness {
    ...
    template<typename T>
    static typename std::enable_if<std::is_floating_point<T>::value, void>::type check_for_inf(
            const std::string& str, T& arg) {
        std::string sinf;
        std::stringstream ss;
    ...
    public:
        ...
        void serialize(Archive& ar) {
            ar& value& default_value& derived_value& user_defined_value& type& null& comment&
                    allowed_values& print_order& precedence;
        }
    ...
    protected:
        ...
        bool try_setting_user_defined_value(const std::string& key, const std::string& val) {
            ...
            if (print_debug)
                ::madness::print("key:", key, "will set type", std::type_index(typeid(T)).name());
