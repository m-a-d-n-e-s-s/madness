...
static double radius2(const coord_3d &r){
	return (r[0]*r[0]+r[1]*r[1]+r[2]*r[2]);
}

static double gauss_1(const coord_3d &r){
	double exponent = radius2(r);
	return exp(-exponent);
}

static double gauss_2(const coord_3d &r){
	double exponent = radius2(r);
	return r[0]*exp(-exponent);
}

static double gauss_3(const coord_3d &r){
	double exponent = radius2(r);
	return r[1]*exp(-2.0*exponent);
}
...
static double testf12(const coord_6d &r){
	double exponent = r12(r);
...
vecfuncT CC_Operators::S2b(const Pairs<CC_Pair> u, const CC_Singles & singles ) const {
	...
	//{
		...
		//
		//		u00.print_size("U00 before dirac convolution");
		//		real_function_3d delta_result = u00.dirac_convolution<3>();
		//		u00.print_size("U00 after dirac convolution");
		//		delta_result.print_size("result of dirac convolution");
		//
		...
		//
		//
		//		real_function_3d testg1 = real_factory_3d(world).f(gauss_1);
		//		double norm1=testg1.norm2();
		//		testg1.scale(1.0/norm1);
		//
		//		real_function_3d testg2 = real_factory_3d(world).f(gauss_2);
		//		double norm2=testg1.norm2();
		//		testg1.scale(1.0/norm2);
		//
		//		real_function_3d testg3 = real_factory_3d(world).f(gauss_3);
		//		double norm3=testg1.norm2();
		//		testg1.scale(1.0/norm3);
		//
		//		real_function_6d ug12= CompositeFactory<double,6,3>(world).particle1(copy(testg1)).particle2(copy(testg2));
		//		ug12.fill_tree().truncate().reduce_rank();
		//		real_function_3d deltag12 = ug12.dirac_convolution<3>();
		//		real_function_3d control12 = (testg1*testg2).truncate();
		//
		//		double norm12 = deltag12.norm2();
		//		double cnorm12 = control12.norm2();
		//		if(world.rank()==0) std::cout << "|| <delta|g1g2> ||=" << norm12 << std::endl;
		//		if(world.rank()==0) std::cout << "|| |g1>*|g2> ||=" << cnorm12 << std::endl;
		//
		//		real_function_6d ug23= CompositeFactory<double,6,3>(world).particle1(copy(testg2)).particle2(copy(testg3));
		//		ug23.fill_tree().truncate().reduce_rank();
		//		real_function_3d deltag23 = ug23.dirac_convolution<3>();
		//		real_function_3d control23 = (testg2*testg3).truncate();
		//
		//		double norm23 = deltag23.norm2();
		//		double cnorm23 = control23.norm2();
		//		if(world.rank()==0) std::cout << "|| <delta|g2g3> ||=" << norm23<< std::endl;
		//		if(world.rank()==0) std::cout << "|| |g21>*|g3> ||=" << cnorm23<< std::endl;
		//
		//		if(fabs(norm23-cnorm23)<FunctionDefaults<3>::get_thresh() and fabs(norm12-cnorm12)<FunctionDefaults<3>::get_thresh()){
		//			if(world.rank()==0) std::cout << "Delta Projection seems to be fine\n";
		//		}else if(world.rank()==0) warning("error in delta projection");
		//
		//		if(world.rank()==0)std::cout << "\n\nEnd S2b Debug section\n";
		//
		//
		//		real_function_6d test6d = CompositeFactory<double,6,3>(world).particle1(copy(mo_ket_.front())).particle2(copy(mo_ket_.front()));
		//		test6d.fill_tree().truncate().reduce_rank();
		//
		//		real_function_6d ktest6d = multiply(copy(test6d),copy(mo_bra_.front()),2);
		//		poisson -> particle() = 2;
		//		real_function_6d gktest6d = (*poisson)(ktest6d);
		//		real_function_3d test3d = gktest6d.dirac_convolution<3>();
		//
		//		double dtest6d = test3d.inner(mo_bra_.front());
		//		double dtest3d = make_ijgu(0,0,test6d);
		//
		//		if(world.rank()==0) std::cout << "Dirac convoltion test: " << "\nValue1=" << dtest6d << " Value2=" << dtest3d << std::endl;
		//
		//		if(world.rank()==0) std::cout << "\n\n  Testing Poisson Pointer\n";
		//		real_function_3d kgk = (*poisson)(mo_bra_.front()*mo_ket_.front());
		//		real_function_3d kgkk = kgk * mo_ket_.front();
		//		double control = mo_bra_.front().inner((intermediates_.get_exchange_intermediate()[0][0])*mo_ket_.front());
		//		double kkgkk = kgkk.inner(mo_bra_.front());
		//		if(world.rank()==0) std::cout << "Poisson test: " << "\nValue1=" << kkgkk << " Value2=" << dtest3d << " Value3=" << control << std::endl;
		//
		//		{
		//		real_function_6d f12 = CompositeFactory<double, 6, 3>(world).g12(corrfac.f()).particle1((mo_ket_.front())).particle2(copy(mo_ket_.front()));
		//		f12.fill_tree().truncate().reduce_rank();
		//		real_function_3d  deltaf12 = f12.dirac_convolution<3>();
		//		deltaf12.print_size("(int delta f12*|00> d2) ... default thresh");
		//		}
		//		{
		//			real_function_6d f12 = CompositeFactory<double, 6, 3>(world).g12(corrfac.f()).particle1((mo_ket_.front())).particle2(copy(mo_ket_.front()));
		//			f12.set_thresh(0.1*FunctionDefaults<6>::get_thresh());
		//			f12.fill_tree().truncate().reduce_rank();
		//			real_function_3d  deltaf12 = f12.dirac_convolution<3>();
		//			deltaf12.print_size("(int delta f12*|00> d2) ... thresh*0.1");
		//		}
		//		real_function_6d fr12 = real_factory_6d(world).f(r12);
		//		real_function_3d deltar12 = fr12.dirac_convolution<3>();
		//		deltar12.print_size("int delta r12 d2");
		//
		//		real_function_6d r1200 = CompositeFactory<double,6,3>(world).particle1((mo_ket_.front())).particle2(copy(mo_ket_.front())).ket(copy(fr12));
		//		r1200.fill_tree().truncate().reduce_rank();
		//		real_function_3d deltar1200 = r1200.dirac_convolution<3>();
		//		deltar1200.print_size("int delta r1200");
		//
		//	}
		/// DEBUG END
	// DEBUG STUFF FOR s2c ... dont want to wait till s2c is ready
	{
		real_function_3d test = convolute_x_gQf_yz(mo_bra_.front(),mo_ket_.front(),mo_ket_.front());
	}
		...
		for(size_t i=0;i<singles.size();i++){
			...
			for(size_t k=0;k<singles.size();k++){
				{
					...
					S2b2.info();
					if(parameters.debug){
						CC_Timer s2b_debug_time(world,"s2b-debug-6D-part");
			...
			real_function_3d t1partx;
			{
				...
				if(parameters.debug){
					...
					if(fabs(debug-debug2)>FunctionDefaults<3>::get_thresh()) warning("S2b potential: Different result for t1 part " + stringify(debug) + " , " + stringify(debug2));
			...
			CC_Timer s2c_6d_timer(world,"s2c-6D-part");
			{
				for(size_t k=0;k<mo_bra_.size();k++){
					for(size_t l=0;l<mo_bra_.size();l++){
						...
						if(parameters.debug){
							CC_Timer s2c_6d_debug(world,"s2c-6D-debug");
							real_function_6d test = multiply(copy(u(k,l).function),copy(intermediates_.get_exchange_intermediate()[i][l]),2);
							real_function_3d test_tmp = test.project_out(mo_bra_[k],1);
			...
			CC_Timer s2c_3d_timer(world,"s2c_3D_part");
			{
				for(size_t k=0;k<mo_bra_.size();k++){
					for(size_t l=0;l<mo_bra_.size();l++){
						...
						if(parameters.debug){
							CC_Timer s2c_3d_debug(world,"s2c-3D-debug");
	...
	real_function_3d CC_Operators::convolute_x_gQf_yz(const real_function_3d &x, const real_function_3d &y, const real_function_3d &z)const{
		...
		if(parameters.debug){
			std::shared_ptr<real_convolution_3d> testf12op = std::shared_ptr
					< real_convolution_3d
					> (SlaterF12OperatorPtr(world, corrfac.gamma(),
							parameters.lo,
							parameters.thresh_poisson_3D));
			real_function_3d xz = (x*z).truncate();
			real_function_3d xfz = (*testf12op)(xz);
			xfz.print_size("xfz");
			real_function_3d xgz = (*poisson)(xz);
			xgz.print_size("poisson equiv: xgz");
		}
		...
		if(parameters.debug){
			...
			if(check1 > FunctionDefaults<3>::get_thresh()) warning("<k|Qf|xy> check1 failed");
			if(check4 > FunctionDefaults<3>::get_thresh()) warning("<k|Qf|xy> check4 failed");
			result.print_size("result");
			test4.print_size("check_result");
			part1.print_size("part1");
			test1.print_size("check_part1");
