...
class MolecularPotentialFunctor : public FunctionFunctorInterface<double, 3> {
 ...
 public:
  MolecularPotentialFunctor(const Molecule& molecule) : molecule(molecule) {}
...
class PotentialManager {
 ...
 public:
  ...
  const real_function_3d& vnuclear() { return vnuc; }

  vector_real_function_3d core_projection(World& world,
                                          const vector_real_function_3d& psi,
                                          const bool include_Bc = true) {
    int npsi = psi.size();
    if (npsi == 0) return psi;
    int natom = molecule.natom();
    vector_real_function_3d proj =
        zero_functions_compressed<double, 3>(world, npsi);
    ...
    for (int i = 0; i < natom; ++i) {
      ...
      for (unsigned int c = 0; c < nshell; ++c) {
        ...
        for (int m = 0; m < max_m; ++m) {
          real_function_3d core = real_factory_3d(world).functor(
              real_functor_3d(new CoreOrbitalFunctor(molecule, i, c, m)));
  ...
  double core_projector_derivative(World& world,
                                   ...
                                   const real_tensor& occ, int atom, int axis) {
    ...
    for (unsigned int c = 0; c < ncore; ++c) {
      ...
      int max_m = (l + 1) * (l + 2) / 2;
      for (int m = 0; m < max_m; ++m) {
        real_functor_3d func =
            real_functor_3d(new CoreOrbitalFunctor(molecule, atom, c, m));
        cores.push_back(real_function_3d(
            real_factory_3d(world).functor(func).truncate_on_project()));
        func = real_functor_3d(
            new CoreOrbitalDerivativeFunctor(molecule, atom, axis, c, m));
        dcores.push_back(real_function_3d(
            real_factory_3d(world).functor(func).truncate_on_project()));
  ...
  void apply_nonlocal_potential(World& world,
  ...
  void make_nuclear_potential(World& world) {
    ...
    vnuc =
        real_factory_3d(world)
            .functor(real_functor_3d(new MolecularPotentialFunctor(molecule)))
            .thresh(vtol)
            .truncate_on_project();
    ...
    vnuc.reconstruct();
    if (core_type != "") {
      real_function_3d c_pot =
          real_factory_3d(world)
              .functor(
                  real_functor_3d(new MolecularCorePotentialFunctor(molecule)))
              .thresh(vtol)
              .initial_level(4);
