...
DistributedMatrix<T> Kinetic<T, NDIM>::kinetic_energy_matrix(
    World& world, const vecfuncT& v) const {
  int n = v.size();
...
template <typename T, std::size_t NDIM>
DistributedMatrix<T> Kinetic<T, NDIM>::kinetic_energy_matrix(
    World& world, const vecfuncT& vbra, const vecfuncT& vket) const {
  int n = vbra.size();
  int m = vket.size();
...
template <typename T, std::size_t NDIM>
std::vector<Function<T, NDIM> > Laplacian<T, NDIM>::operator()(
...
Coulomb::Coulomb(World& world, const Nemo* nemo)
    : world(world), R_square(nemo->R_square) {
  reset_poisson_operator_ptr(nemo->get_calc()->param.lo(),
...
real_function_3d Coulomb::compute_density(const SCF* calc) const {
  real_function_3d density =
      calc->make_density(world, calc->get_aocc(), calc->get_amo());
  if (calc->is_spin_restricted()) {
    density.scale(2.0);
  } else {
    real_function_3d brho =
        calc->make_density(world, calc->get_bocc(), calc->get_bmo());
    density += brho;
  }
  density.truncate();
  return density;
...
real_function_3d Coulomb::compute_potential(const madness::Nemo* nemo) const {
  ...
  if (nemo->get_calc()->is_spin_restricted()) {
    ...
    real_function_3d brho = nemo->get_calc()->make_density(
        world, nemo->get_calc()->get_bocc(), nemo->get_calc()->get_bmo());
    density += brho;
  }
...
Nuclear::Nuclear(World& world, const SCF* calc) : world(world) {
  ncf = std::shared_ptr<NuclearCorrelationFactor>(
      new PseudoNuclearCorrelationFactor(world, calc->molecule,
                                         calc->potentialmanager, 1.0));
}
...
template <typename T, std::size_t NDIM>
std::vector<Function<T, NDIM> > Nuclear::operator()(
    const std::vector<Function<T, NDIM> >& vket) const {
...
DNuclear::DNuclear(World& world, const SCF* calc, const int iatom,
                   const int iaxis)
...
DNuclear::DNuclear(World& world, const Nemo* nemo, const int iatom,
                   const int iaxis)
    : world(world), iatom(iatom), iaxis(iaxis) {
  ncf = nemo->ncf;
}
...
vecfuncT DNuclear::operator()(const vecfuncT& vket) const {
  ...
  real_function_3d u2x_f =
      real_factory_3d(world).functor(u2x).thresh(vthresh).truncate_on_project();
  ...
  if (ncf->type() != NuclearCorrelationFactor::None) {
    ...
    // apply the derivative operator on each function for each dimension
    for (std::size_t i = 0; i < NDIM; ++i) {
      std::vector<Function<double, NDIM> > dv =
          apply(world, *(gradop[i]), vket, true);
      truncate(world, dv);
      ...
      real_function_3d U1 =
          real_factory_3d(world).functor(u1x).truncate_on_project();
    ...
    real_function_3d u3x_f =
        real_factory_3d(world).functor(u3x).truncate_on_project();
...
Exchange<T, NDIM>::Exchange(World& world, const SCF* calc, const int ispin)
  ...
  if (ispin == 0) {  // alpha spin
    mo_ket = convert<double, T, NDIM>(
        world, calc->amo);  // deep copy necessary if T==double_complex
    occ = calc->aocc;
...
Exchange<T, NDIM>::Exchange(World& world, const Nemo* nemo,
                            const int ispin)  // @suppress("Class members should
  ...
  poisson = std::shared_ptr<real_convolution_3d>(CoulombOperatorPtr(
      world, nemo->get_calc()->param.lo(), nemo->get_calc()->param.econv()));
}
...
std::vector<Function<T, NDIM> > Exchange<T, NDIM>::operator()(
  ...
  if (small_memory_) {  // Smaller memory algorithm ... possible 2x saving using
                        // i-j sym
    for (int i = 0; i < nocc; ++i) {
      if (occ[i] > 0.0) {
        vecfuncT psif =
            mul_sparse(world, mo_bra[i], vket, mul_tol);  /// was vtol
        truncate(world, psif);
...
template <typename T>
std::vector<Function<T, 3> > XCOperator::operator()(
    const std::vector<Function<T, 3> >& vket) const {
...
double XCOperator::compute_xc_energy() const {
  ...
  real_function_3d vlda =
      multiop_values<double, xc_functional, 3>(xc_functional(*xc), xc_args);
...
/// \f[
///        second_{local} = \frac{\partial^2 f_{xc}}{\partial
///        \rho_\alpha^2}\tilde \rho
///        + 2\frac{\partial^2 f_{xc}}{\partial
///        \rho_\alpha\sigma_{\alpha\alpha}}
///            \left(\vec\nabla \rho_a\cdot \vec \nabla\tilde\rho\right)
/// \f]
///  the second partial derivatives that need to be multiplied with the density
///  gradients
/// \f[
///      second_{semilocal} = -\vec\nabla\cdot\left((\vec\nabla\rho)
///             \left[2\frac{\partial^2
///             f_{xc}}{\partial\rho_\alpha\partial\sigma_{\alpha\alpha}}\tilde\rho
///             + 4\frac{\partial^2 f_{xc}}{\partial\sigma_{\alpha\alpha}^2}
///                \left(\vec\nabla\rho_\alpha\cdot\vec\nabla\tilde\rho\right)\right]\right)
/// \f]
/// and the first derivatives that need to be multiplied with the density
/// gradients \f[
///      first_{semilocal} =
///        -\vec\nabla\cdot\left(2\frac{\partial
///        f_{xc}}{\partial\sigma_{\alpha\alpha}}\vec\nabla\tilde\rho\right)
/// \f]
real_function_3d XCOperator::apply_xc_kernel(
    const real_function_3d& dens_pt, const vecfuncT grad_dens_pt) const {
  ...
  if (xc->is_gga()) {
    // turn intermediates into quantities that can be digested by the div
    // operator
    vecfuncT semilocal(3);
    semilocal[0] = intermediates[1];
    semilocal[1] = intermediates[2];
...
vecfuncT XCOperator::prep_xc_args(const real_function_3d& arho,
  ...
  if (have_beta)
    xcargs[XCfunctional::enum_rhob] = copy(brho.reconstruct());  // beta density
  world.gop.fence();
  ...
  if (xc->is_gga()) {
    ...
    vecfuncT grada;
    if (dft_deriv == "bspline")
      grada = grad_bspline_one(logdensa);  // b-spline
    else if (dft_deriv == "ble")
      grada = grad_ble_one(logdensa);  // BLE
    else
    ...
    if (have_beta) {
      ...
      vecfuncT gradb;
      if (dft_deriv == "bspline")
        gradb = grad_bspline_one(logdensa);  // b-spline
      else if (dft_deriv == "ble")
        gradb = grad_ble_one(logdensa);  // BLE
      else
        gradb = grad(logdensa);  // Default is abgv
...
void XCOperator::prep_xc_args_response(const real_function_3d& dens_pt,
  ...
  if (xc->is_gga()) {
    if (ddens_pt.size() == 0)
      ...
      print(" using provided ddens_pt in prep_xc_args_response");
    ...
    xc_args[XCfunctional::enum_sigma_pta_div_rho] =
        dot(world, zeta, ddens_pt);  // sigma_a
    // for RHF add factor 2 on rho; will be done in
    // xcfunctional_libxc::make_libxc_args \sigma_pt = 2 * rho_a *
    // sigma_pta_div_rho
    world.gop.fence();
    ...
    if (have_beta) {
      ...
      xc_args[XCfunctional::enum_sigma_ptb_div_rho] =
          dot(world, zeta, ddens_pt);  // sigma_b
    }
    world.gop.fence();
  }
  world.gop.fence();
...
Fock::Fock(World& world, const SCF* calc, double scale_K)
    : world(world),
      J(world, calc),
      K(world, calc, 0),
      T(world),
      V(world, calc),
      scale_K(scale_K) {}
Fock::Fock(World& world, const Nemo* nemo, double scale_K)
    : world(world),
      J(world, nemo),
      K(world, nemo, 0),
      T(world),
...
template std::vector<Function<double, 3> > XCOperator::operator()(
    const std::vector<Function<double, 3> >& vket) const;
template std::vector<Function<double_complex, 3> > XCOperator::operator()(
    const std::vector<Function<double_complex, 3> >& vket) const;

template class Exchange<double_complex, 3>;
template class Exchange<double, 3>;

template std::vector<Function<double, 3> > Nuclear::operator()(
    const std::vector<Function<double, 3> >& vket) const;
template std::vector<Function<double_complex, 3> > Nuclear::operator()(
    const std::vector<Function<double_complex, 3> >& vket) const;
