...
template <int NDIM> struct unaryexp {
...
SCF::SCF(World &world, const std::string &inputfile)
  ...
  xc.initialize(param.xc(), !param.spin_restricted(), world,
                param.print_level() > 1);
  // xc.plot();
...
void SCF::load_mos(World &world) {
  ...
  if (archive_version != version) {
    if (world.rank() == 0)
      print("Loading from a different version of archive. Archive version",
            archive_version, "MADNESS version", version);
...
distmatT SCF::localize_new(World &world, const vecfuncT &mo,
  ...
  default_random_generator.setstate(
      182041 +
      world.rank() * 10101); // To help with reproducibility for debugging, etc.
  ...
  if (world.rank() == 0) {
    ...
    auto QQ = [&at_to_bf, &at_nbf, &breaksym](const tensorT &C, int i, int j,
                                              int a) -> double {
    ...
    for (int iter = 0; iter < 1200; ++iter) {
      ...
      if (randomize && iter == 0) {
        rprev = true; // since did not use real gradient
               ...
               // actually now use f(0), df(0)/dx, df(mu)/dx for better accuracy
        double f0 = W;
        double f1 = newW;
        ...
        if (doprint)
          print(f0, f1, f0 - f1, f2p, "dxg", dxgrad, "hess", hess, "mu", mu,
                "mu2", mu2);
        mu = mu2;
      }
...
distmatT SCF::kinetic_energy_matrix(World &world, const vecfuncT &vbra,
  ...
  if (world.rank() == 0) {
    try {
      ...
      if (Saoamo.dim(0) != int(ao.size()) ||
        ...
        print(" AO alpha restart data size mismatch --- starting from atomic "
              "guess instead",
      ...
      if (!param.spin_restricted()) {
        ...
        if (Saobmo.dim(0) != int(ao.size()) ||
          ...
          print(" AO beta restart data size mismatch --- starting from atomic "
                "guess instead",
...
void SCF::initial_guess(World &world) {
  ...
  if (param.restart()) {
    ...
    functionT rho =
        factoryT(world)
            .functor(
                functorT(new MolecularGuessDensityFunctor(molecule, aobasis)))
            .truncate_on_project();
...
ek+ep);
    ...
    amo =
        transform(world, ao, c(_, Slice(ncore, ncore + param.nmo_alpha() - 1)),
                  vtol, true);
    truncate(world, amo);
    normalize(world, amo);
    ...
    if (param.nbeta() && !param.spin_restricted()) {
      bmo =
          transform(world, ao, c(_, Slice(ncore, ncore + param.nmo_beta() - 1)),
                    vtol, true);
      truncate(world, bmo);
      normalize(world, bmo);
...
void SCF::initial_load_bal(World &world) {
  ...
  lb.add_tree(vnuc, lbcost<double, 3>(param.vnucextra() * 1.0,
                                      param.vnucextra() * 8.0));
...
vecfuncT SCF::apply_potential(World &world, const tensorT &occ,
                              const vecfuncT &amo, const functionT &vlocal,
                              double &exc, double &enl, int ispin) {
  PROFILE_MEMBER_FUNC(SCF);
  ...
  if (xc.hf_exchange_coefficient()) {
    ...
    Exchange<double, 3> K =
        Exchange<double, 3>(world, this, ispin).small_memory(false).same(true);
    vecfuncT Kamo = K(amo);
  ...
  if (world.rank() == 0 and (param.print_level() > 1)) {
    ...
    for (size_t i = 0; i < molecule.natom(); ++i) {
      ...
      printf(" %5d %12.6f %12.6f %12.6f %12.6f %12.6f %12.6f\n", int(i), atom.x,
             atom.y, atom.z, r[i * 3 + 0], r[i * 3 + 1], r[i * 3 + 2]);
    }
  }
  return r;
}

void SCF::dipole_matrix_elements(World &world, const vecfuncT &mo,
                                 const tensorT &occ, const tensorT &energy,
                                 int spin) {
  START_TIMER(world);
  int nmo = mo.size();
  ...
  fprintf(f, "#initial | Energy (eV) | final  | Energy (eV) | Matrix el.  | "
             "Trans. E (eV)\n");
  fprintf(f, "%4i  %4i\n", nmo, nmo);
  fprintf(f, "%2i\n", 1);
  fprintf(f, "%13.8f\n", 0.0);
    ...
    for (int i = 0; i < nmo; ++i) {
      for (int j = 0; j < nmo; ++j) {
        fprintf(f, "%4i\t %13.8f\t %4i\t %13.8f\t %13.8f\t %13.8f\n", i + 1,
                energy(i) * ha2ev, j + 1, energy(j) * ha2ev, mat_el(axis, i, j),
...
vecfuncT SCF::compute_residual(World &world, tensorT &occ, tensorT &fock,
                               const vecfuncT &psi, vecfuncT &Vpsi,
                               double &err) {
...
void SCF::loadbal(World &world, functionT &arho, functionT &brho,
  ...
  lb.add_tree(
      vnuc, lbcost<double, 3>(param.vnucextra() * 1.0, param.vnucextra() * 8.0),
      false);
...
void SCF::rotate_subspace(World &world, const distmatT &dUT,
                          subspaceT &subspace, int lo, int nfunc,
...
void SCF::update_subspace(World &world, vecfuncT &Vpsia, vecfuncT &Vpsib,
                          tensorT &focka, tensorT &fockb, subspaceT &subspace,
                          tensorT &Q, double &bsh_residual,
...
void SCF::iterate_trotter(World &world, Convolution1D<double_complex> *G,
                          cvecfuncT &camo, cvecfuncT &cbmo, double t,
                          double time_step, double thresh) {
  PROFILE_MEMBER_FUNC(SCF);
  ...
  if (!param.spin_restricted() && param.nbeta()) {
    cvecfuncT cbmo3 = mul_sparse(world, expV, cbmo2,
                                 vtol); // Removed nofence --- must fence here
...
void SCF::solve(World &world) {
  ...
  const double tolloc = 1e-6; // was std::min(1e-6,0.01*dconv) but now trying to
...
printf("                total %32.24f\n\n", etot);*/
  ...
  if (world.rank() == 0 and (param.print_level() > 1)) {
    if (param.do_localize())
      print("Orbitals are localized - energies are diagonal Fock matrix "
            "elements\n");
    else
...
void SCF::update_response_subspace(World &world, vecfuncT &ax, vecfuncT &ay,
                                   vecfuncT &bx, vecfuncT &by, vecfuncT &rax,
                                   vecfuncT &ray, vecfuncT &rbx, vecfuncT &rby,
...
vecfuncT SCF::apply_potential_response(World &world, const vecfuncT &dmo,
                                       const XCOperator &xcop,
                                       const functionT &vlocal, int ispin) {
  functionT vloc = copy(vlocal);
  ...
  if (xc.hf_exchange_coefficient()) {
    ...
    vecfuncT Kdmo;
    Exchange<double, 3> K =
        Exchange<double, 3>(world, this, ispin).small_memory(false).same(false);
    if (ispin == 0)
      Kdmo = K(amo);
    if (ispin == 1)
      Kdmo = K(bmo);
...
void SCF::calc_freq(World &world, double &omega, tensorT &ak, tensorT &bk,
                    int sign) {
...
functionT SCF::make_density_ground(World &world, functionT &arho,
                                   functionT &brho) {
...
functionT SCF::make_derivative_density(World &world, const vecfuncT &mo,
                                       const tensorT &occ, const vecfuncT &x,
                                       const vecfuncT &y) {
...
functionT SCF::calc_exchange_function(World &world, const int &p,
                                      const vecfuncT &dmo1,
                                      const vecfuncT &dmo2, const vecfuncT &mo,
                                      int &spin) {
...
/// @param[in]  drho    the perturbed density
vecfuncT SCF::calc_djkmo(World &world, XCOperator &xc_alda,
                         const vecfuncT &dmo1, const vecfuncT &dmo2,
                         const functionT &drho, const vecfuncT &mo,
                         const functionT &drhos, int spin) {
...
vecfuncT SCF::calc_rhs(World &world, const vecfuncT &mo, const vecfuncT &Vdmo,
...
void SCF::dpolar(World &world, tensorT &polar, functionT &drho,
                 const int axis) {
  for (int i = 0; i < 3; ++i) {
    std::vector<int> f(3, 0);
    f[i] = true;
...
void SCF::calc_dpolar(World &world, const vecfuncT &ax, const vecfuncT &ay,
                      const vecfuncT &bx, const vecfuncT &by, const int axis,
                      tensorT &Dpolar_total, tensorT &Dpolar_alpha,
                      tensorT &Dpolar_beta) {
...
double SCF::residual_response(World &world, const vecfuncT &x,
                              const vecfuncT &y, const vecfuncT &x_old,
                              const vecfuncT &y_old, vecfuncT &rx,
                              vecfuncT &ry) {
  double residual = 0.0;
  ...
  for (size_t axis = 0; axis < param.response_axis().size(); axis++) {
    ...
    for (int iter = 0; iter < param.maxiter(); ++iter) {
      ...
      if (iter == 0) {
        ...
        djkamox = calc_djkmo(world, xc_alda, dipoleamo, dipoleamo, drho, amo,
                             drhoa, 0);
        axrhs = calc_rhs(world, amo, aVx, dipoleamo, djkamox);

        if (!param.spin_restricted() && param.nbeta() != 0) {
          bVx = apply_potential_response(world, dipolebmo, xcop, vlocal, 0);
          djkbmox = calc_djkmo(world, xc_alda, dipolebmo, dipolebmo, drho, bmo,
                               drhob, 0);
        ...
        if (omega != 0.0) {
          ...
          djkamoy = calc_djkmo(world, xc_alda, dipoleamo, dipoleamo, drho, amo,
                               drhoa, 0);
          ayrhs = calc_rhs(world, amo, aVy, dipoleamo, djkamoy);
          if (!param.spin_restricted() && param.nbeta() != 0) {
            bVy = apply_potential_response(world, dipolebmo, xcop, vlocal, 0);
            djkbmoy = calc_djkmo(world, xc_alda, dipolebmo, dipolebmo, drho,
                                 bmo, drhob, 0);
      ...
      if (iter > 0) {
        ...
        world.gop.fence();
        if (!param.spin_restricted() && param.nbeta() != 0) {
          bresidual = aresidual + residual_response(world, bx, by, bx_old,
                                                    by_old, rbx, rby);
        ...
        update_response_subspace(world, ax, ay, bx, by, rax, ray, rbx, rby,
                                 subspace, Q, update_residual);
    ...
    calc_dpolar(world, ax_old, ay_old, bx_old, by_old, axis, Dpolar_total,
                Dpolar_alpha, Dpolar_beta);
