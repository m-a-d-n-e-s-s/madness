...
/// apply the BSH operator on a vector of functions with corresponding
/// potentials
...
class BSHApply {
 ...
 public:
  BSHApply(World& world) : world(world), metric() {
    bshtol = FunctionDefaults<NDIM>::get_thresh();
    printme = printme and world.rank() == 0;
  }

  /// apply the BSH operator on the vector of functions and respective
  /// potentials

  /// @param[in]	eps		orbital energies or the square fock
  /// matrix
  /// @param[in]	Vpsi	vector of functions V*MOs
  /// @return		an MO structure holding the residuals and the orbital energy
  /// updates
  std::tuple<std::vector<Function<T, NDIM> >, Tensor<double> > operator()(
    ...
    // no coupling means F_{ij} =\eps_i \delta_{ij}, and the coupling term
    // vanishes
    Vpsi -= add_coupling_and_levelshift(psi, eps);

    std::vector<std::shared_ptr<SeparatedConvolution<double, NDIM> > > ops(
        psi.size());
    for (int i = 0; i < eps.dim(0); ++i) {
      T e_i = (eps.ndim() == 2) ? eps(i, i) : eps(i);
      ops[i] = std::shared_ptr<SeparatedConvolution<double, NDIM> >(
          BSHOperatorPtr<NDIM>(
              world, sqrt(-2.0 * eps_in_green(e_i)), lo, bshtol));
      ops[i]->destructive() = true;
    }

    const std::vector<Function<T, NDIM> > tmp = apply(world, ops, -2.0 * Vpsi);
    const std::vector<Function<T, NDIM> > res =
        truncate(psi - tmp, FunctionDefaults<NDIM>::get_thresh() * 0.1);
    ...
    for (size_t i = 0; i < psi.size(); ++i)
      delta_eps(i) = std::real(in(i)) / (norms[i] * norms[i]);
  ...
  /// @param[in]	arg		the MO structure holding MOs and orbital
  /// energies
  /// @param[in]	Vpsi	vector of functions V*MOs
  /// @return		an MO structure holding the residuals and the orbital energy
  /// updates
  MolecularOrbitals<T, NDIM> operator()(
      const MolecularOrbitals<T, NDIM>& arg,
  ...
  std::vector<Function<T, NDIM> > make_bra(
      const std::vector<Function<T, NDIM> >& rhs) const {
    if (metric.is_initialized()) return metric * rhs;
    return rhs;
  }

  /// limit the orbital energy (diagonal fock matrix element) entering the
  /// Green's function parameter mu
  ...
  std::vector<Function<T, NDIM> > add_coupling_and_levelshift(
      const std::vector<Function<T, NDIM> > psi,
      const Tensor<T> fock1) const {
    // check dimensions
    bool consistent = (psi.size() == fock1.dim(0));
    if ((fock1.ndim() == 2) and not(psi.size() == fock1.dim(1)))
      consistent = false;
    ...
    if (not consistent) {
      ...
      MADNESS_EXCEPTION("confused Fock matrix/orbital energies in BSHApply - 1",
                        1);
    }
    ...
    if (do_coupling) {
      ...
      if (fock1.ndim() == 1)
        for (int i = 0; i < fock1.dim(0); ++i)
          eps[i] = fock1(i) - eps_in_green(fock1(i));
      if (fock1.ndim() == 2)
        for (int i = 0; i < fock1.dim(0); ++i)
          eps[i] = fock1(i, i) - eps_in_green(fock1(i, i));
