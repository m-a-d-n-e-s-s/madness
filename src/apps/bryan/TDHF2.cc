...
// orbitals
ResponseFunction TDHF::create_coulomb_derivative(
    World& world, ResponseFunction& f, std::vector<real_function_3d>& orbitals,
...
ResponseFunction TDHF::create_exchange_derivative(
    World& world, ResponseFunction& f, std::vector<real_function_3d>& orbitals,
    double small, double thresh) {
  // Get sizes
  int m = f.size();
...
ResponseFunction TDHF::create_gamma(World& world, ResponseFunction& f,
                                    ResponseFunction& g,
                                    std::vector<real_function_3d>& orbitals,
                                    double small, double thresh,
                                    int print_level, std::string xy) {
  ...
  ResponseFunction deriv_J =
      create_coulomb_derivative(world, f, orbitals, small, thresh);
...
Tensor<double> TDHF::create_response_matrix(
...
void TDHF::iterate(World& world) {
  ...
  while (iteration < Rparams.max_iter && !all_converged) {
    ...
    // Create gamma
    x_gamma = create_gamma(world, x_response, y_response, Gparams.orbitals,
                           Rparams.small, FunctionDefaults<3>::get_thresh(),
                           Rparams.print_level, "x");
    if (!Rparams.tda)
      y_gamma = create_gamma(world, y_response, x_response, Gparams.orbitals,
                             Rparams.small, FunctionDefaults<3>::get_thresh(),
    ...
    if (Rparams.print_level >= 2) {
      if (world.rank() == 0)
        print(
            "   Norms of off-diagonal hamiltonian correction for x "
            "components:");
    ...
    if (not Rparams.tda) {
      ...
      if (Rparams.print_level >= 2) {
        if (world.rank() == 0)
          print(
              "   Norms of off-diagonal hamiltonian correction for y "
              "components:");
        print_norms(world, temp);
      }
    }
    ...
    if (not relax) {
      ...
      if (num_conv == m) {
        ...
        if (world.rank() == 0)
          print(
              "   All components converged. Unfreezing all states for final "
              "relaxation.");
...
ResponseFunction TDHF::create_random_guess(
    World& world, int m, int n, std::vector<real_function_3d>& grounds,
    Molecule& molecule) {
  // Basic output
  if (world.rank() == 0)
