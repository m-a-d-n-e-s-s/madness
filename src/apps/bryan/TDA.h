...
class TDA 
{
   ...
   public:
      ...
      double calculate_max_residual(World & world, std::vector<std::vector<real_function_3d>> f);
      ...
      // Prints iterate headers, used in default printing
      void print_guess_iterate_headers(World & world);

      // Iterates the trial functions until covergence or it runs out of iterations
      void iterate(World & world);

      // Iterates the intial guess functions 5 times to get a feel for which guesses 
      // should be used in the calculation
      Tensor<double> guess_iterate(World & world,
                                   std::vector<std::vector<real_function_3d>> f,
                                   std::vector<real_function_3d> orbitals,
                                   Tensor<double> energies,
                                   double thresh,
