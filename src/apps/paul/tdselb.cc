...
struct InputParameters {
  ...
  int lbmode;         // load balancing mode:
                      // 0: No load balancing
                      // 1: Uniform cost
                      // 2: Profiled cost
  ...
  void serialize(Archive & ar) {
    ...
    ar & k & thresh & safety & cut & prefix & ndump & nplot & nprint & nloadbal & nio & target_time & lbmode &
		tScale;
  }
};
...
ostream& operator<<(ostream& s, const InputParameters& p) {
 s << p.L<< " " << p.Lsmall<< " " << p.Llarge<< " " << p.F<< " " << p.omega<<
        ...
        p.nplot << " " << p.nprint << " "  << " " << p.lbmode << p.nloadbal << " " << p.nio << p.tScale << std::endl;
return s;
}
...
complex_functionT chin_chen(const complex_functionT& expV_0,
    ...
    CostFun::Instance(world).print_timing_stats(step);
    if (psi1.world().rank() == 0) {
        printf("chin-chen: %d %.2f %.2f %.2f %.2f %.2f %.2f \n", step, t1-t0, t2-t1, t3-t2, t4-t3, t5-t4, t5-t0);
    }
...
struct lbcost {
    ...
    template <typename T>
    unsigned long operator()(const Key<NDIM>& key, const FunctionNode<T,NDIM>& node) const {
        double lfac = 1.0; //pow(1.2,1.0*key.level());
        if (key.level() <= 1) {
            return (unsigned long) 128.0;
        }
        else if (node.is_leaf()) {
            return (unsigned long) leaf_value*lfac;
        }
        else {
            return (unsigned long) parent_value*lfac;
        }
        //return (unsigned long) key.level()+1.0;
    }
};
...
void loadbal(World& world, 
    ...
    // turn off timers for constant-cost LB
    if (param.lbmode == 1) {
      CostFun::Instance(world).set_master_status(false);
    }
    world.gop.fence();
...
void propagate(World& world, int step0) {
    ...
    while (step < nstep) {
        ...
        t0 = wall_time();
        if (param.lbmode > 0 && (step < 2 || (step%param.nloadbal) == 0))
        ...
        if (use_trotter) {
            ...
            // Apply Chin-Chen
            psi = chin_chen(expv_0, expv_tilde, expv_1, G, psi, step, world);
            t10 = wall_time();
        }
...
// TEST - start with gaussian mapping
    FunctionDefaults<3>::set_pmap(SharedPtr<MyPmap<3> >(new MyPmap<3>(world)));
    //FunctionDefaults<3>::set_pmap(pmapT(new MyPmap<3>(world)));
    ...
    // always turn on timers so cost of accumulating is part of all
    // types of runs
    CostFun::Instance(world).set_master_status(true);
    world.gop.fence();
