...
template class MyCostFn<3>;
template <int D> template <typename T> Cost MyCostFn<D>::cost_fun(const Key<D>& key, const FunctionNode<T,D>& node);
template <int D> Cost MyCostFn<D>::actual_cost_fun(const Key<D>& key);
template <int D> void MyCostFn<D>::set_wt_factor(double w);
template <int D> void MyCostFn<D>::set_apply_time(SharedPtr<ApplyTime<D> > apt);
template <int D> void MyCostFn<D>::set_defaults();
template <int D> double MyCostFn<D>::wt_factor;
template <int D> SharedPtr<ApplyTime<D> > MyCostFn<D>::apply_time;
...
complex_functionT chin_chen(const complex_functionT& expV_0,
			    ...
			    SharedPtr<ApplyTime<3> > apply_time = SharedPtr<ApplyTime<3> > () ) {
    ...
    double t1 = wall_time();
    // about to set apply_time_ptr if apply_time is not null
    if (apply_time) {
      psi1.set_apply_time_ptr(apply_time);
    }
...
void propagate(World& world, int step0, SharedPtr<ApplyTime<3> > apply_time = SharedPtr<ApplyTime<3> > ()) {
	...
	//set apply_time_ptr
	psi.set_apply_time_ptr(apply_time);
        t0 = wall_time();
        if ((step != 0) && (step < 2 || (step%param.nloadbal) == 0))
        ...
        if (use_trotter) {
            ...
            // Apply Chin-Chen
            psi = chin_chen(expv_0, expv_tilde, expv_1, G, psi, apply_time);
            t10 = wall_time();
        }
...
void doit(World& world) {
    ...
    // create apply_time
    SharedPtr<ApplyTime<3> > apply_time = SharedPtr<ApplyTime<3> > (new ApplyTime<3>(world));
    potn.set_apply_time_ptr(apply_time);
    MyCostFn<3>::set_apply_time(apply_time);
