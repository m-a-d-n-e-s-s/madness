...
/// @param[inout]	xp	the response function
/// @param[in]		hf 	the HF reference object
/// @param[in]	iteration	the current iteration
double iterate_CCS(World& world, const vecfuncT& amo, double& omega, vecfuncT& xp,
	...
	vecfuncT Vx=mul(world,vlocal,xp);

	// and the exchange potential is K xp
	vecfuncT Kx=hf.get_calc().apply_hf_exchange(world,hf.get_calc().aocc,amo,xp);
	...
	for (int p=0; p<nmo; ++p) {
		...
		for (int i=0; i<nmo; ++i) rhoprime+=xp[i]*amo[i];
		Gamma[p]=2.0*(*poisson)(rhoprime)*amo[p];

		// the exchange part Eq. (8)
		for (int i=0; i<nmo; ++i) Gamma[p]-=xp[i]*(*poisson)(amo[i]*amo[p]);
	...
	for (int p=0; p<amo.size(); ++p) {
		...
		t1+=0.5*(inner(Vphi[p],GVphi[p]) - inner(Vphi[p],xp[p]));
		double n=GVphi[p].norm2();
		t2+=n*n;
	}
	double delta=-t1/t2;
	// do some damping for the first few iterations
	if (iteration<5) omega+=0.1*delta;
	...
	for (int p=0; p<nmo; ++p) {
		xp[p]=GVphi[p] - rho0(GVphi[p]);

		// Is there any reason to normalize the transition density?
		// The results are identical either way.
//		double norm=xp[p].norm2();
//		xp[p].scale(1.0/norm);
	}
