...
bool iterate(World &world, const ResponseManager &rm, const GroundStateData &gs, const LinearResponseDescriptor &state,
             ResponseType &response, ResponseDebugLogger &logger, size_t max_iter, double conv_thresh) {
  ...
  const double dconv = std::max(FunctionDefaults<3>::get_thresh() * 10, conv_thresh);
  auto density_target = dconv * static_cast<double>(std::max(size_t(5.0), gs.molecule.natom()));
  ...
  auto bsh_ops = make_bsh_operators(world, rm, state.current_frequency(), orbital_energies,
  ...
  for (size_t iter = 0; iter < max_iter; ++iter) {
    ...
    DEBUG_TIMED_BLOCK(world, &logger, "compute_rsh",
                      { x_new = CoupledResponseEquations(world, gs, rvec, vp, bsh_ops, rm, logger); });
    ...
    auto alpha = alpha_factor(rvec) * ResponseSolverUtils::inner(world, rvec.flat, vp);
    DEBUG_LOG_VALUE(world, &logger, "alpha", alpha);
    // 7. Convergence check
    if (world.rank() == 0) {
      ResponseSolverUtils::print_iteration_line(iter, res_norm, drho_change, alpha, density_target, x_residual_target);
    }
...
inline bool solve_response_vector(World &world, const ResponseManager &rm, const GroundStateData &gs,
  ...
  return std::visit(
      overloaded{
          // Only wire types that are READY:
          [&](StaticRestrictedResponse &r) { return iterate(world, rm, gs, state, r, logger, max_iter, conv_thresh); },
          [&](DynamicRestrictedResponse &r) { return iterate(world, rm, gs, state, r, logger, max_iter, conv_thresh); },
          [&](auto &) -> bool { throw std::logic_error("This response type isn’t implemented yet"); }},
      response_variant);
}

// return std::visit(overloaded{[&](StaticRestrictedResponse &r) {
//                                return iterate(world, rm, gs, state, r, logger, max_iter, conv_thresh);
//                              },
...
inline void computeFrequencyLoop(World &world, const ResponseManager &rm, LinearResponseDescriptor &state,
                                 const GroundStateData &ground_state, ResponseMetadata &metadata,
    ...
    if (is_saved && load_response_vector(world, num_orbitals, state, guess, thresh_index, freq_index)) {
      if (world.rank() == 0) {
        madness::print("📂 Loaded response vector from disk.");
               ...
               load_response_vector(world, num_orbitals, state, guess, thresh_index - 1, freq_index)) {
      if (world.rank() == 0) {
