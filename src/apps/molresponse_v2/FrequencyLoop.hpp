    ...
    for (size_t iter = 0; iter < max_iter; ++iter) {
        ...
        if (res_norm > rm.params().maxrotn()) {
            DEBUG_TIMED_BLOCK(world, &logger, "step_restriction", {
                              ResponseSolverUtils::do_step_restriction(world, all_x, kain_x, res_norm, "a", rm.params().
                                  maxrotn());
                              });
        }
        ...
        // 7. Convergence check
        if (world.rank() == 0) {
            ResponseSolverUtils::print_iteration_line(iter, res_norm, drho_change, alpha, density_target,
                                                      x_residual_target);
        }
...
inline void promote_response_vector(World &world, const ResponseVector &x_in, ResponseVector &x_out) {
...
inline void computeFrequencyLoop(World &world, const ResponseManager &rm, LinearResponseDescriptor &state_desc,
                                 const GroundStateData &ground_state, ResponseRecord &response_record,
    ...
    ResponseVector previous_response = make_response_vector(num_orbitals, state_desc.is_static(), is_unrestricted);
    ...
    // Frequency loop
    for (size_t i = state_desc.current_frequency_index; i < state_desc.frequencies.size(); i++) {
        state_desc.set_frequency_index(i);
        auto freq_index = state_desc.current_frequency_index;
        bool is_saved = response_record.is_saved(state_desc); // Check if already saved
        bool should_solve = !is_saved || (at_final_protocol && !response_record.is_converged(state_desc));
        if (!should_solve) {
            if (world.rank() == 0) {
                print("⚠️  Skipping frequency", state_desc.current_frequency(), "at protocol", protocol, "for state:",
                      pertDesc);
            }
            continue;
        }
        world.gop.fence();
        ResponseVector x0 = make_response_vector(num_orbitals, state_desc.is_static(), is_unrestricted);

        if (is_saved && load_response_vector(world, num_orbitals, state_desc, thresh_index, freq_index, x0)) {
            if (world.rank() == 0) {
                madness::print("📂 Loaded response vector from disk.");
            }
        } else if (thresh_index > 0 &&
                   load_response_vector(world, num_orbitals, state_desc, thresh_index - 1, freq_index, x0)) {
            if (world.rank() == 0) {
            ...
            if (have_previous_freq_response) {
                ...
                load_response_vector(world, num_orbitals, state_desc, thresh_index, freq_index - 1, x0);
            }

            world.gop.fence();
            promote_response_vector(world, previous_response, x0);
        } else {
        ...
        bool converged = solve_response_vector(world, rm, ground_state, state_desc, x0, logger, max_iter, conv_thresh);
        logger.finalize_state();
