...
#define NOT_IMPLEMENTED_THROW throw std::runtime_error("This solver is not yet implemented.");

template <typename ResponseType>
inline bool iterate(World &world, const ResponseManager &rm, const GroundStateData &gs, const ResponseState &state, ResponseType &response,
  ...
  const auto density_target = dconv * static_cast<double>(gs.orbitals.size()) * thresh;
  ...
  auto bsh_ops = Policy::make_bsh_operators(world, rm, state.current_frequency(), orbital_energies, static_cast<int>(gs.orbitals.size()), logger);

  response_solver solver(response_vector_allocator(world, static_cast<int>(all_x.size())),
                         /*do_printing*/ false);
  ...
  for (size_t iter = 0; iter < max_iter; ++iter) {
    ...
    DEBUG_LOG_VALUE(world, &logger, "<x|x>", ResponseSolverUtils::inner(world, rvec.flat, rvec.flat));
    // 1. Coupled-response equations
    vector_real_function_3d x_new;
    DEBUG_TIMED_BLOCK(world, &logger, "compute_rsh", { x_new = Policy::CoupledResponseEquations(world, gs, rvec, vp, bsh_ops, rm, logger); });
    // 2. Form residual r = x_new - x
    auto residuals = x_new - all_x;
    vector_real_function_3d kain_x;
    DEBUG_TIMED_BLOCK(world, &logger, "KAIN step", { kain_x = solver.update(all_x, residuals); });
    ...
    if (res_norm > rm.params().maxrotn() and false) {
      DEBUG_TIMED_BLOCK(world, &logger, "step_restriction",
                        { ResponseSolverUtils::do_step_restriction(world, all_x, kain_x, res_norm, "a", rm.params().maxrotn()); });
    }
    ...
    auto alpha = Policy::alpha_factor * ResponseSolverUtils::inner(world, rvec.flat, vp);
    DEBUG_LOG_VALUE(world, &logger, "alpha", alpha);
    // 7. Convergence check
    if (world.rank() == 0) {
      ResponseSolverUtils::print_iteration_header();
      ResponseSolverUtils::print_iteration_line(iter, res_norm, drho_change, alpha);
    }
  ...
  if (world.rank() == 0) print("⚠️  Reached max iterations without convergence.");
  logger.finalize_state();
  return false;
};

inline bool solve_response_vector(World &world, const ResponseManager &rm, const GroundStateData &gs, const ResponseState &state,
                                  ResponseVector &response_variant,  // the std::variant<…>
                                  ResponseDebugLogger &logger, size_t max_iter = 10, double conv_thresh = 1e-4) {
  return std::visit(overloaded{[&](StaticRestrictedResponse &r) { return iterate(world, rm, gs, state, r, logger, max_iter, conv_thresh); },
                               [&](DynamicRestrictedResponse &r) { return iterate(world, rm, gs, state, r, logger, max_iter, conv_thresh); },
                               [&](StaticUnrestrictedResponse &r) {
                                 throw std::runtime_error("Static unrestricted response not implemented yet");
                                 return false;
                                 /*return iterate(world, rm, gs, state, r, logger, max_iter,*/
                                 /*               conv_thresh);*/
                               },
                               [&](DynamicUnrestrictedResponse &r) {
                                 throw std::runtime_error("Dynamic unrestricted response not implemented yet");
                                 return false;
...
inline void promote_response_vector(World &world, const ResponseVector &promote_from, ResponseVector &promote_to) {
  if (std::holds_alternative<StaticRestrictedResponse>(promote_from)) {
    if (world.rank() == 0) madness::print("🔁 Promoting static restricted → dynamic restricted");
    ...
    if (world.rank() == 0) madness::print("🔁 Promoting static unrestricted → dynamic unrestricted");
    ...
    if (world.rank() == 0) madness::print("📥 Copying dynamic restricted response");
  ...
  } else if (std::holds_alternative<DynamicUnrestrictedResponse>(promote_from)) {
    if (world.rank() == 0) madness::print("📥 Copying dynamic unrestricted response");
    ...
    throw std::runtime_error("Unknown response variant in promote_response_vector");
  }
}

inline void computeFrequencyLoop(World &world, const ResponseManager &rm, ResponseState &state, const GroundStateData &ground_state, ResponseMetadata &metadata,
  ...
  ResponseVector previous_response = make_response_vector(num_orbitals, state.is_static(), is_unrestricted);
  bool have_previous_freq_response = false;
  auto pertDesc = state.perturbationDescription();

  // Frequency loop
  for (size_t i = state.current_frequency_index; i < state.frequencies.size(); i++) {
    ...
    bool is_saved = metadata.is_saved(pertDesc, protocol, freq);  // Check if already saved
    bool should_solve = !is_saved || (at_final_protocol && !metadata.is_converged(pertDesc, protocol, freq));
    if (!should_solve) {
      if (world.rank() == 0) {
        print("⚠️  Skipping frequency", freq, "at protocol", protocol, "for state:", pertDesc);
      }
      continue;
    }
    world.gop.fence();
    ResponseVector guess = make_response_vector(num_orbitals, is_static, is_unrestricted);

    if (world.rank() == 0) {
      madness::print("🔄 Attempting to load response vector for", state.description());
    }
    // At this point, I know that I'm either not saved or not at the final
    if (is_saved && load_response_vector(world, num_orbitals, state, guess, thresh_index, freq_index)) {
      if (world.rank() == 0) {
        madness::print("📂 Loaded response vector from disk.");
      }
    } else if (thresh_index > 0 && load_response_vector(world, num_orbitals, state, guess, thresh_index - 1, freq_index)) {
      if (world.rank() == 0) {
      ...
      if (have_previous_freq_response) {
        ...
        load_response_vector(world, num_orbitals, state, previous_response, thresh_index, freq_index - 1);
      }
    ...
    bool converged = solve_response_vector(world, rm, ground_state, state, guess, logger, max_iter, conv_thresh);
    logger.finalize_state();
    ...
    if (world.rank() == 0) {
      madness::print("💾 Saved response vector at protocol:", protocol, state.description());
    }
  ...
  state.is_converged = state.at_final_threshold() && metadata.final_converged(state_id);
}
