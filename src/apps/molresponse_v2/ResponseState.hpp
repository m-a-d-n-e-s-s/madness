...
struct LinearResponseDescriptor : public AbstractResponseDescriptor {
  ...
  LinearResponseDescriptor(Perturbation pert, const std::vector<double> &freq, const std::vector<double> &thresh,
                           bool spin_restricted)
      : perturbation(pert), frequencies(freq), thresholds(thresh), current_frequency_index(0), current_thresh_index(0),
  ...
  [[nodiscard]] bool at_final_frequency() const { return current_frequency_index + 1 == frequencies.size(); }
  [[nodiscard]] bool at_final_threshold() const { return current_thresh_index + 1 == thresholds.size(); }
  ...
  [[nodiscard]] bool is_static(size_t freq_index) const { return std::abs(frequencies[freq_index]) < 1e-8; }
  ...
  [[nodiscard]] std::string make_key(size_t ti, size_t fi) const { return make_key(thresholds[ti], frequencies[fi]); }
  ...
  [[nodiscard]] std::string description() const { return make_key(current_threshold(), current_frequency()); }
...
struct SecondOrderResponseDescriptor : public AbstractResponseDescriptor {
  ...
  SecondOrderResponseDescriptor( Perturbation p1, Perturbation p2, double f1,
      ...
      : perturbations_(p1, p2), frequencies_(f1, f2), thresh(thresh), spin_restricted_(spin_restricted) {}

  [[nodiscard]] LinearResponseDescriptor B_state() const {
    return LinearResponseDescriptor(perturbations_.first, {frequencies_.first}, {thresh}, spin_restricted_);
  }
  [[nodiscard]] LinearResponseDescriptor C_state() const {
    return LinearResponseDescriptor(perturbations_.second, {frequencies_.second}, {thresh}, spin_restricted_);
  }

  [[nodiscard]] std::pair<LinearResponseDescriptor, LinearResponseDescriptor> get_states() const {
  ...
  [[nodiscard]] std::string make_key(double f1, double f2, double thresh) const {
    std::ostringstream oss;
  ...
  [[nodiscard]] std::string response_filename() const override {
    return make_key(frequencies_.first, frequencies_.second, thresh) + ".response";
  }
  [[nodiscard]] std::string response_filename(const size_t &thresh_index, const size_t &freq_index) const override {
...
inline real_function_3d make_perturbation_operator(World &world, const GroundStateData &gs,
...
inline real_function_3d make_perturbation_operator(World &world, const GroundStateData &gs,
  ...
  real_function_3d f = real_factory_3d(world).functor(
      real_functor_3d{new madchem::MolecularDerivativeFunctor(gs.molecule, n.atom_index, dipole_map.at(n.direction))});
  f.truncate(FunctionDefaults<3>::get_thresh());
  return f;
}
...
inline real_function_3d raw_perturbation_operator(World &world, const GroundStateData &gs, const Perturbation &p) {

  return std::visit([&](const auto &pp) -> real_function_3d { return make_perturbation_operator(world, gs, pp); }, p);
}
...
// -----------------------------------------------------------------------------
  ...
  // -----------------------------------------------------------------------------
  inline vector_real_function_3d project_perturbation_onto_orbitals(World & world, const GroundStateData &gs,
  ...
  inline madness::vector_real_function_3d perturbation_vector(madness::World & world, GroundStateData const &gs,
