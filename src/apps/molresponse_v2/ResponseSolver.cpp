...
real_function_3d compute_density(World &world,
                                 const StaticUnrestrictedResponse &rvec,
                                 const vector_real_function_3d &phi0) {
  throw std::runtime_error(
      "compute_density for StaticUnrestrictedResponse not implemented");
  // auto phi_phi = phi0;
  // phi_phi.insert(phi_phi.end(), phi0.begin(), phi0.end());
  // auto xphi = mul(world, rvec.flat, phi_phi, true);
...
real_function_3d compute_density(World &world,
                                 const DynamicUnrestrictedResponse &rvec, 
                                 const vector_real_function_3d &phi0) {
  throw std::runtime_error(
      "compute_density for DynamicUnrestrictedResponse not implemented");
  // auto phi_phi = phi0;
  // phi_phi.insert(phi_phi.end(), phi0.begin(), phi0.end());
  // auto &x = rvec.flat;
  //
...
make_bsh_operators(World &world, const ResponseManager &response_manager,
  ...
  if ((orbital_energies[static_cast<long>(n) - 1] + freq) >= 0.0) {
    x_shifts =
        -shift_factor - (freq + orbital_energies[static_cast<long>(n) - 1]);
  }
...
vector_real_function_3d CoupledResponseEquations(
    World &world, const GroundStateData &g_s,
    const StaticRestrictedResponse &vecs, const vector_real_function_3d &v_p,
    const std::vector<poperatorT> &bsh_x,
    const ResponseManager &response_manager, ResponseDebugLogger &logger) {

  auto num_orbitals = g_s.orbitals.size();

  std::vector<int> state_index;
  std::vector<int> ii;

  int i = 0;
  for (int j = 0, si = 0; j < num_orbitals; j++) {
  ...
  DEBUG_TIMED_BLOCK(world, &logger, "g0_task", {
    k_0 = g0_task(ii, state_index, vecs.flat, g_s.orbitals, true);
  });
  DEBUG_TIMED_BLOCK(world, &logger, "gx_task", {
    g_x = gx_task(ii, state_index, vecs.flat, g_s.orbitals, true);
  });
...
vector_real_function_3d CoupledResponseEquations(
    World &world, const GroundStateData &g_s,
    const DynamicRestrictedResponse &vecs, const vector_real_function_3d &v_p,
    const std::vector<poperatorT> &bsh_x,
    const ResponseManager &response_manager, ResponseDebugLogger &logger) {
  ...
  DEBUG_TIMED_BLOCK(world, &logger, "g0_task", {
    k_0 = g0_task(ii, state_index, all_x, g_s.orbitals, false);
  });
  DEBUG_TIMED_BLOCK(world, &logger, "gx_task", {
    g_x = gx_task(ii, state_index, all_x, g_s.orbitals, false);
  });
