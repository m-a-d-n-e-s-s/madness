...
struct ResponseParameters : public QCCalculationParametersBase {
  ...
  ResponseParameters() {
    ...
    initialize<std::vector<std::string>>("requested_properties", {"polarizability"}, "properties to calculate (polarizability, hyperpolarizability, Raman.)");
    //** if properites are requested, then one should specify directions, frequencies, and atom_indices(for nuclear response) */
 ...
 public:
  ...
  [[nodiscard]] std::vector<std::string> requested_properties() const { return get<std::vector<std::string>>("requested_properties"); }
  [[nodiscard]] bool dipole() const { return get<bool>("dipole"); }
  [[nodiscard]] std::vector<double> dipole_frequencies() const { return get<std::vector<double>>("dipole.frequencies"); }
  ...
  [[nodiscard]] std::vector<double> nuclear_frequencies() const { return get<std::vector<double>>("nuclear.frequencies"); }
 ...
 private:
  void validate_user_specified_properties() {
    ...
    if (is_user_defined("requested_properties")) {
      ...
      for (auto const &prop : props) {
        if (prop == "polarizability" || prop == "hyperpolarizability") {
          if (!is_user_defined("dipole.frequencies") || !is_user_defined("dipole.directions"))
            throw std::runtime_error("When requesting '" + prop + "', you must also set dipole.frequencies and dipole.directions.");
        }
        if (prop == "raman") {
          if (!is_user_defined("dipole.frequencies") || !is_user_defined("dipole.directions") || !is_user_defined("nuclear.frequencies") ||
              !is_user_defined("nuclear.directions") || !is_user_defined("nuclear.atom_indices"))
            throw std::runtime_error("When requesting 'raman', you must set both dipole.* and nuclear.* parameters.");
        }
      }
    }
  }
