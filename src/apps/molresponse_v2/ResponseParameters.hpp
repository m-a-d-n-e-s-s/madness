...
struct ResponseParameters : public QCCalculationParametersBase {
  ...
  ResponseParameters() {
    initialize<std::string>("prefix", "response",
                            "prefixes your output/restart/json/plot/etc files");
    initialize<std::string>("fock_json_file", "moldft.fock.json",
    ...
    initialize<size_t>(
        "maxsub", 8, "size of iterative subspace ... set to 0 or 1 to disable");
    ...
    initialize<std::vector<double>>("dipole.frequencies", {0.0},
                                    "frequencies for dipole response");
    initialize<std::string>("dipole.directions", "xyz",
                            "directions for dipole response");
    initialize<bool>("nuclear", false, "Compute nuclear response");
    initialize<std::string>("nuclear.directions", "xyz",
                            "directions for nuclear response");
    initialize<std::vector<double>>("nuclear.frequencies", {0.0},
                                    "frequencies for nuclear response");
    initialize<std::vector<int>>("nuclear.atom_indices", {0},
                                 "atom indices for nuclear response");
    initialize<bool>(
        "quadratic", false,
        "Compute quadratic response properties from defined perturbations");
 ...
 public:
  ...
  [[nodiscard]] std::vector<int> nuclear_atom_indices() const {
    return get<std::vector<int>>("nuclear.atom_indices");
  }
    ...
    if (property()) {
      ...
      for (auto const &prop : props) {
        if (prop == "polarizability" || prop == "hyperpolarizability") {
          if (!is_user_defined("dipole.frequencies") ||
              !is_user_defined("dipole.directions"))
        ...
        if (prop == "raman") {
          if (!is_user_defined("dipole.frequencies") ||
              !is_user_defined("dipole.directions") ||
              !is_user_defined("nuclear.frequencies") ||
              !is_user_defined("nuclear.directions") ||
