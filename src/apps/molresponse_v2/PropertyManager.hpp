...
public:
  ...
  void print_table() const {
    ...
    for (auto const &[k, r] : rows_) {
      ...
      std::cout << std::left << std::setw(22) << r.property << std::setw(8) << r.component << std::setw(8) << o1.str()
  ...
  for (auto &freq_b : frequencies.first) {
    for (auto &freq_c : frequencies.second) {
      ...
      for (auto [B, C] : BC_pairs) {
        ...
        if (skip) {
          if (world.rank() == 0)
            print("✅ Skipping already computed β at ωB =", freq_b, "ωC =", freq_c, "for directions:", abc_string);
          continue;
        }
