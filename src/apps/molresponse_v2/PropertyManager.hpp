...
inline madness::Tensor<double> compute_response_inner_product_tensor(madness::World &world,
                                                                     const std::vector<vector_real_function_3d> &A_vecs,
                                                                     const std::vector<vector_real_function_3d> &B_vecs,
                                                                     bool save_contributions = false,
...
public:
    ...
    [[nodiscard]] bool has_raman(double w1, double w2, std::string comp) const {
    ...
    // Insert or overwrite β entries
    void set_raman(double w1, double w2, const std::string &comp, double value) {
        PropKey k{"raman", comp, w1, w2};
        PropRow r{
            .property = k.property,
            .component = k.component,
    ...
    void print_table() const {
        ...
        // 1) Header
        std::cout << "\n📊 Property Results\n";
        std::cout << std::left << std::setw(18) << "Property" << std::setw(8) << "Comp" << std::setw(8) << "ω1"
                << std::setw(8) << "ω2" << std::setw(12) << "Value" << "\n";
        ...
        for (auto const &[k, r]: rows_) {
            ...
            std::cout << std::left << std::setw(22) << r.property << std::setw(8) << r.component << std::setw(8) << o1.
                    str()
                    << std::setw(8) << o2.str() << std::setw(12) << std::fixed << std::setprecision(6) << *r.value <<
                    "\n";
        }
    }
                ...
                for (auto const &[w2_s, entry]: sub.items()) {
                    ...
                    for (auto const &[bc, tens_json]: entry.items()) {
                        ...
                        for (int k = 0; k < tens.size(); ++k) {
                            std::string comp = std::string{tens_json.value("directions", "")[k]} + bc;
 ...
 */
void compute_alpha(World &world, std::map<std::string, LinearResponseDescriptor> &state_map, const GroundStateData &gs,
                   const std::vector<double> &frequencies, const std::string &directions, PropertyManager &pm) {
    ...
    for (size_t f = 0; f < num_frequencies; ++f) {
        ...
        for (size_t j = 0; j < num_directions; ++j) {
            ...
            load_response_vector(world, num_orbitals, active_state, active_state.thresholds.size() - 1, f,
                                 load_vector[j]);
        ...
        madness::Tensor<double> alpha = compute_response_inner_product_tensor(world, response_vecs, perturb_vecs, true,
...
[[nodiscard]] VibrationalResults compute_hessian(World &world,
    ...
    for (size_t iatom = 0; iatom < natom; ++iatom) {
        for (int iaxis = 0; iaxis < 3; ++iaxis) {
            ...
            load_response_vector(world, num_orbitals, active_state, active_state.thresholds.size() - 1, 0, load_vector);
        ...
        for (auto &freq_c: frequencies.second) {
            ...
            for (auto [B, C]: BC_pairs) {
                ...
                for (auto a: perturbation_A) {
                    ...
                    auto comp = (describe_perturbation(a) + describe_perturbation(B) + describe_perturbation(C));
                ...
                if (skip) {
                    if (world.rank() == 0)
                        print("✅ Skipping already computed β at ωB =", freq_b, "ωC =", freq_c, "for directions:",
                              abc_string);
                    continue;
                }
                ...
                for (int a = 0; a < perturbation_A.size(); ++a) {
                    ...
                    auto state_A =
                            LinearResponseDescriptor(pertA, {omega_A}, {FunctionDefaults<3>::get_thresh()},
                                                     is_spin_restricted);
                ...
                madness::Tensor<double> beta_tensor =
                        compute_response_inner_product_tensor(world, xa_vecs, {vbc_flat}, true, "beta_contribs" + bc);
                ...
                y_zeta_bc.y_alpha =
                        SimpleVBCComputer::make_zeta_bc(world, std::get<DynamicRestrictedResponse>(xb).y_alpha,
                                                        std::get<DynamicRestrictedResponse>(xc).x_alpha, gs.orbitals);
                y_zeta_cb.y_alpha =
                        SimpleVBCComputer::make_zeta_bc(world, std::get<DynamicRestrictedResponse>(xc).y_alpha,
                ...
                madness::Tensor<double> beta_2 =
                        compute_response_inner_product_tensor(world, ra_y_zeta_bc, xb_phi0_vec, true, "beta_zeta_bc_1");

                file_name = "responses/" + vbc_state.perturbationDescription() + "_beta_zeta_bc_1.json";
                madness::Tensor<double> beta_3 =
                        compute_response_inner_product_tensor(world, ra_y_zeta_cb, xc_phi0_vec, true, file_name);
                if (world.rank() == 0) {
                ...
                for (auto comp: abc_string) {
                    if (prop_type == PropertyType::Raman)
                        pm.set_raman(freq_b, freq_c, comp, beta_tensor(aa, 0)); // set the Raman value
                    else if (prop_type == PropertyType::Beta)
                        pm.set_beta(freq_b, freq_c, comp, beta_tensor(aa, 0)); // set the β value
                    aa++;
                }
...
// ─────────────────────────────────────────────────────────────────────────────
void compute_Raman(World &world, const GroundStateData &gs,
                   const std::vector<double> &frequencies, // ωB and ωC
    ...
    for (char b: dip_dirs) {
        for (auto &n: nuc_dirs) {
            for (int index: nuc_indices) {
                BC_pairs.emplace_back(DipolePerturbation{b}, NuclearDisplacementPerturbation{index, n});
            }
        }
    }
