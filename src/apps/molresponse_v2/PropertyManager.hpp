...
public:
  ...
  void set_alpha(double omega, const madness::Tensor<double>& tensor, const std::string& dirs)
  {
    ...
    for (size_t i = 0; i < N; ++i)
    {
      ...
      {
        components comp = {std::format("{}", dirs[i]), std::format("{}", dirs[j])};
        PropKey k{"polarizability", comp, omega, std::nullopt};
        PropRow r{
            .property = k.property,
  ...
  void print_table() const
  {
    ...
    // 1) Header
    std::cout << "\n📊 Property Results\n";
    std::cout << std::left << std::setw(18) << "Property" << std::setw(8) << "Comp" << std::setw(8)
              << "ω1" << std::setw(8) << "ω2" << std::setw(12) << "Value" << "\n";
    ...
    for (const auto& [k, r] : rows_)
    {
      ...
      std::cout << std::left << std::setw(22) << r.property << std::setw(8) << r.component
                << std::setw(8) << o1.str() << std::setw(8) << o2.str() << std::setw(12)
 ...
 */
void compute_alpha(World& world, std::map<std::string, LinearResponseDescriptor>& state_map,
                   ...
                   const std::string& directions, PropertyManager& pm)
{
  ...
  if (world.rank() == 0)
  {
    print("▶️ Computing α tensor for", num_directions, "directions and", num_frequencies,
          "frequencies.");
  }
  ...
  for (const auto& dir : direction_keys)
  {
    ...
    else
    {
      perturbations.push_back(
          perturbation_vector(world, gs, state_map.at(dir))); // Get the perturbation vector
    }
  }
  ...
  for (size_t ffi = 0; ffi < num_frequencies; ++ffi)
  {
    ...
    for (size_t j = 0; j < num_directions; ++j)
    {
      ...
      load_response_vector(world, num_orbitals, active_state, active_state.thresholds.size() - 1,
                           ffi, load_vector[j]);
    ...
    madness::Tensor<double> alpha = compute_response_inner_product_tensor(
        world, response_vecs, perturb_vecs, true, "alpha_contribs_" + std::to_string(ffi));
...
compute_hessian(World& world, std::map<std::string, LinearResponseDescriptor>& state_map,
                ...
                const std::shared_ptr<SCF>& scf_calc)
{
  ...
  for (const auto& adx : atom_derivative_keys)
  {
    ...
    {
      perturbations.push_back(
          perturbation_vector(world, gs, state_map.at(adx))); // Get the perturbation vector
    }
  }
  ...
  for (size_t iatom = 0; iatom < natom; ++iatom)
  {
    for (int iaxis = 0; iaxis < 3; ++iaxis)
    {
      ...
      load_response_vector(world, num_orbitals, active_state, active_state.thresholds.size() - 1, 0,
                           load_vector);
                  ...
                  PropertyManager& pm, const PropertyType& prop_type)
{
  ...
  for (auto& freq_b : frequencies.first)
  {
    for (auto& freq_c : frequencies.second)
    {
      ...
      {
        VBCResponseState vbc_state(B, C, freq_b, freq_c, FunctionDefaults<3>::get_thresh(),
                                   is_spin_restricted);
        ...
        {
          if (world.rank() == 0)
            print("✅ Skipping already computed β at ωB =", freq_b, "ωC =", freq_c,
                  "for directions:", abc_string);
          continue;
        }
        ...
        for (int a = 0; a < perturbation_A.size(); ++a)
        {
          ...
          auto state_A = LinearResponseDescriptor(
              pertA, {omega_A}, {FunctionDefaults<3>::get_thresh()}, is_spin_restricted);
        ...
        madness::Tensor<double> beta_tensor = compute_response_inner_product_tensor(
            world, xa_vecs, {vbc_flat}, true, "beta_contribs" + bc);
        ...
        auto file_name =
            "responses/" + vbc_state.perturbationDescription() + "_beta_zeta_bc_0.json";
