...
class Projector {
 ...
 public:
  ...
  typename std::enable_if<NDIM == FDIM, Function<T, FDIM>>::type operator()(const Function<T, FDIM> &f) const {
...
double rsquared(const coordT &r) { return r[0] * r[0] + r[1] * r[1] + r[2] * r[2]; }
...
struct CalculationParameters {
  ...
  void print(World &world) const {
...
#endif
    if (core_type != "") madness::print("           core type ", core_type);
    ...
    if (derivatives) madness::print("    calc derivatives ");
    if (dipole) madness::print("         calc dipole ");
    //        if(absolvent){
...
struct Calculation {
  ...
  void localize_PM_ij(const int seti,
                      const int setj,
                      const double tol,
                      ...
                      long &ndone_iter,
                      double &maxtheta,
                      double *MADNESS_RESTRICT Qi,
                      double *MADNESS_RESTRICT Qj,
                      double *MADNESS_RESTRICT Ci,
                      double *MADNESS_RESTRICT Cj,
                      double *MADNESS_RESTRICT Ui,
                      double *MADNESS_RESTRICT Uj) {
    if (seti == setj) {
      ...
      double ovij = 0.0;
      for (long a = 0; a < natom; ++a) ovij += Qi[a] * Qj[a];
  ...
  void localize_PM_task_kernel(tensorT &Q,
    ...
    for (long iter = 0; iter < 100; ++iter) {
      ...
      if (doprint) printf("iteration %ld sum=%.4f ndone=%ld tol=%.2e\n", iter, sum, ndone, tol);
      ...
      if (ndone_iter == 0 && tol == thresh) {
        if (doprint) print("PM localization converged in", ndone, "steps");
  ...
  tensorT localize_boys(World &world,
    ...
    if (world.rank() == 0) {
      ...
      for (long iter = 0; iter < 300; ++iter) {
        ...
        for (long i = 0; i < nmo; ++i) {
          for (long j = 0; j < i; ++j) {
            if (set[i] == set[j]) {
              ...
              double h =
                  4.0 * DIP(dip, i, j, i, j) + 2.0 * DIP(dip, i, i, j, j) - DIP(dip, i, i, i, i) - DIP(dip, j, j, j, j);
        ...
        ndone += ndone_iter;
        if (ndone_iter == 0 && tol == thresh) {
          if (doprint) print("Boys localization converged in", ndone, "steps");
  ...
  void initial_guess(World &world) {
    ...
    if (param.restart) {
      ...
      if (world.rank() == 0) std::cout << "alpha set " << 0 << " " << 0 << "-";
      ...
      if (world.rank() == 0) std::cout << param.nmo_alpha - 1 << std::endl;
      ...
      if (param.nbeta && !param.spin_restricted) {
        ...
        if (world.rank() == 0) std::cout << " beta set " << 0 << " " << 0 << "-";
        ...
        if (world.rank() == 0) std::cout << param.nmo_beta - 1 << std::endl;
      }
    }
  }
  ...
  void initial_guess_mp(World &world) {
    ...
    if (param.restart) {
      ...
      for (int i = 0; i < param.nalpha; ++i) aocc_mp[i] = 1.0;

      aset_mp = std::vector<int>(param.nmo_alpha, 0);
      if (world.rank() == 0) std::cout << "alpha set mp " << 0 << " " << 0 << "-";
      ...
      if (param.nbeta && !param.spin_restricted) {
        ...
        for (int i = 0; i < param.nbeta; ++i) bocc_mp[i] = 1.0;

        bset_mp = std::vector<int>(param.nmo_beta, 0);
        if (world.rank() == 0) std::cout << " beta set mp " << 0 << " " << 0 << "-";
  ...
  vecfuncT apply_potential_response(World &world,
    ...
    if (xc.hf_exchange_coefficient()) {
      START_TIMER(world);
      vecfuncT Kdmo;
      if (ispin == 0) Kdmo = apply_hf_exchange(world, occ, amo, dmo);
      if (ispin == 1) Kdmo = apply_hf_exchange(world, occ, bmo, dmo);
  ...
  vecfuncT apply_potential_mp(World &world,
                              const tensorT &occ,
                              const vecfuncT &amo_mp,
                              const vecfuncT &vf,
                              const vecfuncT &delrho,
                              const functionT &vlocal,
                              double &exc,
  ...
  vecfuncT compute_residual(World &world,
    ...
    if (world.rank() == 0) std::cout << "entering apply\n";
  ...
  // vama1      Convolution1D<double_complex>* G = qm_1d_free_particle_propagator(FunctionDefaults<3>::get_k(), c,
  // 0.5*time_step, 2.0*param.L); vama1      printf("Done creating G\n"); vama1 vama1      // Start iteration over time
  ...
  void solve(World &world) {
    ...
    for (int iter = 0; iter < param.maxiter; ++iter) {
      ...
      if (param.localize && do_this_iter) {
        ...
        amo = transform(world, amo, U, trantol, true);  // U'amoU
        truncate(world, amo);
        normalize(world, amo);
      ...
      if (iter > 0) {
        // print("##convergence criteria: density delta=", da < dconv * molecule.natom() && db < dconv *
        // molecule.natom(), ", bsh_residual=", (param.conv_only_dens || bsh_residual < 5.0*dconv));
    ...
    if (world.rank() == 0) {
      if (param.localize)
        print("Orbitals are localized - energies are diagonal Fock matrix elements\n");
      else
  ...
  // dsol                    update_response_subspace(world, ax, ay, bx, by, rax, ray, rbx, rby, subspace, Q,
  // update_residual); dsol#endif dsol                orthogonalize_response(ax, 0); dsol orthogonalize_response(ay, 0);
  // dsol                orthogonalize_response(bx, 1);
  // dsol                orthogonalize_response(by, 1);
  // dsol
  // dsol                if(!param.spin_restricted && param.nbeta != 0) residual = norm2(world, sub(world, ax, ax_old))
  // + norm2(world, sub(world, bx, bx_old)); dsol                else if (param.nbeta != 0) residual = 2 * norm2(world,
  // sub(world, ax, ax_old)); dsol                else residual = norm2(world, sub(world, ax, ax_old)); dsol dsol
  // print("\nOLD response function (plus, alpha_spin) = ", norm2(world, ax_old)); dsol                print("NEW
  // response function (plus, alpha_spin) = ", norm2(world, ax)); dsol                print("\nresiduals_response (final)
  // = ", residual); dsol                print("rconv *(param.nalpha + param.nbeta)*2", rconv *(param.nalpha +
  // param.nbeta)*2); dsol dsol                // 収束判定 dsol                if( residual < (rconv *(param.nalpha +
  // param.nbeta)*2)) dsol                { dsol                    print("\n\n\n"); dsol                    print("
  // ------------------------------------------------------------------------------"); dsol                    print(" |
  // MADNESS CALCULATION POLARIZABILITY                        |"); dsol                    print("
  // ------------------------------------------------------------------------------"); dsol                    print("
  // \n\n"); dsol dsol                    break; dsol                } dsol            } dsol            ax_old = ax;
  // dsol            ay_old = ay;
  // dsol            bx_old = bx;
  // dsol            by_old = by;
  // dsol
  ...
  Converged !
      ...
      timer : Overlap 0.02s 0.02s timer
      : focka eigen sol 0.00s 0.00s
        ...
        0.71734259 -
        0.58481048 -
        0.510olmp if (!param.spin_restricted && param.nbeta != 0){
            // dsolmp                    if (param.localize_pm) {
            ...
            // dsolmp                if (xc.is_gga()) {
            // dsolmp
            // dsolmp                    for (int axis=0; axis<3; ++axis) delrho.push_back((*gradop[axis])(arho,false));
            // // delrho dsolmp                    if (xc.is_spin_polarized()) dsolmp                        for (int
            // axis=0; axis<3; ++axis) delrho.push_back((*gradop[axis])(brho,false)); dsolmp dsolmp dsolmp
            // world.gop.fence(); // NECESSARY dsolmp dsolmp
            // vf.push_back(delrho[0]*delrho[0]+delrho[1]*delrho[1]+delrho[2]*delrho[2]);     // sigma_aa dsolmp dsolmp
            // if (xc.is_spin_polarized()) dsolmp
            // vf.push_back(delrho[0]*delrho[3]+delrho[1]*delrho[4]+delrho[2]*delrho[5]); // sigma_ab dsolmp if
            // (xc.is_spin_polarized()) dsolmp vf.push_back(delrho[3]*delrho[3]+delrho[4]*delrho[4]+delrho[5]*delrho[5]);
            // // sigma_bb dsolmp dsolmp                    for (int axis=0; axis<3; ++axis) vf.push_back(delrho[axis]);
            // // dda_x dsolmp dsolmp                    if (xc.is_spin_polarized()) dsolmp                        for
            // (int axis=0; axis<3; ++axis) vf.push_back(delrho[axis + 3]); // ddb_x dsolmp world.gop.fence(); //
            // NECESSARY dsolmp                } dsolmp                if (vf.size()) { dsolmp reconstruct(world, vf);
            ...
            // dsolmp
            // dsolmp            if (!param.localize && do_this_iter) {
            // dsolmp                tensorT U = diag_fock_matrix(world, focka, amo_mp, Vpsia, aeps_mp, aocc_mp,
            // FunctionDefaults<3>::get_thresh()); dsolmp                rotate_subspace(world, U, subspace, 0,
            // amo_mp.size(), trantol); dsolmp                if (!param.spin_restricted && param.nbeta != 0) { dsolmp U
            // = diag_fock_matrix(world, fockb, bmo_mp, Vpsib, beps_mp, bocc_mp, FunctionDefaults<3>::get_thresh());
            // dsolmp                    rotate_subspace(world, U, subspace, amo_mp.size(), bmo_mp.size(), trantol);
            // dsolmp                }
            // dsolmp            }
            // dsolmp
            ...
            // dsolmp
            // dsolmp            if(iter > 0){
            // dsolmp                //print("##convergence criteria: density delta=", da < dconv * molecule.natom() &&
            // db < dconv * molecule.natom(), ", bsh_residual=", (param.conv_only_dens || bsh_residual < 5.0*dconv));
            // dsolmp                if(da < dconv * molecule.natom() && db < dconv * molecule.natom() &&
            // (param.conv_only_dens || bsh_residual < 5.0*dconv)){ dsolmp                    if(world.rank() == 0) {
            // dsolmp                        print("\nConverged!\n");
            // dsolmp                    }
            // dsolmp
            ...
            // dsolmp                }
            // dsolmp            }
            // dsolmp            update_subspace_mp(world, Vpsia, Vpsib, focka, fockb, subspace, Q, bsh_residual,
            // update_residual, mflag, amo_mp, bmo_mp); dsolmp        } dsolmp        if (world.rank() == 0) { dsolmp if
            // (param.localize) print("Orbitals are localized - energies are diagonal Fock matrix elements\n"); dsolmp
            // else print("Orbitals are eigenvectors - energies are eigenvalues\n"); dsolmp            print("Analysis of
            // alpha MO vectors"); dsolmp        } dsolmp dsolmp        analyze_vectors(world, amo_mp, aocc_mp, aeps_mp);
            // dsolmp        if (param.nbeta != 0 && !param.spin_restricted) {
            // dsolmp            if (world.rank() == 0)
            ...
            void response_frequency(World & world, int &axis){if (world.rank() == 0){print("\n");
  if (axis == 0)
...
vecfuncT calc_dipole_mo(World &world, vecfuncT &mo, int &axis, int &size) {
  ...
  // dipolefunc * mo[iter]
  for (int p = 0; p < size; ++p) dipolemo[p] = dipolefunc * mo[p];
...
vecfuncT initial_guess_response(World &world,
                                vecfuncT &dipolemo,
                                functionT &vlocal,
                                vecfuncT &vf,
                                int spin,
...
functionT make_derivative_density(World &world, vecfuncT &x, vecfuncT &y, int flag, int &size) {
...
functionT calc_derivative_Jmo(World &world, int &p, functionT &drho, vecfuncT &mo, int spin, int &size) {
...
functionT calc_exchange_function(World &world, int &p, vecfuncT &dmo1, vecfuncT &dmo2, vecfuncT &mo, int &size) {
...
vecfuncT calc_djkmo(World &world, vecfuncT &dmo1, vecfuncT &dmo2, functionT drho, int spin) {
  START_TIMER(world);
...
void calc_dpolar(World &world,
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (world.rank() == 0) {
      ...
      if (param.nbeta != 0) print("Dynamic Polarizability beta  [", axis, "][", i, "]", Dpolar_beta(axis, i));
  ...
  if (world.rank() == 0) {
    if (fflag != 0) {
      for (int j = 0; j < 3; ++j) {
        for (int k = 0; k < 3; ++k) {
          if (Dpolar_alpha(j, k) < 0.001 && Dpolar_alpha(j, k) < -0.001) Dpolar_alpha(j, k) = 0.0;
          if (Dpolar_beta(j, k) < 0.001 && Dpolar_beta(j, k) < -0.001) Dpolar_beta(j, k) = 0.0;
          if (Dpolar_total(j, k) < 0.001 && Dpolar_total(j, k) < -0.001) Dpolar_total(j, k) = 0.0;
        }
      }
      print("\n");
      print("************************************************************************");
...
void solve_response(World &world) {
  ...
  for (int axis = 0; axis < 3; ++axis) {
    ...
    if (world.rank() == 0) print(" Frequency for response function = ", omega);
...
class MolecularEnergy : public OptimizationTargetInterface {
 ...
 public:
  MolecularEnergy(World &world, Calculation &calc) : world(world), calc(calc), coords_sum(-1.0) {}
      ...
      if (proto == 0) {
        if (calc.param.restart) {
          ...
          if (calc.param.polar == true) {
            if (calc.param.polar_ds == 0) calc.initial_guess_mp(world);
          }
...
int main(int argc, char **argv) {
  ...
  {  // limit lifetime of world so that finalize() can execute cleanly
    ...
    try {
      ...
      // vama1                print(" ----------------------------------------------------------\n");
      // vama1                //          calc.propagate(world,VextCosFunctor<double>(world,new
      // DipoleFunctor(2),0.1),0); vama1                calc.propagate(world,0.1,0); vama1            }
      else {
