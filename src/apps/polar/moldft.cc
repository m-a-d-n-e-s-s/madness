...
struct CalculationParameters
{
    ...
    CalculationParameters()
        : charge(0.0), smear(0.0), dconv(1e-4), L(0.0), maxrotn(0.25), nvalpha(0), nvbeta(0), nopen(0), maxiter(20), nio(1), spin_restricted(true), plotlo(0), plothi(-1), plotdens(false), plotcoul(false), localize(true), localize_pm(true), restart(false), save(true), maxsub(8), npt_plot(101), aobasis("6-31g"), core_type(""), derivatives(false), dipole(false), conv_only_dens(false), nalpha(0), nbeta(0), nmo_alpha(0), nmo_beta(0), lo(1e-10), xc_data("lda"), protocol_data(madness::vector_factory(1e-4, 1e-6)), gopt(false), gtol(1e-3), gtest(false), gval(1e-5), gprec(1e-4), gmaxiter(20), algopt("BFGS"), tdksprop(false)
          //, absolvent(false)
          //, epsilon_2(78.304)
          //, Gamma(0.0719)
          //, beta(1.3)
          ...
          //, sigma(0.3)
          ,
          polar(false), polar_ds(0), response(false), response_axis(0), response_freq(0.0), nonrotate(false), rconv(1e-6), epsf(0.0)
    {
    }
    ...
    void set_molecular_info(const Molecule &molecule, const AtomicBasisSet &aobasis, unsigned int n_core)
    {
        ...
        nbeta = (nelec - nopen) / 2;
        if (nalpha < 0)
            error("negative number of alpha electrons?", nalpha);
        if (nbeta < 0)
            error("negative number of beta electrons?", nbeta);
...
struct Calculation
{
    ...
    double PM_q(const tensorT &S, const double *MADNESS_RESTRICT Ci, const double *MADNESS_RESTRICT Cj, int lo, int nbf)
    {
        double qij = 0.0;
        if (nbf == 1)
        { // H atom in STO-3G ... often lots of these!
        ...
        for (int iter = 0; iter < param.maxiter; ++iter)
        {
            ...
            if (xc.is_dft())
            {
                ...
                if (xc.is_gga())
                {
                    ...
                    for (int axis = 0; axis < 3; ++axis)
                        vf.push_back(delrho[axis]); // dda_x

                    if (xc.is_spin_polarized())
                        for (int axis = 0; axis < 3; ++axis)
                            vf.push_back(delrho[axis + 3]); // ddb_x
                    world.gop.fence();                      // NECESSARY
                }
                if (vf.size())
                {
    ...
    Converged !
        ...
        timer : Overlap 0.02s 0.02s timer : focka eigen sol 0.00s 0.00s

                                            alpha eigenvalues
                                                [*] -
                                            20.56419765 - 1.35226451 - 0.71734259 - 0.58481048 - 0.510olmp if (!param.spin_restricted && param.nbeta != 0){//dsolmp                    if (param.localize_pm) {
...
void solve_response(World &world)
{
    ...
    if (xc.is_dft())
    {
        ...
        if (xc.is_gga())
        {
            ...
            vf.push_back(
                delrho[0] * delrho[0] + delrho[1] * delrho[1] + delrho[2] * delrho[2]); // sigma_aa

            if (xc.is_spin_polarized() && param.nbeta != 0)
                vf.push_back(
                    delrho[0] * delrho[3] + delrho[1] * delrho[4] + delrho[2] * delrho[5]); // sigma_ab
            if (xc.is_spin_polarized() && param.nbeta != 0)
                vf.push_back(
                    delrho[3] * delrho[3] + delrho[4] * delrho[4] + delrho[5] * delrho[5]); // sigma_bb
