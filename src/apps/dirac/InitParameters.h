...
namespace madness{
     ...
     struct InitParameters{
          ...
          void read(World& world, const std::string& raw_filename, const double& myc, bool restart, bool Krestricted){ 
               // Save the filename (normalized to strip .00000 chunk suffix if present)
               std::string filename = clean_archive_filename(raw_filename);
               inFile = filename;
               ...
               if(restart){
                    ...
                    if (meta.representation == Representation::nemo || meta.representation == Representation::znemo) {
                         if (world.rank() == 0) {
                              print("ERROR: restartdata holds '" + madness::to_string(meta.representation) +
                                    "' functions, but DFdriver expects standard MOs.");
                         }
                         MADNESS_EXCEPTION("restartdata representation does not match DFdriver", 1);
                    }

                    Init_total_energy = meta.current_energy;
                    spinrestricted = meta.spin_restricted;
                    L = meta.L;
                    order = meta.k;
                    molecule = meta.molecule;
                    conv_thresh = meta.converged_for_thresh;

                    input & num_occupied;        // unsigned int
