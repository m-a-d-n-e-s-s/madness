...
Fcwf::Fcwf(const std::vector<complex_function_3d>& phi, const double myc){
     MADNESS_ASSERT(m_psi.size() == 0);
...
//multiply an Fcwf by a complex function using sparse multiplication
Fcwf Fcwf::mul_sparse(const madness::complex_function_3d& phi, double tol) const {
     MADNESS_ASSERT(m_initialized);
     World& world = m_psi[0].world();
     std::vector<complex_function_3d> temp = madness::mul_sparse(world, phi, m_psi, tol);
     return Fcwf(temp, speed_of_light);
}

//multiply an Fcwf by a real function using sparse multiplication
Fcwf Fcwf::mul_sparse(const madness::real_function_3d& phi, double tol) const {
     MADNESS_ASSERT(m_initialized);
     World& world = m_psi[0].world();
     std::vector<complex_function_3d> temp = madness::mul_sparse(world, phi, m_psi, tol);
     return Fcwf(temp, speed_of_light);
