...
bool test_lrccs(World& world){
	...
	std::vector<std::string> symmetry_param={"c1", "full"};
	for(const auto symmetry:symmetry_param){
		for(const auto nemo_type:nemo_param){
			bool passed=true;
			// print the input
			// primitive version since we hopefully have a nice data format in the future
			if(world.rank()==0){
				...
				std::vector<double> expected_results = {0.3182483205,0.3794405844,0.3999497675,0.40982120415};
				std::vector<double> results;
				for(const auto& x:roots) results.push_back(x.omega);


				if(world.rank()==0){
					std::cout << "TEST FOR H2O ENDED:\n with nemo=" << nemo_type << " and symmetry=" << symmetry << "\n";
