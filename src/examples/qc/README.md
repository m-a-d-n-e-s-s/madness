...
5.3e-6), the second with `gtol`/`xtol`/`gradient_precision` pinned in the
`optimization` group at the values the retired in-SCF path used to impose (3
steps, 9.1e-7). They agree on the energy to the last printed digit
(−7.987363048) and on the bond length to 7e-5 bohr. The tight case reproduces
the removed path's numbers *exactly*, which is what establishes that moving the
optimizer out of the SCF changed no arithmetic — only who chooses the
thresholds. `nemo_lih_optimize` lands 3e-4 bohr away (3.034271), the difference
between a regularized and a plain SCF reference.
