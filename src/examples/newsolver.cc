...
struct F {
  ...
  F(const real_function_3d& x, const real_function_3d& y) : x(x), y(y) {}

  F operator-(const F& b) const { return F(x - b.x, y - b.y); }
...
struct allocator {
  ...
  F operator()() { return F(real_function_3d(world), real_function_3d(world)); }
};
...
int main(int argc, char** argv) {
  ...
  double two_electron_energy =
      inner(op(rho), rho);  // <u|rho> = <phi phi | 1/r12 | phi phi>
  double nuclear_attraction_energy =
      2.0 * inner(Vnuc, rho);  // <V|rho> = <phi|V|phi>
  ...
  double d2Ea =
      (4 * ans1) + (8 * ans2) + (4 * ans3) - (4 * (eps * ans4)) + (8 * ans5);
  double d2Eb = 4 * ans5;
  ...
  for (int j = 0; j <= 4; j++) {
    ...
    XNonlinearSolver<F, double, allocator> solver =
        XNonlinearSolver<F, double, allocator>(allocator(world));

    for (int iter = 1; iter <= 20; iter++) {
      double err =
          iterate_xy(world, solver, potential, psi, eps, ri, x, y, omega);
      if (err < 10 * thresh) break;
    }
