...
AC_DEFUN([ACX_CHECK_TYPE_TRAITS], [
  ...
  if test "$acx_cv_std_type_traits" = yes; then
    AC_DEFINE([MADNESS_HAVE_STD_TYPE_TRAITS],[1],[define if std type traits are available.])
  fi
  ...
  if test "$acx_cv_std_tr1_type_traits" = yes; then
    AC_DEFINE([MADNESS_HAVE_STD_TR1_TYPE_TRAITS],[1],[define if std::tr1::shared_ptr is available.])
  fi
